...
> **受控轮询例外**：Rokid AIUI QuickJS 只提供整包 `wx.request`，没有 SSE、WebSocket 或分块响应回调。为保证前台主动消息与离线 backlog 功能，独立 AIX 保留 25 秒长等待；页面隐藏、切模式或断开后立即停止，不做高频空请求。宿主也没有经过验证的眼镜电量变更事件，因此仅在页面可见时每 60 秒读取一次手机 CXR 上报的状态。时间与转写时长显示使用可取消的一次性 timer，不使用固定 interval。Android companion 路线仍优先使用 Relay 事件流，眼镜原生链只传音频。
...
- 配置 Route 启用状态、消息入口、Agent、人格、模型、Codex Hook、Pipeline preset、pipeline 覆盖字段和端口。
...
- 计划审批意见不会暴露给 AIUI 配置助手：该操作会写审计记录并通知真实 Agent，必须在 RibiWebGUI 或托盘中查看完整计划和步骤后显式提交。
