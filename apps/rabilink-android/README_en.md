...
This is an on-demand diagnostic, not a product requirement to complete one fixed 24-hour soak. The production path is the Android foreground service plus automatic RabiLink network connection; USB/ADB is not a runtime dependency. The monitor treats PCM actually received by RabiSpeech as the health source of truth: the selected client must be online, capture must be enabled, `last_audio_at` must be fresh, and PC-side PCM bytes must grow. ADB is supplemental and read-only; when online, it adds foreground-service, wake-lock, phone-byte, and automatic-restart evidence. After USB is unplugged, omit `-Serial` and the monitor continues instead of treating missing ADB as stopped recording. New RabiSpeech versions expose exact `received_bytes`. Against an older service, the script estimates bytes from `last_sequence × sample_rate × chunk_ms × 16-bit` and records `sequence_estimate` in `pcmByteCounterSources`.
