...
$script:RabiLinkPackage = "com.rabi.link"
...
function Resolve-MiHealthAdbPath {
    if (-not [string]::IsNullOrWhiteSpace($script:AdbPath) -and (Test-Path -LiteralPath $script:AdbPath)) {
    ...
    $projectAdb = Join-Path $PSScriptRoot "..\out\tools\android-sdk\platform-tools\adb.exe"
    if (Test-Path -LiteralPath $projectAdb) {
        $script:AdbPath = (Resolve-Path -LiteralPath $projectAdb).Path
        return $script:AdbPath
    }

    if (-not [string]::IsNullOrWhiteSpace($env:ANDROID_HOME)) {
        $androidHomeAdb = Join-Path $env:ANDROID_HOME "platform-tools\adb.exe"
        if (Test-Path -LiteralPath $androidHomeAdb) {
            $script:AdbPath = (Resolve-Path -LiteralPath $androidHomeAdb).Path
            return $script:AdbPath
        }
    }
    ...
    if (Test-Path -LiteralPath $pythonAdb) {
        $script:AdbPath = $pythonAdb
        return $script:AdbPath
    }

    throw "没有找到 adb。请把 adb 加入 PATH，设置 ANDROID_HOME，或保留工程内置 out\tools\android-sdk\platform-tools\adb.exe。"
}
...
function Invoke-HealthConnectProbe {
    ...
    try {
        Invoke-Adb -Serial $Serial -AdbArgs @("shell", "pm", "path", $script:RabiLinkPackage) | Out-Null
        ...
        Invoke-Adb -Serial $Serial -AdbArgs @(
            ...
            "broadcast",
            "-n",
            "$script:RabiLinkPackage/.modules.xiaomi.HealthConnectReadReceiver",
            "--el",
            "heart_rate_hours",
