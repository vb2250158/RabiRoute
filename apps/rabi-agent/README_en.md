...
3. Check your own `nodeId` and `connected` in `/api/lan-agent/self`. **Enable Agent** (Chinese label: **是否启用Agent**) is checked by default for new enrollments. Enabled remote Agents are used in the same way as local Agents, without a separate API or skills grant. For message delivery, select the exact Agent under **RemoteAgent(<IP address>)** in the Route's message adapter and save.

Manager owns the remote enabled state. Changing the switch immediately sends a PUT, and disabling works while offline without saving execution parameters. Saving the task, workspace or model does not re-enable an Agent; existing disabled states with saved bindings remain in effect across reconnections. An older never-authorized state that cannot be distinguished from an intentional disable also needs an administrator to confirm and enable it. Failed reads show an error rather than assuming enabled. An explicit connector-local stop still rejects execution. The changes have passed a source build but have not been deployed or verified across two computers.
...
Management enable/disable operations retain the compatible `authorization` endpoint and `enabledAgentIds` state projection, not a second API permission switch. See the [setup API contract](../../docs/lan-rabi-agent-bootstrap_en.md#api) for concurrency, idempotency and uncertain-result readback.
