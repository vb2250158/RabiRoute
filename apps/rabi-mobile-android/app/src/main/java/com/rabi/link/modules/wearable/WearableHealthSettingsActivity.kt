...
class WearableHealthSettingsActivity : Activity() {
    ...
    private fun save(startAfterSave: Boolean) {
        runCatching {
            ...
            val config = configFromForm(enabled = startAfterSave)
            persist(config)
            authKey.setText("")
            enabled.isChecked = config.enabled
            com.rabi.link.RabiConversationService.syncHealth(this)
            if (problem != null) {
    ...
    private fun syncNow() {
        ...
        if (problem != null) showGuidance(problem)
        runCatching {
            ...
            showGuidance(RabiSetupGuidance(
                "已请求总记录服务检查健康采集",
                "请求不会开启全天记录；仅在运行许可允许时采集当前窗口，断网先保存。PC 在途请求的停止尚需远端确认。",
