...
public final class RabiGlassPcBackend {
    ...
    public interface Listener {
        ...
        /** Local capture cannot be durably accepted. Owner must stop hardware; never emitted for network failures. */
        default void onCaptureBlocked(String reason) { }
    }
    ...
    private void requireCurrentResponseGeneration() {
        Long responseGeneration = responseEndpointGeneration.get();
        if (responseGeneration == null || !sameEndpointGeneration(responseGeneration, endpointGeneration))
            throw new IllegalStateException("endpoint changed during audio request; response deferred");
    }
    private volatile String activeExpectedWorkerId = "";
    private volatile String activeExpectedEndpointIdentity = "";

    public synchronized void setTargetWorkerId(String workerId) {
    ...
    private String endpointIdentity() {
        if (!configured() || targetWorkerId.isEmpty()) return "";
        try {
            java.security.MessageDigest digest = java.security.MessageDigest.getInstance("SHA-256");
            byte[] bytes = digest.digest((trimSlash(baseUrl) + "\\n" + token + "\\n" + targetWorkerId).getBytes(StandardCharsets.UTF_8));
    ...
    private static final class CaptureContext {
        final String id, source, route, policy;
        CaptureContext(String id, String source, String route, String policy) {
            this.id = id; this.source = source; this.route = route; this.policy = policy;
        }
    }

    /** Starts local admission without requiring a Relay or PC. The caller owns microphone lifetime. */
    public String beginCapture(String source, String frozenRoute, String processingPolicy) {
        return beginCapture(source, frozenRoute, processingPolicy, java.util.UUID.randomUUID().toString());
    }

    public String beginCapture(String source, String frozenRoute, String processingPolicy, String captureId) {
    ...
    public void endCapture(Runnable onDrained) {
        ...
        enqueueAudioWriteControl(() -> {
            try {
                ...
                blockCapture("seal_failure");
                listener.onError("录音封口失败，原文件保留 · " + shortError(error));
            }
        });
    }
    ...
    public void setProcessingEnabled(boolean enabled) {
        ...
        if (enabled) requestAudioStreamDrain();
    }

    public void registerCaptureEndpoint(String recordId) throws Exception {
        audioSpool.bindCaptureEndpoint(recordId, endpointIdentity());
    }

    /** Blocking import; call only from a background worker after video decoding completes. */
    public boolean appendImportedCapture(String source, String route, String policy, String recordId, File pcmFile) throws Exception {
        return appendImportedCapture(source, route, policy, recordId, pcmFile, 0L);
    }
    public boolean appendImportedCapture(String source, String route, String policy, String recordId, File pcmFile, long capturedAt) throws Exception {
        synchronized (captureLock) {
            if (captureContext != null || importingCapture) return false;
            importingCapture = true;
        }
        try {
            boolean complete;
            synchronized (audioWriteIoLock) {
                if (audioSpoolClosed.get()) throw new IllegalStateException("audio repository is closed");
                // Derived task IDs use parentCaptureId:taskHash; colon is normalized for Windows filenames.
                String durableId = recordId;
                int separator = recordId == null ? -1 : recordId.indexOf(':');
                if (separator > 0) {
                    String parent = recordId.substring(0, separator);
                    String child = recordId.substring(separator + 1);
                    if (!parent.matches("[A-Za-z0-9_-]{1,100}") || !child.matches("[A-Za-z0-9_-]{1,100}"))
                        throw new IllegalArgumentException("invalid derived capture id");
                    java.security.MessageDigest hash = java.security.MessageDigest.getInstance("SHA-256");
                    StringBuilder suffix = new StringBuilder();
                    for (byte value : hash.digest(recordId.getBytes(StandardCharsets.UTF_8))) suffix.append(String.format(java.util.Locale.US, "%02x", value & 255));
            ...
            return complete;
        } finally { synchronized (captureLock) { importingCapture = false; } }
    }

    public JSONArray listCaptureRecords(int limit) {
        return com.rabi.link.recording.RabiAudioRecordRepository.listCaptureRecords(context, limit);
    }

    public File exportCaptureWave(String captureId) throws Exception {
        return com.rabi.link.recording.RabiAudioRecordRepository.exportCaptureWave(context, captureId);
    }
    ...
    public synchronized void configure(String baseUrl, String token, String deviceId) {
        if (!this.baseUrl.equals(trimSlash(baseUrl)) || !this.token.equals(clean(token))) {
            endpointGeneration++;
            audioCapabilityCache.invalidate();
    ...
    public void streamPcmFromSource(byte[] pcm, String sourceDeviceKind) {
        ...
        if (!audioWriteQueue.offer(copy, source, route, capture == null ? "" : capture.id, capture == null ? "agent" : capture.policy,
            ...
            if (!audioStorageFailureReported) {
                ...
                listener.onError("录音落盘队列已满 · 已记录背压缺口，采集已暂停");
            }
            blockCapture("capture_backpressure");
    ...
    private void drainAudioStream() {
        // Lack of setup is a deferred processing state, never a recording failure.
    ...
    private String activeAudioProcessingPolicy = "agent";
    private String activeAudioCaptureId = "";
    private volatile long activeAudioEndpointGeneration = -1L;
    private boolean supportsCaptureProcessing(String policy) {
        Boolean cached = audioCapabilityCache.get(policy);
        if (cached != null) return cached;
        long revision = audioCapabilityCache.revision();
        boolean agent = false, transcribe = false;
        try {
            JSONObject response = jsonRequest("GET", "/api/rabilink/speech/v1/capabilities", null, null, 10000);
            JSONObject capability = response.optJSONObject("rabilinkAudioStream");
            if (capability == null && response.optJSONObject("data") != null) capability = response.getJSONObject("data").optJSONObject("rabilinkAudioStream");
            agent = capability != null && capability.optBoolean("expectedWorkerFencing");
            if (agent && capability.optBoolean("processingPolicyFrozen")) {
                JSONArray policies = capability.optJSONArray("processingPolicies");
                if (policies != null) for (int i = 0; i < policies.length(); i++)
                    if ("transcribe".equals(policies.optString(i))) transcribe = true;
            }
        } catch (Exception ignored) { /* Failure is cached until an explicit retry/config/network event. */ }
        if (!audioCapabilityCache.store(revision, agent, transcribe)) return false;
        return Boolean.TRUE.equals(audioCapabilityCache.get(policy));
    }

    private void ensureAudioStream(String sourceDeviceKind, String routeProfileId, String processingPolicy, String captureId) throws Exception {
        ...
        if (!activeAudioStreamId.isEmpty()
                ...
                && activeAudioStreamRoute.equals(route) && activeAudioProcessingPolicy.equals(processingPolicy)
        ...
        synchronized (this) {
        requireCurrentResponseGeneration();
        activeAudioStreamId = streamId;
        activeAudioEndpointGeneration = endpointGeneration;
        activeAudioStreamSource = sourceDeviceKind;
        activeAudioStreamRoute = route;
        activeAudioProcessingPolicy = processingPolicy;
    ...
    private void flushAudioStreamChunk() throws Exception {
        ...
        if (pending == null || !processingEnabled) return;
        if (!pending.processingPolicy.equals(activeAudioProcessingPolicy) || !pending.captureId.equals(activeAudioCaptureId))
            throw new IllegalStateException("audio stream capture identity mismatch");
    ...
    private void stopActiveAudioStream() {
        ...
        String streamId = activeAudioStreamId;
        if (activeAudioEndpointGeneration != endpointGeneration) { resetAudioStream(); return; }
        try {
    ...
    static String captureStreamId(String deviceId, String source, String route, String policy, String captureId) {
        if (clean(captureId).isEmpty()) return safeStreamId(deviceId + "-" + source + "-" + routeKey(route) + "-audio");
        try {
            // Length-prefixed tuple avoids delimiter ambiguity; full digest survives the server's 100-character limit.
            java.security.MessageDigest digest = java.security.MessageDigest.getInstance("SHA-256");
            for (String part : new String[]{deviceId, source, route, policy, captureId}) {
                byte[] bytes = clean(part).getBytes(StandardCharsets.UTF_8);
                digest.update(java.nio.ByteBuffer.allocate(4).putInt(bytes.length).array());
                digest.update(bytes);
            }
            StringBuilder id = new StringBuilder("capture-audio-");
            for (byte value : digest.digest()) id.append(String.format(java.util.Locale.US, "%02x", value & 255));
            return id.toString();
        } catch (Exception error) { throw new IllegalStateException("cannot derive audio stream identity", error); }
    }
    ...
    private byte[] request(String method, String path, String contentType, byte[] body, int timeout) throws Exception {
        ...
        synchronized (this) {
            requestGeneration = endpointGeneration;
            boolean fenced = path.startsWith("/api/rabilink/speech/") && !activeExpectedEndpointIdentity.isEmpty();
            if (fenced && !activeExpectedEndpointIdentity.equals(endpointIdentity()))
                throw new IllegalStateException("recorded target changed; audio processing deferred");
            ...
            if (fenced) connection.setRequestProperty("X-RabiLink-Expected-Worker-Id", activeExpectedWorkerId);
        }
        ...
        synchronized (this) {
            if (path.startsWith("/api/rabilink/speech/") && !sameEndpointGeneration(requestGeneration, endpointGeneration))
                throw new IllegalStateException("endpoint changed during audio request; response deferred");
            responseEndpointGeneration.set(requestGeneration);
        }
    ...
    static String normalizeCaptureSource(String source) {
        if ("mobile".equals(source) || SOURCE_PHONE.equals(source)) return SOURCE_PHONE;
        if (SOURCE_GLASSES.equals(source)) return SOURCE_GLASSES;
        throw new IllegalArgumentException("unsupported capture source");
    }
