...
object WearableHealthSettings {
    ...
    fun save(context: Context, config: WearableHealthConfig, authKey: String? = null) {
        val prefs = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
        val participationStartedAt = if (config.enabled && !prefs.getBoolean("enabled", false))
            System.currentTimeMillis() else prefs.getLong("participationStartedAt", 0L)
        prefs.edit()
            .putLong("participationStartedAt", participationStartedAt)
