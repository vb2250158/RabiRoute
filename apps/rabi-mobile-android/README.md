...
- 全天记录 owner 维护唯一常驻状态通知，点击回记录入口；最新回复使用独立可清除的普通消息通知，点击直达对应 `routeProfileId` 聊天，不覆盖采集状态。
...
| 眼镜状态协调（原独立 service 已移除） | 由唯一全天记录 owner 协调设备状态回调，不再自行发布另一条常驻通知；真实电量、充电与断线行为仍需本轮设备验收。 |
...
```text
apps\rabi-mobile-android\out\mi-health-cloud\
```
