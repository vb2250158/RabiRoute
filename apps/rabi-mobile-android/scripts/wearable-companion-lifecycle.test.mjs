...
test("legacy apps entry points cannot create a second lifecycle", async () => {
  const [install, start, resolve, sync, manifestText, manager, workerService, packageStart, controlPlane] = await Promise.all([
    read("apps/rabi-mobile-android/scripts/Install-RabiLinkWearableCompanionTask.ps1"),
    read("apps/rabi-mobile-android/scripts/Start-RabiLinkWearableCompanion.ps1"),
    read("apps/rabi-mobile-android/scripts/Resolve-RabiRouteHostManagerUrl.ps1"),
    read("apps/rabi-mobile-android/scripts/Sync-MiHealthWearableToRabiLink.ps1"),
