...
- Generic Webhook, Heartbeat, NapCat, and WeCom now have Fiber-owned lifecycles. Each WeCom mount creates one inbound SDK client; disposal disconnects it and invalidates late events, connection failure rolls the client back and records `error`, and missing credentials do not create a client. The Gateway entry mounts registered message adapters first and keeps the compatibility factory for unmigrated adapters such as Weixin and Feishu.
