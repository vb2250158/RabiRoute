...
- Generic Webhook, Heartbeat, and NapCat now have complete Fiber-owned lifecycles. NapCat waits for every enabled OneBot WebSocket listener, rolls back earlier ports when a later instance fails, and terminates clients, closes all listeners, rejects new message work, and records `disabled` on disposal. The Gateway entry mounts registered message adapters first and keeps the compatibility factory for unmigrated adapters such as WeCom, Weixin, and Feishu.
