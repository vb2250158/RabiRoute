...
- Generic Webhook, Heartbeat, NapCat, WeCom, and Weixin now have Fiber-owned lifecycles. Weixin QR requests, long polling, waits, and inbound image downloads share one cancellation signal; disposal aborts the active request, awaits loop exit, rejects stale results before state updates or message delivery, and records `disabled`. The Gateway entry mounts registered message adapters first and keeps the compatibility factory for unmigrated adapters such as Feishu.
