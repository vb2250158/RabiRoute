...
- The resident Gateway now uses one Cordis root Context and mounts the Agent Adapter Registry, Message Adapter Registry, and Contribution Registry under that root. `src/index.ts` only dispatches invocation types. One-shot alert, replay, manual-trigger, role-panel, plan-feedback, speech, and Direct Agent Envelope commands cannot start the Message Adapter Runtime or Contribution Runtime. `create-gateway-host` is complete; the next stage establishes the Manager Plugin Runtime and unified Plugin Catalog API.
- WebGUI and Desktop remain extensible minimal hosts, and plugins can contribute pages, actions, status, and settings. Ordinary DSH plugins run in the same Node process, while Cordis `isolate` separates service scope only. RabiRoute provides an optional separate-process policy for unknown or high-risk plugins.
