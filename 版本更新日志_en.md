...
- Manager Plugin Runtime and the unified Plugin/Contribution Catalog API are implemented. Manager mounts plugin state and contribution catalogs under its own root Context. Built-in plugins declare current WebGUI navigation, settings sections, status cards, and the Desktop settings entry. `GET /api/plugins/catalog` returns one snapshot; `host=web|desktop` filters presentation contributions only, while the plugin-instance list stays complete. A failed or explicitly unmounted plugin can remount under the same instance ID. Manager startup now uses one rollback path: listener failure or later initialization failure stops started resources, closes HTTP/SSE, removes signal handlers, and disposes the Manager root. WebGUI/Desktop catalog consumption is still pending.
- “Everything is a plugin” includes presentation: WebGUI and Desktop are minimal hosts whose pages, actions, status, settings, menus, hotkeys, and themes come from the plugin contribution catalog. Ordinary DSH plugins run as Fibers in the same Node process, while Cordis `isolate` separates service scope only; dedicated capabilities create subprocesses and Workers explicitly. RabiRoute provides an optional separate-process policy for unknown or high-risk plugins.
