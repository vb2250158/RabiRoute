...
- Added six Manager service instances with no UI contributions: `manager:gateway-runtime`, `manager:rabilink-relay`, `manager:memory-consolidation`, `manager:message-processing-automation`, `manager:plan-feedback-delivery`, and `manager:napcat-supervisor`. The new group owns plan-change subscriptions, knowledge-callback and Agent-response reminders, plan-feedback recovery scans and delivery, and the NapCat startup-login check. Deactivation clears timers, removes subscriptions, prevents stale knowledge callbacks from rescheduling, cancels the remaining NapCat account queue, and waits for started reminders, scans, feedback deliveries, or the current login check. `manager:performance` creates a fresh `PerformanceApi` and route batch per activation. Gateway deactivation stops the full process tree and waits for exit; Persona Sync LAN rejects stale callbacks by generation; memory deactivation stops instance-owned one-shot processes and waits for the scheduler. Remaining centralized optional API dispatch and post-restart runtime acceptance are still pending.
