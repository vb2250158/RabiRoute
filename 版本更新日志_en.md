...
- Validation: TypeScript type check passed; the `codexHookContext`, `roleKnowledge`, `personaPlanWorkflow` and `agentCompletionDelivery` test files pass 128/128, including 2 new regression cases covering "a cold role does not break healthy delivery" and "a missing roles root does not crash". The before/after difference was reproduced with an isolated script: it threw before the fix, and after it returns `ignored / no_enabled_plan_task_binding` while creating the storage layout.

- WorkBuddy Agent endpoint message delivery is implemented: delivery goes through the bound task's session-process local gateway (`POST /api/v1/runs`) with `source.conversation.id` pinned to the full session ID, so the message enters that task's conversation area as a genuine user turn executed by that task with its own model, tools and approvals; same-id redelivery creates no new task. Verified by measurement and reproduced from a separate process with no `CODEBUDDY_*` environment variable. The credential is recorded once in the local ignored file `data/workbuddy-auth.json`; when missing, delivery fails closed and never sends an unauthenticated request. Delivery outcomes distinguish unreachable (safe to retry), rejected (not applied), credential-rejected (never replayed) and unknown (check the receipt first). Route binding mirrors DSH and never stores the credential in route config; WebGUI gained an agent card and a task dropdown with liveness. Still experimental: the desktop exposes no pairing handoff and cold start is unverified. See the [WorkBuddy integration plan](docs/workbuddy-agent-adapter-plan_en.md).
- Added experimental WorkBuddy Agent endpoint discovery: it reads the session process descriptors and the "workbuddy.db" task database, resolves a single newest task by full session ID or by saved name plus normalized workspace, reports a create-once state when nothing matches, and fails closed on cwd conflict or a stale ID; workspace comparison follows Windows case-insensitive semantics and dropdown names prefer the user-defined title. See the [WorkBuddy integration plan](docs/workbuddy-agent-adapter-plan_en.md).
- The WorkBuddy Agent endpoint can now be added and bound from the Route configuration page: the agent list gained a WorkBuddy card, and its parameter panel offers a task dropdown with liveness, task name, work directory and last gateway address, reusing the shared agent scan endpoint. The capability declaration is narrowed to message processing plus lifecycle hooks — plan assistants and memory consolidation go through the Codex/DSH thread driver, so neither is declared until those paths are adapted, and the UI no longer offers panels whose calls would necessarily fail. `gatewayConfigModel` gained four WorkBuddy binding fields with normalization (the session ID accepts UUID and `interactive-<pid>` forms, the gateway address accepts loopback HTTP only), the instance-binding allowlist now uses `isAgentAdapterType`, and the WebGUI copy is translated both ways. Still experimental: no desktop pairing handoff yet.
- Verification: TypeScript type checking passes for both the backend and the WebGUI; the three WorkBuddy test files plus related regressions pass 55/55 and the WebGUI suite passes 302/302.

- The WorkBuddy Agent endpoint supports lifecycle hooks on par with Codex and DSH: a new `plugins/rabi-workbuddy-context` package (with `hooks/hooks.json` as the single declaration source, isomorphic to `rabi-codex-context`) covers `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse` and `Stop`, reusing the same Manager contract for entry-context injection and plan completion reporting. CodeBuddy's hooks are the same family as Codex's (same event names, same input contract, same `hookSpecificOutput.additionalContext` output), and writing them into `~/.workbuddy/settings.json` was measured to actually execute. Installation does not go through the plugin CLI: a `codebuddy plugin` subcommand takes over ninety seconds to start and its executable path moves with every desktop upgrade, so the Manager instead copies the scripts to a stable path, substitutes the plugin-root placeholder in `hooks/hooks.json` with that absolute path, and merges the result into the user settings — replacing only entries carrying its own fingerprint, preserving the user's hooks and every other settings key, staying idempotent, and refusing to write when the settings file is not valid JSON. The `workbuddy` manifest now declares `managedTasks.hooks`, so that card's hook panel and update button follow from the capability declaration. Still experimental: end-to-end validation on a real interactive task is pending.
- Verification: TypeScript type checking passes; the WorkBuddy hook plugin passes 10/10, `hookInstallation` 5/5, the three WorkBuddy test files plus related regressions 122/122, the WebGUI suite 302/302 and persona sync 5/5, and `git diff --check` is clean.

- The capability model gains `deliveryReceiptRecovery` with a deliberately narrow meaning: receipt recovery needs a transport that keeps a locally readable log of inbound prompts, which only Codex Desktop's rollout files satisfy. A gateway transport that returns a one-shot acceptance receipt and keeps no readable inbound log does not declare it. The recovery entry point therefore reports "the adapter has no receipt log" as the distinct `adapter_has_no_receipt_log` outcome instead of the generic `not_recoverable` dead end, so a missing capability is no longer misdiagnosed as a failed delivery. Agent capability declarations are likewise narrowed to a verified set per adapter instead of a shared broad baseline.

## 0.3.3 - 2026-09-15

- Validation: TypeScript type check passed. The full backend suite ran 2075 tests: 1985 passed, 6 failed, 81 cascaded cancellations, 3 skipped. Those six are exactly the process and lease cases that already reproduce at the previous version's pre-commit baseline, so this change adds no new failures. The 12 affected TypeScript test files pass 86/86, the desktop tray Python suite passes 34/34, and `git diff --check` is clean. The two RabiSpeech Python test files only had a display name replaced; the local venv lacks fastapi so they could not be executed, and the create/assert name pairing was verified by inspection instead.

- Scrubbed private identifiers from the public repository. Plan-secretary prompt rule 6 is now generic rather than project-specific (external editor occupancy, unavailable dependencies or tools, and shared-test queues are not a global wait; never stop another process or cancel someone else's tests; the task keeps working on assets, serialization contracts, non-interactive runners, CLI and narrowed scope) with unchanged intent. The managed issue-ledger module is renamed from `pangHuIssueLedger` to `roleIssueLedger`, with exports renamed to `RoleIssueLedgerItem` and `readRoleIssueForPlan`.
- Test fixtures and documentation examples now use generic workspace paths, generic names and synthetic account numbers instead of private directories, real names or a real QQ number.
- The Windows tray app identity moves from an organization-prefixed AppUserModelID to the neutral `RabiRoute.Desktop`, and `APP_ORGANIZATION` becomes `RabiRoute`. Migration: Windows treats the tray and task window as a new application, so existing taskbar pins and notification settings must be recreated.
- Repository hygiene: `RabiPlanCache/` (a runtime-written Manager plan-queue cache) is no longer part of the source submission, `artifacts/` (local hot-patch candidate distributions) is ignored, and the stale ignore rule for the deleted archive directory is gone. Local personas, plans, credentials and patch receipts are preserved; examples never overwrite runtime data.
