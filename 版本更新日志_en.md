...
- Fixed Agent-to-Agent provenance showing the first initialization prompt as the source task name. Manager still verifies the source against the complete task ID and Desktop state, then overlays the current sidebar name from the session index instead of trusting SQLite's first-message title. Message Agent initialization also resolves the Primary Persona's current name by complete ID rather than retaining a stale Route display name. Xinghai's saved Primary binding name is now `星海建造师 策划 程序`; its task ID and workspace are unchanged.
