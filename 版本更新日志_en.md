...
- Migration of all 26 built-in Manager plugins is complete. The central HTTP chain is limited to LAN authentication, the read-only write gate, plugin route dispatch, Manager SSE, plugin catalog/reconciliation, static assets, JSON 404 for control paths, and WebGUI HTML fallback for all other paths.
...
- Page, navigation, command, menu, hotkey, and theme references must resolve within the same plugin instance and registration batch. Recovery entries remain available when the catalog is unavailable. A controlled Extension Host for arbitrary third-party presentation code remains future work.

### Unified validation

- Status: pending. After all refactoring and documentation changes are complete, the main task will run `git diff --check`, TypeScript typecheck, backend and script tests, backend/WebGUI builds, and configuration checks together. The main task will record the actual commands and results after they pass.
