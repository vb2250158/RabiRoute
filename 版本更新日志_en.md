...
- Approval semantics are stricter: a current step waiting for approval, plan confirmation, or authorization must explicitly set `isBlocked=true` and name who must approve what. The secretary continues follow-up, but implementation is not repeatedly dispatched for a running metric. Incomplete approval contracts are shown as approval-disabled; QA, missing materials, and external artifacts are not blocked automatically.
- Existing runtime plans can be previewed with `node scripts/migrate-plan-approval-blocking.mjs --role-dir=data/roles/<RoleId>` and migrated only with explicit `--apply`. The script backs up changed files inside the role directory, adds approval-blocking facts, and deliberately leaves unknown approvers, provenance, recommendations, and alternatives incomplete rather than inventing evidence.
...
- Added Route-scoped Console and knowledge paths: `#/routes/<Route-config-name>/overview|knowledge?webgui_token=<key>`. Changing the sidebar **Current Route** immediately redirects the current Route page URL. The Console can copy a complete LAN link for bookmarking or authorized sharing, while the address bar still removes the key under the existing safety rule.
