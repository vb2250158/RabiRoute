...
- Fixed intermittent long waits before the tray context menu appeared and made the tray a frontend of the same Manager backend as RibiWebGUI. Qt-free `DesktopRefreshService` calls only `/gateways?summary=1` and `/api/roles/:roleId/*` for Route, plan, memory, conversation, and avatar DTOs; the packaged tray no longer imports local plan/memory repositories or reads runtime `data/`. Generic `qt_async` runs every Manager I/O operation in the thread pool, while the Qt thread only applies results. Hidden panels request neither conversation/avatar data nor widget rebuilds, refresh application waits while the menu is visible, unchanged state does not rebuild menus or panels, and entries beyond five are lazy. Windows uses an `activated(Context)` → non-blocking `QMenu.popup()` fast path and prewarms top-level layout. Coverage now locks backend/frontend boundaries, slow I/O, the 100 ms popup entry, and a 50-persona stress case.
