...
- Manager Plugin Runtime and the unified Plugin/Contribution Catalog API are implemented. Manager mounts plugin state and contribution catalogs under its own root Context. `GET /api/plugins/catalog` returns one snapshot; `host=web|desktop` filters presentation contributions only, while the plugin-instance list stays complete. Schema v2 rebuilds plugin manifests and contributions from field allowlists and never publishes arbitrary `target`, `endpoint`, `query`, `body`, or `resourceRoot` values. A failed or explicitly unmounted plugin can remount under the same instance ID. Listener failure or later initialization failure stops started resources, closes HTTP/SSE, removes signal handlers, and disposes the Manager root.
- “Everything is a plugin” includes presentation: WebGUI and Desktop are minimal hosts. WebGUI now generates primary navigation, utility navigation, the User Guide, and the persona-sync entry from the catalog. Desktop generates the Plugin tray submenu asynchronously, caches the latest successful catalog, and supports only `desktop.open-webgui` and `desktop.open-settings`. Fixed recovery entries remain when catalog loading fails. Ordinary DSH plugins run as Fibers in the same Node process, while Cordis `isolate` separates service scope only; dedicated capabilities create subprocesses and Workers explicitly. RabiRoute provides an optional separate-process policy for unknown or high-risk plugins.
...
- Added eight Manager service instances with no UI contributions. This slice adds `manager:fennenote-output` and `manager:message-processing-control`. The FenneNote playback/reply compatibility endpoints reuse the shared Outbox output service and are removed with in-flight request cancellation on deactivation. Message-processing board, requirement, send-context, outcome, and knowledge-callback APIs are registered by a separate control plugin while records and background automation remain owned by their existing modules. The previous six instances are: `manager:gateway-runtime`, `manager:rabilink-relay`, `manager:memory-consolidation`, `manager:message-processing-automation`, `manager:plan-feedback-delivery`, and `manager:napcat-supervisor`. The new group owns plan-change subscriptions, knowledge-callback and Agent-response reminders, plan-feedback recovery scans and delivery, and the NapCat startup-login check. Deactivation clears timers, removes subscriptions, prevents stale knowledge callbacks from rescheduling, cancels the remaining NapCat account queue, and waits for started reminders, scans, feedback deliveries, or the current login check. `manager:performance` creates a fresh `PerformanceApi` and route batch per activation. Gateway deactivation stops the full process tree and waits for exit; Persona Sync LAN rejects stale callbacks by generation; memory deactivation stops instance-owned one-shot processes and waits for the scheduler. Remaining centralized optional API dispatch and post-restart runtime acceptance are still pending.
- Unregistered or deactivated `/api/*` paths now return a JSON 404 instead of falling through to the WebGUI single-page-app HTML response.
- The catalog now controls the WebGUI Desktop-settings section, speech summary, and performance summary. Desktop Diagnostics reads catalog-declared speech status, performance status, and Desktop settings. Both hosts consume only active owners with matching `pluginId`/manifest host, locally supported `requiredCapabilities`, and fixed renderer/query/schema/command/handler IDs; unknown contributions are not rendered as extension entries, and Desktop does not issue their queries or commands. WebGUI keeps the existing settings, speech, and performance clients, while Desktop settings changes still open the unified Settings page.
- WebGUI pages now accept catalog declarations through a fixed `routeId -> rendererId -> Vue component` registry, and navigation must reference a page from the same plugin instance and registration batch. The first catalog failure opens a fixed recovery page; later failures retain the latest successful page catalog. WebGUI and Desktop themes accept only the host-owned resource IDs for `system`, `light`, and `dark`. The Desktop catalog can also enable the existing system-screenshot and clipboard-pin hotkeys; `hotkey.commandId` must reference a safe command from the same batch, while the actual binding still comes from the user's Desktop settings. The current catalog controls host-pre-registered entries only; third-party page components, theme resources, and command handlers remain part of the later isolation stage. Catalog responses carry a Manager runtime generation, so Desktop retries an initial failure and accepts lower revisions after a Manager restart when the generation changes.
