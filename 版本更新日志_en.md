...
- Approval submission no longer waits synchronously for the Agent role-panel child process. Manager returns `pending` immediately after persistence, deduplicates background delivery by `feedbackId`, applies a 45-second terminal deadline, and emits the final state as `plan_feedback_changed`. WebGUI updates only the affected card instead of reloading all plans and memory; the tray approval request boundary is reduced to five seconds.
