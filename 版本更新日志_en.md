...
- Generic Webhook and Heartbeat now have complete Fiber-owned lifecycles. Webhook waits for its HTTP listener, rolls back failed activation, and closes the port on disposal. The Heartbeat Fiber owns every timer, clears future triggers on disposal, prevents stale callbacks from running or rearming, records `disabled`, and cleans timers created before activation failure. The Gateway entry mounts registered message adapters first and keeps the compatibility factory for unmigrated adapters such as NapCat.
