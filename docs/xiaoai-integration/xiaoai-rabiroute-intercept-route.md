...
6. `data/route/xiaoai/routeConfig.json`
   - 小爱入口规则配置。
...
5. `examples/data/route/voice-chat/routeConfig.json`：已有 webhook + TTS 语音管道示例。
6. `roleMessageConfig.json`：已有 `voice-chat` 配置，能把语音转写投给 Agent。
...
```text
data/route/xiaoai/routeConfig.json
data/roles/Rabi/roleMessageConfig.json 里新增 configName = "xiaoai"
```
...
roleMessageConfig notificationRules
...
这个映射很重要：小爱接入不能绕过 `roleMessageConfig` 直接调 Agent，也不能让 Agent 直接调小米账号或 HA token。

### 推荐落地顺序

1. 不改代码，先用现有 `/webhook` 测通小爱文本进入 `voice_transcript`。
2. 新增 `data/route/xiaoai/routeConfig.json` 和 `roleMessageConfig` 的 `xiaoai` config。
