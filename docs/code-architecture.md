...
`src/roleKnowledge.ts` 定义九个唯一合法的计划状态：`分析中 / 待审批 / 执行中 / 等待打包 / 等待 QA / 待讨论 / 暂停 / 完成 / 关闭`。`plan.status` 是列表、详情、排序、WebGUI 和 Qt 托盘共用的唯一状态真源；步骤、`waitingFor`、`approvalRequest` 与交付证据只用于校验和说明，不能派生或覆盖状态。`archiveStatus=未归档 | 已归档` 是独立归档变量，只有 `完成` 或 `关闭` 可以在超时后归档，归档时保留原 `status`。`src/roleKnowledgePresentation.ts` 只按 `plan.status` 返回 tone、色板、排序等级与视图；`presentation.status` 必须与其完全相同。`src/roleKnowledgePagination.ts` 使用同一状态生成筛选面和 `counts.stages`。已归档计划在普通列表和关键词召回前被排除，只能通过明确计划 ID 或归档视图读取。旧五状态、`workPhase` 和 `discussionState` 只在读取边界用于兼容映射，并在下一次规范写入时清理。
