...
> Status: current route-selection guide. Since 2026-07-18, the native app is the primary development route. AIUI feature development is paused; its existing implementation remains as historical and compatibility evidence.
...
flowchart TB
    ...
    subgraph NativeApp["Native app · highest freedom / system responsibility"]
        P1["Glasses app owns<br/>audio/media I/O · HUD"]
        P2["Phone companion owns<br/>Relay transport · glasses settings · cursor/queues"]
        P1 <--> P2
    end
...
| Speech and reasoning owner | The hosted agent interprets a turn and invokes tools; RabiRoute receives work through OpenAPI/Relay | The historical implementation uses AIUI native ASR/TTS; product development is paused | The app captures and plays audio only; glasses-message ASR, Agent reasoning, and TTS all run on the Rabi PC |
| Capability ownership boundary | Lingzhu owns wake, ASR/TTS, conversation UI, and tool orchestration; the project mainly owns OpenAPI tools and backend processing | Lingzhu owns agent identity, entry, and page launch; the AIUI page owns HUD, ASR/TTS, and its foreground state machine; the PC owns primary Agent capability | Glasses own audio/media I/O and HUD; phone owns device connection, Relay credentials, cursor, and glasses settings; PC owns ASR/TTS, Agent, persona, memory, configuration, and the action gate |
...
| Best fit | Fast Q&A, tool calls, compatibility entry, and low-cost experiments | Historical reference and regression testing | Current primary route: native HUD, push-to-talk audio upload, PC ASR/TTS, audio playback, and later device capabilities |
| Current maturity | Existing OpenAPI/plugin compatibility path; experimental compatibility route | Development paused; code and evidence retained | Buildable push-to-talk audio-client baseline; public Relay and physical-glasses end-to-end acceptance remain |
...
Current status: **new feature development is paused**. The AIX source, tests, and evidence remain available for regression, protocol reference, and compatibility work. Craft review and AIUI physical-device closure are no longer blockers for the current product route.
...
This route installs a standalone Android APK on the glasses and uses the phone app as its backend. The glasses only capture/send PCM, receive/play PCM, and send device media. The phone stores Relay credentials, selected PC, cursor, and glasses settings. The glasses neither host a second ASR/TTS/Agent stack nor access Relay directly.
...
- The phone backend should reuse the same Relay observation, unified-ledger, and Outbox contracts instead of creating another Agent, memory system, or outbound permission model. PC configuration is edited through the remote WebGUI opened by the phone.
...
- [Android RabiLink probe](../apps/rabi-mobile-android/README_en.md)
