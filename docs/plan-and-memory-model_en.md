...
`暂停` is a real top-level plan state for an explicit temporary stop. A current step waiting for approval, plan confirmation, or authorization remains in progress but must set `isBlocked=true`, a concrete `blockedBy`, and `waitingFor`. The secretary keeps following up, while the business task is not re-dispatched merely to satisfy a running metric. QA, missing materials, and external artifacts do not become blocked automatically.
...
A current step that requires approval should include a complete `approvalRequest`. `approver`, `request`, `recommendation`, `alternatives`, and `reason` state the owner, decision, preferred and fallback choices, and rationale. `files` lists exact paths, `create/modify/delete/move`, and the concrete edit; `commands` contains complete commands, purpose, and expected effects; `changes` identifies configuration, database, cloud, or external-system targets. `validation`, `rollback`, and `outOfScope` define acceptance, recovery, and explicit exclusions. `requestedAt`, `sourceMessageId / feedbackId`, and `responseStatus` record request provenance and receipt state. At least one of `files / commands / changes` must be concrete. Legacy plans remain readable. Missing required fields produce `presentation.approval.state=incomplete` and `enabled=false`, so formal approval remains disabled until the contract is complete. RibiWebGUI still accepts missing details, change requests, and attachments for the Agent to repair the contract; such feedback does not approve the plan.

When upgrading an existing runtime persona, first run `node scripts/migrate-plan-approval-blocking.mjs --role-dir=data/roles/<RoleId>` for a read-only report and add `--apply` only after confirming the target. Apply mode copies each original plan into `migration-backups/approval-blocking-<timestamp>/` under that role, then adds the blocking facts and a compatibility approval contract. The script does not guess an approver, provenance, recommendation, alternatives, or request time. Missing evidence therefore remains `incomplete` until the Agent fills it from real messages and investigation results.
...
WebGUI and tray submissions use `kind=approval_suggestion`, `author=user`, `source=webgui|tray`, and `notifyAgent=true`. RibiWebGUI also accepts ordinary files from the picker and clipboard images pasted with `Ctrl+V` inside the feedback field. A submission may contain up to 8 attachments, limited to 10 MiB each and 25 MiB in total. Typing `@` opens the current plan's top-level attachment candidates with filename filtering, arrow-key navigation, Enter selection, and Escape dismissal. The selected file appears as a readable mention token while the browser submits only its attachment ID. Manager verifies that every ID belongs to the current plan and stores the managed metadata plus local path as an audit snapshot. Newly uploaded content is materialized under the private runtime directory `plans/feedback/attachments/<feedbackId>/`, and feedback JSONL never embeds binary content. Manager persists the feedback and immediately returns `deliveryStatus=pending`, then emits an independent `plan_feedback` system event carrying the text, new uploads, and mentioned plan attachments through Forwarding, AgentPacket, and the existing Agent adapter to the bound Agent; the UI no longer waits for the Agent child process. This event is not written to the role-panel timeline or unified conversation ledger, and its recent-message budget is fixed at `0`. Completion or failure appends a `delivered / failed` state for the same `feedbackId` and emits `plan_feedback_changed`. WebGUI reacts by reading only that plan's feedback summary instead of reloading all plans and memory. A failed notification restores the submitted text and attachments for a same-ID retry; the same `feedbackId` cannot silently replace uploaded content or plan-attachment references.
...
While background delivery of the previous feedback is pending, WebGUI lets the user edit the next text, pasted image, and attachments but prevents another submission until the previous feedback reaches a terminal state. A visible status row explains the reason and recovery condition so “cannot send twice yet” is never presented as “cannot express feedback.” The same status surface covers an unavailable approval entry, an active save, or a missing Route.

## Manager presentation order and plan views

The Manager still derives `Blocked` only from explicit `isBlocked=true` plus a non-empty `blockedBy`; the Agent or secretary must write that fact for approval, plan-confirmation, and authorization waits. QA, missing materials, and external artifacts are not blocked automatically. `Awaiting QA` is likewise derived only from an in-progress structured `qa-* / verify-*` step selected by `steps/currentStepId`; an implementation step remains `In progress` even if its free text mentions a QA gate or says QA has not been notified. `presentation.approval.state` is `none`, `incomplete`, or `ready`: incomplete contracts have `enabled=false` and cannot be formally approved, while WebGUI still exposes remediation feedback for missing details or change requests. The plan card directly shows the approver, decision, recommendation and alternatives, reason, files, commands, external changes, validation, rollback, out-of-scope items, attachments, request time, source identifier, and receipt status.
...
The Qt panel displays current plans, recent memory, consolidated memory, and diagnostics. It does not create, complete, archive, delete, normalize, or migrate plan/memory content. Approval-enabled plan cards may append formal feedback through Manager without changing the plan itself. For incomplete contracts, the Qt input stays disabled while RibiWebGUI separately permits remediation details or change requests that are not treated as approval.
