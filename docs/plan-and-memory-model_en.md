...
The filesystem is the source of truth. The Manager API reads and writes these files. Qt and WebGUI keep plan content read-only; approval feedback stays on Manager-declared approval steps, while RibiWebGUI also accepts whole-plan guidance for Analyzing or Executing plans outside approval.
...
`plan.status` is the single status source and accepts exactly:
...
`archiveStatus` is independent and accepts only `未归档` or `已归档`. Only completed or closed plans are archived after the delay, without changing `plan.status`. Archived plans do not participate in keyword recall. Manager, WebGUI, and the tray display `plan.status` directly; step text and approval metadata never derive a second display status.
...
An explicit discussion wait writes `status=待讨论`. An ordinary pause writes `status=暂停`; resumption chooses the real next phase, normally `分析中` or `执行中`.
...
Completed or Closed plans whose `archiveStatus=未归档` remain visible for confirmation. A role-knowledge snapshot archives them when the latest `updatedAt` is more than the current fixed 72-hour window old. It changes only `archiveStatus` to `已归档`, preserves `plan.status`, sets `archivedAt`, and moves the whole directory from `plans/active/<planId>/` to `plans/archive/<planId>/`.
...
Candidates include all non-archived plans, unconsolidated recent memories, consolidated memories, and non-archived role skills. Plans in `分析中`, `待审批`, `执行中`, `等待打包`, or `等待 QA` enter the default active-plan index. Plans in `待讨论`, `暂停`, `完成`, or `关闭` remain searchable but are not treated as active. Active plans and active recent memories receive only a small ranking bonus. A candidate must still match the current message to enter required reads.
...
While background delivery is pending, WebGUI keeps the next draft editable but prevents another submission until the prior feedback reaches a terminal state. Plan guidance appears only for Analyzing or Executing plans outside approval; approval plans continue to expose only the approval contract and approval input inside the owning step.

## Manager presentation order and plan views

Manager uses the current step's `approvalRequest` as the approval-entry contract. A complete actionable contract with `responseStatus=pending` requires stored `plan.status=待审批` and produces `presentation.approval.state=ready`, `enabled=true`, and the internal `blocked` tone. An incomplete contract produces `incomplete` and requires the plan to remain `分析中`. Legacy `isBlocked` is only a compatibility projection, and `blockedBy` is explanatory text; neither changes plan status.

Manager displays `plan.status` directly: Analyzing, Awaiting approval, Executing, Awaiting package, Awaiting QA, Awaiting discussion, Paused, Completed, or Closed. Presentation metadata supplies only tone, palette, and integer sort order.

`nextAction`, `currentStep`, step titles, and `waitingFor` explain work and waiting reasons only. They never override `plan.status`.

For content-changing work, the stored lifecycle is `执行中 → 等待打包 → 等待 QA → 完成`; QA failure returns it to `分析中` or `执行中`. The Agent writes `等待打包` only after applicable sync, commit, and conflict-free readback, and writes `等待 QA` only after target-package inclusion is proven. Manager validates that evidence but does not infer a status from step text. A QA delivery receipt is evidence inside `等待 QA`, not another main status.

`presentation.status` must equal `plan.status`. Status order is Analyzing, Awaiting approval, Executing, Awaiting package, Awaiting QA, Awaiting discussion, Paused, Completed, and Closed. Archive view membership comes from `archiveStatus`, not from the Closed status.
