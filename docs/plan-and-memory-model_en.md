...
Manager exposes eight non-terminal presentation states: cyan `Analyzing`, green `Executing`, amber `Awaiting discussion`, blue `Awaiting package`, purple `Awaiting QA`, gray `Paused`, red `Awaiting approval`, and orange `Awaiting manual verification`. External information, assets, owners, accounts, devices, authorization, and receipts remain internal wait details. The top-level lifecycle field is unchanged.

When the current step still has a safe action, Manager uses `workPhase` to show cyan `Analyzing` or green `Executing`. A complete pending approval contract is red `Awaiting approval`; a paused current resume step with `discussionState=pending` is amber `Awaiting discussion`; delivery closure followed only by missing package identity or inclusion proof is blue `Awaiting package`; proven inclusion is purple `Awaiting QA`; and a development-closed `manual-verify-*` step is orange `Awaiting manual verification`. No remaining safe action is gray `Paused`. These special states override `workPhase`.
...
`presentation` exposes `status`, `tone`, `statusLevel`, `sortBucket`, the shared palette, and approval details. `counts.stages` summarizes only the public stages: `Analyzing` uses `analyzing`, `Executing` uses `executing`, `Awaiting discussion` uses `discussion`, and orange `Awaiting manual verification` uses `manualVerification`. Non-terminal presentation sorts `Awaiting approval → Awaiting discussion → Awaiting QA → Awaiting manual verification → Analyzing/Executing → Awaiting package → Paused`; analysis and execution share one sort level and are then ordered by update time, with Paused last.
