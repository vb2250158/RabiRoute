...
Manager uses the current step's `approvalRequest` as the only blocked-state source of truth. A complete actionable contract with `responseStatus=pending` simultaneously produces `presentation.approval.state=ready`, `enabled=true`, and `Blocked`, so every blocked item has an approval entry. An incomplete contract produces `incomplete` while the plan remains in progress for Agent investigation and repair. Legacy `isBlocked` is only a compatibility projection, and `blockedBy` is explanatory text; neither can independently block a plan. QA, missing materials, execution failures, and external artifacts stay actionable. `Awaiting QA` is derived only from an in-progress structured `qa-* / verify-*` step selected by `steps/currentStepId`; `Awaiting package` still requires a structured package/build step, completed prior work, and an explicit package dependency. `presentation` continues to expose `status`, `tone`, `sortBucket`, and the shared palette.
