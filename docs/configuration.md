...
- `codexThreadId` / `codexThreadName`：下拉显示 Desktop 任务的名称和最后时间，内部保存完整任务 ID 与可见名称。有效且同工作目录的 ID 是稳定身份；Desktop 改名、SQLite 标题滞后或 goal 完成都不会触发新建。用户明确输入新名称时前端会清空旧 ID，后端才按名称 + 目录查找，零匹配时幂等创建，多个同名时要求按最后时间选择。
