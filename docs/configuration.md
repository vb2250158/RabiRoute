...
- `rabiLinkRelayEnabled` / `rabiLinkRelayUrl` / `rabiLinkRelayToken`：RabiLink 电脑端直连 Relay 开关、地址和 token。开启后，RabiRoute 会直接从 Relay 领取 Rokid/灵珠任务，把任务交给当前路由处理，并把回复写回 Relay 下行队列。
...
如果要让 Rokid/灵珠在公网访问 RabiRoute，不应暴露本机 manager，而是部署公网 Relay，并在 `rabilink` 消息端里配置 Relay 地址和 token。当前主链路不经过手机桥：Relay 由电脑端 RabiLink worker 直接领取任务、回写增量回复。
