...
- `codexThreadId` / `codexThreadName`：下拉显示 Desktop 任务的名称和最后时间，内部保存完整任务 ID 与可见名称。有效且同工作目录的未归档 ID 是稳定身份；保存 ID 指向已归档任务时明确阻止并要求恢复/重选，不自动创建替代任务。用户明确输入新名称时前端会清空旧 ID，后端才按名称 + 目录查找；一个或多个同名同目录候选按最后更新时间绑定唯一最新者，零匹配时幂等创建，最大时间并列时要求选择。
