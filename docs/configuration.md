...
- `data/route/<配置名>/routeConfig.json`：消息端、端口、Agent 端、工作目录、指向人格。
- `data/roles/<角色名>/persona.md`：人格正文。
- `data/roles/<角色名>/roleMessageConfig.json`：消息模板规则。一个人格可以服务多个路由配置。

如果运行期 data 不存在，manager 会优先复制整包 `examples/data`，让默认路由和 Rabi 示例人格一起落地。`examples/data` 不是运行依赖；缺少 examples 时，manager 也能创建最小 QQ / NapCat 到 Codex Desktop 配置。
...
- 消息模板规则不写在 `routeConfig.json` 里。manager 会按 `agentRoleId` 读取对应角色的 `roleMessageConfig.json`。
...
每个 `data/route/<配置名>/routeConfig.json` 是一条可启动路由。它可以有自己的消息端、端口和 Agent 工作目录：
...
```text
data/route/main/routeConfig.json          -> agentRoleId: Rabi
data/route/fennenote-voice/routeConfig.json -> agentRoleId: Rabi
data/roles/Rabi/roleMessageConfig.json
```

人格里的 `roleMessageConfig.json` 用 `configName` 区分不同路由配置的消息模板规则。没有有效 `roleMessageConfig.json` 规则时，消息只会记录，不会投递给处理端。
