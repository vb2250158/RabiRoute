...
- `messageAdapterPolicies`：每个消息端的权限。`inputEnabled` 控制是否接收，`outputEnabled` 控制是否允许出站，`outputMode` 可为 `replyOnly`、`direct` 或 `draft`。主动跨目标发送必须配置为 `direct`，并填写 `allowedGroups` / `allowedUsers`，或显式打开 `allowBroadcast`。
...
旧配置仍然兼容：`messageInputsDisabled=true` 会临时关闭整个路由的消息进入；`messageAdaptersDisabled` 会被视为对应 adapter 的 `inputEnabled=false`。新配置建议优先使用 `messageAdapterPolicies`，因为它能分别表达“接收”“发送”“发送模式”和“禁用/允许管道”。
