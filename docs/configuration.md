...
- `rabiLinkWebhookPort` / `rabiLinkWebhookPath`：RabiLink 手机桥监听端口和路径，默认路径 `/rabilink`。用于手机 App、Rokid/灵珠文本桥把消息投递给 Codex。RabiLink 默认监听 `0.0.0.0`，方便同一局域网手机用电脑局域网 IP 测试；可用环境变量 `RABILINK_WEBHOOK_HOST=127.0.0.1` 改回仅本机监听。
...
- `rabilink`：RabiLink 手机桥，接收手机 App、Rokid/灵珠桥接出来的文本事件，写入 `rabilink-voice-transcripts.jsonl`，再按 `voice_transcript` 路由给 Codex。
- `webhook`：接收暂时没有专用消息端的外部系统 POST 事件。FenneNote、小爱、企业微信、RabiLink 这类已命名平台应使用各自专用消息端，避免日志、模板变量和回传语义混在通用 webhook 里。

如果要让不在同一局域网的手机 App 接入 RabiRoute，RabiLink 第一版可以用 `rabilink` 专用消息端验证入站消息；正式双向接入仍应使用公网 HTTPS、WebSocket 和设备消息队列，见 [手机 App 远程接入](mobile-app-webhook-integration.md)。
