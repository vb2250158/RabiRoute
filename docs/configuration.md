...
- `codexThreadId` / `codexThreadName`：下拉显示全部 Desktop 任务的名称和最后时间，内部把 Desktop 返回的完整任务 ID 与可见名称成对保存。旧配置若误把名称写进 ID 字段，会在读取边界自动迁回名称。保存时由后端统一执行“读取 ID 并校验保存名称 → 名称 + ID 都匹配才精确绑定 → 不一致时按保存名称 + 目录查找 → 零匹配时幂等创建”；用户不需要填写或修正 UUID。
