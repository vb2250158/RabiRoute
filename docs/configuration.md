...
| agent / runtime | Desktop 管理的 Codex 负责线程、turn、工具调用和执行 | 是，稳定 id 为 `codex` |
...
Codex 已并入新的 ChatGPT desktop，但 Codex 仍是 Agent 和 runtime 的名称。不要把 adapter 改名为 `chatgpt`。RabiRoute 的正式投递协议是 Desktop IPC；实验性 app-server WebSocket 不进入主链。
...
- `codexThreadId` / `codexThreadName`：下拉显示全部 Desktop 任务的名称和最后时间，内部用 Desktop 返回的完整任务 ID 绑定。旧配置若误把名称写进 ID 字段，会在读取边界自动迁回名称。保存时由后端统一执行“有效 ID 精确绑定 → 名称 + 目录查找 → 零匹配时创建”；用户不需要填写或修正 UUID。
- `codexCwd`：目标 Desktop 任务的项目目录。它用于同名任务消歧、名称自动查找和新建位置；选择已有任务时自动采用任务自己的目录。投递前仍会校验规范化目录，防止进入其他项目的同名任务。
...
- 无有效 ID 时会按名称和规范化工作目录自动查找：唯一匹配自动绑定，零匹配自动创建，多个同名要求用户按最后时间选择。已有精确 ID 的目录冲突、Desktop 未就绪或 owner 无法加载时仍 fail closed，不启动备用 Runtime。
