...
Manager can submit work-ended events explicitly posted to `/api/work-events/ended` to that same FIFO. The Speech Service **Task completion announcements** card controls source toggles, persona, template, preview, and recent receipts. Manager cleans the summary, resolves the target persona's `voice-profile.json`, and calls local TTS. Work events are physically rotated into date shards, while the announcement ledger stores only decisions, summary hashes, and playback receipts rather than copying the original summary. This delivery includes the producer contract and consumer path; real Codex / DSH completion events still require integration, so enabling the setting alone does not prove those handlers announce completions.
...
The top of RibiWebGUI's **Speech Service** page provides a page-wide RabiSpeech switch. Turning it on lets the authenticated Manager run this workspace's `scripts/start.ps1`, which prepares Python and private dependencies before launching the formal `runtime/RabiSpeech.exe`. Model, audio-stream, ASR, TTS, voiceprint, and playback controls appear only after the lightweight `/health` endpoint succeeds. Model discovery and warm-up continue after readiness and no longer make an online service look like a startup failure. If the launch log reports a bind conflict, Manager stops the old process and retries once only when the port owner belongs to this workspace and health is still offline; unrelated owners remain a fail-closed error. Turning the switch off first verifies that the listening port belongs to this workspace's `runtime/RabiSpeech.exe` or `windows_host.py`, then stops the process and hides all runtime controls. Unknown port ownership fails closed and is never killed by port alone. This switch currently supports Windows hosts only, and Manager read-only mode continues to reject lifecycle commands. A brief stale SSE disconnect during a runtime transition remains an internal auto-reconnect state instead of an operation error or layout shift, and refreshing runtime configuration does not submit a meaningless settings save.

When startup fails, Manager returns a short result, the concrete cause from the latest readiness check or startup-log tail, and a matching recovery action. The Speech Service page renders separate **Cause** and **How to fix** lines. Missing dependencies, the actual conflicting port, and configuration errors receive different actions. Launch subprocesses use UTF-8 output. Only a bounded log tail is returned, with workspace and user paths replaced by safe placeholders.

The selected-text menu is enabled on the WebGUI **Settings** page. See [Interface and status](user-guide/interface-and-status_en.md) for the operator steps. **Selected-text reading** is a sub-feature of that menu; a click on **Read aloud** enqueues host TTS.

Cache-expiry cleanup moves to the background after `/health` becomes available, so it no longer blocks service readiness. Shutdown still waits for an active cleanup task, keeping that work inside the RabiSpeech lifecycle.

Windows cold start loads the Python environment and scans persona directories, so Manager now waits up to 120 seconds. A live launch process is no longer reported as failed at the former 60-second boundary.
...
The FenneNote-style host pipeline now lives only under **Speech Service → ASR**, showing `host microphone → VAD segmentation → ASR transcription → broadcast delivery → reply/playback`, counters, recent events, and transcript previews. A Route's **Message adapters → Speech endpoint** section keeps only that Route's hot/persona-keyword policy, persona TTS summary, and automatic reply-playback switch. It no longer duplicates host waveform, counters, runtime logs, or recent transcripts. **Enable ASR streaming** independently controls continuous recording and transcription for the whole host, while each speech-endpoint toggle only controls whether that Route receives completed transcripts. With no subscribed Routes, the host still transcribes and stores its record. Disabling the host switch stops continuous capture immediately. Manual uploaded-audio transcription is unaffected.
