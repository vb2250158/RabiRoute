...
Out-of-tree plugins select one Profile with `RABIROUTE_PLUGIN_PROFILE` and add package roots with `RABIROUTE_PLUGIN_PACKAGE_ROOTS`. The Profile `instances` array contains stable `id`, package, version, enablement, JSON configuration, and permission `grants`. There is no Patch format.
