...
The built-in catalog registers 31 Manager plugins. Entries run as `in_process`, `isolated`, or `declarative`; Manager's loader never imports isolated entry top-level code, and every long-lived child holds a process lease. `manager:core` retains recovery, catalog, and basic-page contributions, while optional business entry points are registered by their plugins. The central HTTP chain is limited to LAN authentication, the read-only write gate, plugin route dispatch, Manager SSE, plugin catalog/reconciliation, static assets, the Host-only shutdown route, JSON 404 for control paths, and WebGUI HTML fallback for all other paths. Production business routes declare stable `routeId` values with real `exact` or `prefix` matchers; `dynamic` remains only as an extension contract. The Registry rejects duplicate route IDs and overlapping path conflicts.
...
| `manager:rabilink-relay` | RabiLink Relay runtime and generic peer LAN listener (`PeerLanServer`), retaining remote access, RPC, and tunnels | Stops Relay, the peer listener, and active callbacks without deleting persona or historical data |
...
Persona data synchronization is retired, with no setting to re-enable it. RabiLink retains remote persona access, RPC, and tunnels. Do not disable the whole Relay connection or delete existing personas, plans, memories, or historical sync data as part of retirement. Generic peer transport uses `PeerLanServer`, `peer_lan_status`, and `RABILINK_PEER_LAN_PORT`; device changes use `peer_changed`. These are not aliases for the retired sync service. Source wiring is being aligned; installed behavior needs separate acceptance. See [retirement](persona-data-sync_en.md) and [cross-PC calls](rabilink-peer-rpc_en.md).
