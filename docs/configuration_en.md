...
- `dshSessionId` / `dshSessionName` / `dshCwd` / `dshBaseUrl`: DSH (DeepSeek Harness) delivery binding. Create the DSH session in the DSH WebGUI first, then enter its complete `session-<uuid>` ID. RabiRoute delivers through `POST /api/session.prompt` in queue mode; an empty display name defaults to `DSH CottonGame Luna Max`. It does not create, rename, or archive DSH sessions. This adapter is currently delivery-only: Message Agent mode, plan assistants, memory consolidation, and Hooks remain Codex-owned.
...
- `messageProcessingAgents.codex`: Codex Message Agent eligibility, independent model, and task-count limit. It applies only while Codex is the selected Primary Agent. When the primary changes to DSH or another Agent, ordinary messages go directly to that primary; RabiRoute does not create, reuse, or continue Codex Message Agent tasks. Codex may still receive explicit Agent-to-Agent deliveries as a non-primary Agent. It defaults off. When enabled, chat messages automatically form groups and reuse or dynamically create Message Agent tasks for distinct topics. ASR and structured events continue direct delivery. The Agent list, limit, and actual delivery share one weighted order: quoted Agent sends, the original message group, endpoint, conversation, sender, and recent use are evaluated before the stable index tie-breaker. Optional `maxAgents` accepts `1–32`; leaving it unset preserves dynamic expansion, while reaching the limit reuses a task inside the ranked range instead of creating another one. A value of `1` keeps and reuses the fixed task `<persona name> 协助处理消息1`, including while it is active. Lowering the limit detaches excess tasks from the current Route's Message Agent pool but does not delete Desktop tasks. Completed records retain their original worker for audit. Unfinished plan-progress notifications, plan/memory callbacks, Agent-to-Agent reply requests, and reminders move to the currently ranked worker, so detached or archived tasks are not awakened again. `heartbeat` remains one continuing inspection responsibility and always returns to its selected worker. Without a configured limit, a single worker uses `<persona name> 协助处理消息`; multiple workers use numbered titles. Renaming preserves the original Desktop task ID and workspace. Ordinary dispatch treats only a task reported as `idle` as an idle candidate; `notLoaded` tasks are reused through the normal Desktop-owner path. If Desktop is offline or current status cannot be read, the group remains recoverable and no replacement task is created. `agents.json` stores identity, workspace, and initialization metadata only; routing familiarity lives in `routing-affinity.json`, and neither file stores active/idle state. Defaults remain `gpt-5.6-luna` / `medium`, affecting Message Agent turns only.
