...
| 任务完成 TTS Hook | 自动化执行与去重合同已验证；设备播放按环境验收 | 在人格自动化 Hook 配置 TTS 播报，与群消息共用事件投递规则和 Outbox 回执；使用所选路线的人格声线与自动播放设置，默认新建规则关闭。 |
...
- 完整 Route 诊断只从 JSONL 文件尾部读取有限记录，并在同一次响应中复用相同文件的读取结果。Manager 同时限制请求头接收为 10 秒、请求接收为 30 秒、Keep-Alive 空闲为 5 秒，并限制单连接最多 100 次请求；这些值可从 `GET /meta.httpLimits` 查看。
...
- 人格数据同步已退役，旧按钮、Changed Files 预览、手动同步和冲突工作台不再是当前入口；不是保留实现后关闭开关。通过 RabiLink 在目标电脑访问人格与数据，不创建同步副本；见[退役说明](persona-data-sync.md)。
...
- RabiLink 页集中提供主页、远端智能体和配置；实例名/GUID、Relay 连接、语音中转和 Agent 上传上限在“配置”中管理。目录和局域网访问保留在“设置”，Route 启停仍在控制台。主页方案为紧凑中文原生只读视图：通过 Manager 读取当前应用内电脑名、标识、在线状态和服务，不内嵌管理页面、不要求重复登录、不把 token 返回主页浏览器或写入 URL、不提升管理权限。连接或读取失败不能显示成零设备或首次初始化；管理员另在新窗口 `/manage` 独立登录。该方案的部署与真实浏览器交互尚待验收。
...
- **人格数据同步：已退役。** 自动与手动同步、合并和同步 API 均不再提供；所有现有数据与历史证据保留。RabiLink 远端人格访问、RPC、tunnel 和独立只读 persona manifest 不属于同步功能，继续保留。本轮源码调整不代表部署或实机验收完成。

<details>
<summary>退役前的实验能力记录（历史事实，不是当前操作指南）</summary>

以下接口、脚本与自动对账描述只适用于旧版本，不应继续调用或作为当前验收项。

- 退役前，多电脑人格同步为实验支持：同一 RabiLink 应用 token 下的 PC 可查询 peers，优先经专用局域网数据面直连，失败后经 Relay 受限中转；JSONL 做集合合并，普通文件按共同基线快进，已知基线上的单边删除可传播，删除/编辑并发或双方修改的冲突保存在 `data/persona-sync/conflicts/`。同一人格、路径、peer、远端哈希、删除状态和基线哈希使用固定证据哈希直接定位，不再为自动去重同步遍历旧冲突目录；历史副本只在用户明确检查冲突时异步归组。可重建 manifest 索引只做一次启动校准，之后由文件事件重算变化路径；事件不可用时才在查询前做一次校准。`PersonaSyncAutoReconciler` 把本机文件变化、peer 上下线和 Relay `ready` 作为唤醒信号，持久保存待对账范围并执行一次 manifest 补漏；目标离线时等待事件，在线临时失败只做有界退避，不运行固定业务轮询。本机 Agent 或人格页可查看远端证据并选择保留本地、采用远端/删除或提交合并内容；处理过程校验本地哈希并保留解决审计，随后仅在两端仍匹配证据时把结果即时发布回来源 peer。冲突控制、索引和自动状态诊断不经 LAN/Relay 暴露。语音消息端账号的兼容归类事件带 `supersedes` 分支关系，多 PC 并发判断会保留冲突头并由人格后续 PUT 显式收敛；同步响应立即返回 `semanticConflicts`。`scripts/test-rabi-persona-sync.mjs` 仍可在两台实体 PC 上执行一次显式同步并留下脱敏 JSON 证据。
