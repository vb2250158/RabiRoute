...
| RabiSpeech 语音消息端 | 实验支持 | RabiSpeech 只维护一份 ASR/VAD 和 FIFO。音频流默认使用本机声卡，也可选择局域网 Rabi 语音客户端作为远程麦克风/喇叭；客户端不切句、不跑模型。每段转写仍广播给所有订阅 Route，各 Route 独立执行热投递或人格关键词策略。远程客户端断线不自动回退本机。 |
