...
- WebGUI 当前有：控制台、消息适配器、Rabi 人格、计划与记忆、语音服务、模型管理、日志诊断和使用手册。模型管理是主机级页面，不随当前 Route 切换；快速配置向导可以选择消息入口、处理端和人格。
