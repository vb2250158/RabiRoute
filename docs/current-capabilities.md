...
| 界面主题 | 代码与自动化测试已验证；Windows 实机验收待完成 | 主题保存在主机级 `data/desktop/settings.json`。WebGUI 与托盘读取同一 `system`、`light` 或 `dark` 设置；托盘菜单、角色面板、滑词操作条和截图窗口随之更新。 |
...
处理端创建已接入首个 Cordis 运行时切片：五个内置 Agent Adapter 由独立 Fiber 注册到同一清单，兼容入口和原投递路径保持不变；单个 Fiber 与根 Context 的撤销已通过自动化测试。Manager 扫描、配置枚举、Gateway Host 和表现端贡献目录仍在迁移。
...
- “计划与记忆”页按“当前计划 / 近期记忆 / 沉淀记忆 / 已归档”四个标签分开显示内容；当前计划展示未归档计划，近期记忆只包含尚未沉淀的记录，沉淀记忆展示稳定可召回输出，已归档汇总归档计划和带 `consolidatedAt` 的来源记忆。页面先按 Manager 排序读取首批 8 条计划摘要或 24 条记忆；计划首屏会在挂载卡片前并行补齐这 8 张卡片的完整详情，不显示首屏详情加载提示。随后在页面可见期间自动用最多 250 条的分页补齐当前计划分类，并用最多 100 条的分页补齐当前记忆分类，不需要滚动触发；页面显示已加载数量与总数。计划后续页不重复返回首页已取得的状态和标签统计，Manager 接口使用紧凑 JSON。标签页隐藏时暂停，重新可见后刷新并继续。每页之间只让出一个渲染帧，不等待正文水合。右侧正文从当前目录目标开始挂载有界的向后窗口，计划卡先挂载 8 张、记忆卡先挂载 24 张；向下滚动分批挂载后续计划，向上滚动分批补回更早计划，并在补回后保持当前卡片的视口位置。目录跳转把目标详情放入最高优先级，在当前 Manager 与局域网环境按 1 秒交互预算验收；其余正文、步骤、审批和附件元数据只对真正接近视口的卡片补充，每轮提升最近 10 张且最多 10 个并发请求，完整内容搜索时才按需补齐全部详情。图片/视频附件继续使用浅色加载与失败占位及低优先级懒加载，避免大知识库阻塞首屏。
