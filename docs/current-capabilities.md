...
| FenneNote | 实验支持 | 命名的 Webhook-like 语音转写入口；可按 record-first 规则只写 RabiLink 账本，也可直接路由；Outbox 可转发到 FenneNote reply/playback 端点。 |
...
- 当前 route kind：`private`、`group_message`、`direct_at`、`direct_reply`、`indirect_reply`、`heartbeat`、`manual_trigger`、`role_panel_message`、`voice_transcript`、`rabilink`、`wecom_message`。
...
| FenneNote | 根据 pipeline/request 转发 reply 或 playback 请求，保留语音参数。 |
...
- WebGUI 当前有：控制台、消息适配器、Rabi 人格、日志诊断、项目文档五类页面；快速配置向导可以选择消息入口、处理端和人格。
...
- 日志页展示连接状态、Codex 投递通道、最近日志，并能执行手动触发和 delivery replay。
...
- 英文界面只翻译登记过的界面文案和动态状态；route/persona ID、规则名、模板、正则、任务名、路径、token、日志和运行数据保留原文。项目文档英文页按需读取仓库 `docs/**/*_en.md`，不维护第三份英文事实源。
...
- AgentPacket 构建会执行角色知识快照；命中记忆会刷新 `viewedAt`。只有显式 `memory-consolidation` 手动触发或 Manager API request 才会创建整理 run，提交结果后才标记输入并写入沉淀记忆；当前没有仅凭时间流逝自动启动的后台整理调度器。
- 运行记录以 JSONL 为主，包括消息、适配器日志、AgentPacket、Outbox、heartbeat、manual trigger、role panel、RabiLink conversation 和 delivery replay。
