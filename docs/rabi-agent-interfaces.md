...
RabiRoute 通过后台 app-server 启动 Agent 回合时，当前回合可能没有注入 `codex_app__list_threads`、`codex_app__read_thread`、`codex_app__create_thread`、`codex_app__send_message_to_thread` 等 Codex Desktop 连接器工具。提示词不能补出未注册工具。
...
- `list`：按标题查询本机 Codex 线程。
- `read`：通过 `threadId` 读取线程。
- `create`：在已配置的 Codex 工作区创建正式线程并投递初始提示词。
- `send`：向已有线程启动后续 turn。
...
- `create` 和 `send` 默认使用 `workspace-write`，保证 Shell、工程搜索、AGENTS.md、Skill、代码、配置和资源读取能够正常工作。运行权限不等于业务修改授权；没有明确实施授权时，提示词仍应要求只读调查。
- Windows sandbox 若出现 `CreateProcessAsUserW failed: 1312` 等环境级启动失败，可以对同一任务显式传入 `"sandbox":"danger-full-access"` 重试；不得因此重复创建任务。即使使用该权限，审批前仍禁止修改工程或外部系统。
...
- 连接器工具可用时仍可直接使用连接器；线程桥是后台 Agent 回合的稳定 fallback，不是 multi-agent 子 Agent。
...
- bridge 自带精确锁定的 Codex runtime，通过 app-server stdio 投递；远端 task 只能在 `REMOTE_AGENT_ALLOWED_CWDS` 内使用 `workspaceWrite`，默认禁止网络，不存在 `dangerFullAccess` 路径。
