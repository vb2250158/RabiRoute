...
同一 Relay 还提供受限的 RabiSpeech API。`health/models/audio/speech/audio/transcriptions` 是不进入 Agent 的同步模型调用；`audio-streams/rabilink/*` 是 Android 正式语音主链，流式 PCM 在目标 PC 完成识别后由 RabiSpeech 自动进入通用消息存储与 Route 投递。`POST /api/rabilink/speech/messages` 只保留已完成 ASR 消息的兼容/调试入口。详见 [RabiSpeech 本机 TTS / ASR 插件](rabispeech-plugin.md)。

链路：

```text
语音上行：Rokid 眼镜 / 手机麦克风 -> 手机后端
  -> Relay 受限有序 PCM 流 -> 电脑端 RabiLink worker
  -> RabiSpeech VAD / 切句 / ASR / 声纹
  -> 主机通用语音库 -> rabilink Route -> 人格会话上下文

普通上行：灵珠兼容入口 / 文字 / control / 媒体
  -> RabiLink Relay 公网 HTTPS 输入队列
  -> 电脑端 RabiLink worker -> 统一会话账本
...
```json
{
  ...
  "nextCursor": "oc1.<opaque-relay-generation>.10"
}
```

支持 SSE 的 Android、PC worker 或其它客户端从空 cursor 建立事件流。Relay 只推送“有下行变化”的轻量事件，不在空闲时查询业务队列：

```http
GET /api/rabilink/events?deviceId=<stable-device-id>&deviceKind=glasses&after=
Authorization: Bearer <token>
Accept: text/event-stream
```

收到 `ready` 或 `outbox_available` 后，用保存的 cursor 读取一次增量：

```http
GET /api/rabilink/devices/messages?deviceId=<stable-device-id>&deviceKind=glasses&after=<opaqueCursor>&waitMs=0&stream=1
Authorization: Bearer <token>
```

Rokid AIUI QuickJS 没有 SSE、WebSocket 或分块回调，因此正式 AIUI 页面使用 `GET /rokid/rabilink/messages?stream=1&waitMs=25000`。Relay 在请求内注册一次 `outbox_available` 事件等待器并立即二次检查 cursor，事件到达或 deadline 到期后才返回；等待期间不扫描业务队列，页面隐藏或离开连接会话时停止续接。

普通 Agent 回复和主动消息都按 `seq` 进入同一持久 outbox。事件流或 AIUI 的事件背书长等待都不因某个内部 worker task 完成而关闭；断线重连时按 cursor 查询一次补齐遗漏。Relay 的广播和仅按设备类型投递默认保留 48 小时，独立于十分钟 task 生命周期；带明确 `targetDeviceIds` 的消息在所有明确目标设备回 `delivered` 前不按 Outbox TTL 删除。Android 收到消息后先完成本地持久状态再推进 cursor；可靠回执未上传时由本地队列补传。AIUI 仍按其前台生命周期消费，不具备 Android 的物理播放回执闭环。
...
PC worker 只接受 `/api/rabilink/devices/media/...` 下载路径，使用同一应用 token 下载到 Route 私有数据目录后再写账本。单文件默认上限 64 MiB（`RABILINK_RELAY_DEVICE_MEDIA_MAX_BYTES`），临时对象默认保留 7 天（`RABILINK_RELAY_DEVICE_MEDIA_TTL_MS`）。Relay 启动时清理一次并计算最早到期对象；后续上传只在出现更早到期时间时重排一次性清理 deadline，不做固定周期目录扫描。这是文件消息慢传，不是实时视频或断点续传协议。
...
{
  "messageId": "out-000000010",
  "deliveryId": "stable-delivery-id",
  "deviceId": "phone-user-1",
  "deviceKind": "phone",
  "state": "delivered | played | playback_failed",
  "failure": "仅失败时可选"
}
```
...
- 每个终端独立保存不透明 cursor，不要解析或自行拼接。即使新消息只发给其他设备，本终端返回的 `nextCursor` 也会越过它，避免重复扫描。cursor 同时带 Relay 共享代际，正常重启和同数据目录的多进程不会改变它；运行期状态回滚或游标损坏时，响应返回 `cursorReset=true` 和原因，并从当前仍保留的消息重新回放。终端必须按稳定 `deliveryId`（缺失时才退回消息 ID）去重，再保存新的 `nextCursor`，这样状态回滚不会让旧游标永久领先于服务端。
...
4. 每批处理后原样保存不透明 `nextCursor`；断线重连从保存的 cursor 继续。若返回 `cursorReset=true`，按本地幂等记录接受保留消息重放，不得继续沿用旧 cursor。
...
```http
GET /rokid/rabilink/messages?after=<opaqueCursor>
Authorization: Bearer <token>
```

第一次查询使用 `submitRabiLinkTask` 返回的 `cursor` / `nextCursor` 作为 `after`。`waitMs=0` 返回即时快照；`waitMs>0` 使用 Relay 内部事件等待器，不做周期查询。支持 SSE 的客户端应订阅 `/api/rabilink/events` 后传 `0`；没有 SSE 能力的 AIUI 使用有界前台等待。内部输入任务通常在 RabiRoute 接收后很快结束，但全局下行流继续存在，不用某个 task 的结束态关闭常驻消息端。
...
```json
{
  ...
  "done": false,
  "shouldContinue": true,
  "nextCursor": "oc1.<opaque-relay-generation>.2",
  "cursorReset": false,
  "cursorResetReason": "",
  "messages": [
    {
...
5. 新消息通知来自 `/api/rabilink/events`；没有事件时不调用消息列表。
6. 事件到达后处理全部 messages 并保存 nextCursor；空快照只表示该事件已被其它消费者处理或当前没有可见消息。
7. 只有当 Rabi/Codex 侧已经返回结束、电脑端 worker 完成 finish，并且 messages 为空且 shouldContinue=false 时，才结束单 task 的调试视角；常驻应用事件流继续存在。
...
第一次查询不带 `after`。接口返回当前即时快照，不等待新消息。这个接口主要用于排查单个 task；正式播报订阅 `/api/rabilink/events`，不得把它作为主循环。

可选参数：

- `after`：已经读过的最后一条消息 ID。
- `waitMs`：仅为旧客户端请求兼容保留，服务器不再等待；新客户端固定传 `0` 或省略。
...
2. 保存 nextCursor。
3. 如果 shouldContinue=true，等待 `/api/rabilink/events` 的相应事件后再调用本接口，并把 nextCursor 作为 after；禁止空轮询。
...
服务器上的远程 WebGUI 入口会创建 WebGUI 请求，并通过 `/api/rabilink/events` 推送 `webgui_available`；电脑端 worker 收到事件后即时领取：
...
`bodyBase64` 用于保留 HTML、JS、图片、音视频和 JSON 等有限响应体。服务器会对 HTML/JS/CSS 中常见的 `/api`、`/manager-config` 和 `/assets` 路径做前缀改写，让远程 WebGUI 的保存配置、读取状态和静态资源请求仍然回到同一台 PC Rabi。请求头只透传 `Accept`、`Content-Type`、`User-Agent`、`Range` 和 `If-Range`；响应会移除 hop-by-hop 头和 Cookie。Manager `/api/events` 与 `/api/speech/events` 由 worker 分别解析后按事件逐条 POST 到 `/worker/webgui-events`，并携带流路径；Relay 只向当前账号所选 PC、同一路径的已登录远程页面发布，不用业务轮询。有限代理如果意外收到 `text/event-stream` 会立即取消并拒绝，避免永不结束的响应占住本机连接。
...
| `RABILINK_RELAY_MESSAGE_WAIT_MS` | `60000` | 单 task 调试兼容字段；正式常驻链不依赖任务查询 |
| `RABILINK_RELAY_OUTBOX_WAIT_MS` | `60000` | 无 SSE 客户端的事件背书请求 deadline；支持 SSE 的客户端传 `waitMs=0` |
...
Relay 运行期 task 和全局下行 outbox 会写入 `<dataDir>/runtime-state.json`。这个文件用于多 relay 进程或反代分流时共享任务队列，避免 worker 完成输入时命中另一个进程后出现 `Task not found`，也保证 `getRabiLinkMessages` 能从共享 outbox 取到其他 relay 进程写入的回复。`<dataDir>/outbox-cursor-state.json` 只保存随机共享代际和已发布序号高水位，用于识别 `runtime-state.json` 倒退；它不含消息正文、token 或设备资料。outbox 同时保存 `deliveryId` 和逐设备回执，让跨请求重试保持幂等；TTL 只清理广播、仅按类型投递或所有明确目标已 `delivered` 的旧消息。这两个文件都属于运行期数据，不应提交。
