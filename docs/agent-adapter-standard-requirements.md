...
1. **存在会话时，直接投递到 Rabi 配置所绑定的会话。** 用内部完整 Session ID 读取 owner 记录；ID 存在、workspace 一致且任务未归档就是有效绑定，随后直接续投同一任务，不新建。owner 的标题/SQLite `title` 是可变展示元数据，不能参与身份判定。保存 ID 指向已归档任务时，先复用同 workspace 唯一最新的未归档同名任务；没有候选才明确阻止并要求恢复或重选，绝不创建替代任务。
...
3. ID 已归档：按 savedName + normalizedWorkspace 查询未归档任务；有唯一最新候选就重绑，零匹配停止且不得创建
...
7. 零匹配：仅对空/非法/失效 ID 保持 pending-new；归档 ID 必须停止
```
