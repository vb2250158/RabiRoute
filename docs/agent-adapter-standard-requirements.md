...
1. **存在会话时，直接投递到 Rabi 配置所绑定的会话。** 用内部完整 Session ID 读取 owner 记录；ID 存在、workspace 一致且任务未归档就是有效绑定，随后直接续投同一任务，不新建。owner 的标题/SQLite `title` 是可变展示元数据，不能参与身份判定。保存 ID 指向已归档任务时，不再查找或复用其它同名任务；真实投递或保存提交点幂等创建一个新任务，持久化新 ID 后再投递。
