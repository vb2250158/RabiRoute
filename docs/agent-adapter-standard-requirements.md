...
1. **存在会话时，直接投递到 Rabi 配置所绑定的会话。** 先用内部完整 Session ID 读取 owner 记录；只有保存的“可见名称 + Session ID”同时匹配、workspace 一致且任务未归档才是有效绑定，随后直接续投同一任务，不新建。保存 ID 指向已归档任务时必须明确阻止投递并要求恢复或重选，不能把它当成不存在后自动新建。
