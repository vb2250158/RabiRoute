...
无论 Agent 端还有多少高级能力，首先必须满足以下四条：

1. **存在会话时，直接投递到 Rabi 配置所绑定的会话。** 先用内部完整 Session ID 精确读取并校验 workspace；绑定有效就续投同一任务，不新建、不按名称换到另一条会话。
2. **不存在会话时，只创建一次会话，再向新会话投递。** ID 非法/失效时先按“名称 + 规范化 workspace”查找；唯一匹配直接绑定，零匹配才幂等创建，持久化新 ID 后再投递；多个同名先让用户选择。
...
4. **Rabi 设置页保存时必须完成目标切换。** 选择已有会话后保存，应校验并持久化该会话的完整 ID；输入不存在的名称后保存，应幂等创建一次并持久化新 ID。保存成功后，下一条消息不得再次按名称猜测或创建。
...
```mermaid
flowchart TD
    M["RabiRoute 准备投递真实消息"] --> I{"配置的 Session ID 是否有效？"}
    I -->|"有效且 workspace 一致"| B["绑定原会话"]
    I -->|"无效或不存在"| Q["按名称 + 规范化 workspace 查找"]
    Q -->|"唯一匹配"| P["持久化匹配到的 Session ID"]
    Q -->|"零匹配"| C["按幂等键创建一次会话"]
    Q -->|"多个同名"| A["停止并要求用户选择"]
...
这里的“自动创建”只属于一次真实投递或用户保存配置的提交事务，不属于扫描、刷新、输入、失焦和重启。Rabi 设置页点击保存时必须复用上图同一个 resolver，先解析或创建，再把完整 ID、标题和 workspace 一起保存。创建与投递应被视为一个可恢复流程：创建成功但投递失败时保留并复用已经创建的 ID，后续只重试投递，不能再次创建同名会话。
...
扫描、刷新和健康检查必须只读，不能创建会话、启动 turn、修改配置或发送消息。
...
```text
1. 有有效 Session ID：精确读取并校验 workspace
2. ID 非法或失效：按 name + normalizedWorkspace 查询
3. 唯一匹配：自动绑定并持久化 ID
4. 多个匹配：返回 candidates，等待用户选择
5. 零匹配：保持 pending-new；只有明确提交点才允许创建
```
...
添加 Agent 后应自动展开参数面板并执行只读扫描。面板至少展示：
...
- 保存前先 lookup-only；唯一匹配自动绑定，多匹配要求选择。
- 零匹配且用户点击保存/应用时才创建。
- 创建中禁用重复提交并显示明确状态。
...
- 同一名称因并发或索引延迟重复创建。
- 只能看到前 N 条会话却没有分页/搜索。
- 找不到目标时静默切换同名会话、另一 Runtime 或另一设备。
- 真实消息可能由两条路径执行。
