...
2. **不存在会话时，先按 Manager 保存名称查找，只创建一次再投递。** ID 非法/失效时先按“RabiPC Manager 保存名称 + 规范化 workspace”查找；唯一匹配直接绑定，零匹配才幂等创建，持久化新 ID 后再投递；多个同名先让用户选择。用户也可显式点击“自动初始化会话”，由同一事务创建/绑定后投递包含角色文件、记忆、计划和必读项的人格初始化消息。
...
5. **Agent 端或 Rabi 端改名后必须安全重绑。** 保存名称与 ID 指向任务的当前名称不一致时，旧 ID 视为陈旧绑定；按保存名称 + workspace 重新查找，唯一匹配重绑、零匹配只创建一次、多匹配等待用户选择，不能继续投到旧 ID。
...
4. 唯一匹配：自动绑定并持久化名称 + ID
5. 多个匹配：返回 candidates，等待用户选择
