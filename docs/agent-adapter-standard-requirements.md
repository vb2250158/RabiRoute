...
1. **存在会话时，直接投递到 Rabi 配置所绑定的会话。** 用内部完整 Session ID 读取 owner 记录；ID 存在、workspace 一致且任务未归档就是有效绑定，随后直接续投同一任务，不新建。owner 的标题/SQLite `title` 是可变展示元数据，不能参与身份判定。保存 ID 指向已归档任务时，不再查找或复用其它同名任务；真实投递或保存提交点幂等创建一个新任务，持久化新 ID 后再投递。Desktop 唤醒重试后精确 ID 已无法从本地状态读取时，也按旧 ID 隔离地创建替代任务，投递成功后返回警告。
...
5. **改名不能隐式改变目标。** Agent/Desktop 端改名或内部标题元数据变化时，完整 ID + workspace 仍指向原任务，继续续投且不创建。用户在 Rabi 端明确输入新名称时，UI 清空旧 ID 后再按名称 + workspace 查找/创建并保存新 ID。
...
2. ID 存在、未归档且 workspace 匹配：直接绑定，不比较可变标题
...
- [ ] 完整 ID + workspace 是身份真源；名称/时间只作显示和无 ID 查找。
