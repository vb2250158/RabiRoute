...
1. **存在会话时，直接投递到 Rabi 配置所绑定的会话。** 用内部完整 Session ID 读取 owner 记录；ID 存在、workspace 一致且任务未归档就是有效绑定，随后直接续投同一任务，不新建。owner 的标题/SQLite `title` 是可变展示元数据，不能参与身份判定。保存 ID 指向已归档任务时必须明确阻止投递并要求恢复或重选。
...
flowchart TD
    ...
    I -->|"ID 为空、非法或确实不存在"| Q["按保存名称 + 规范化 workspace 查找"]
    I -->|"已归档或 workspace 冲突"| X["停止并要求恢复、重选或修正目录"]
...
3. ID 为空、非法或确实失效：按 savedName + normalizedWorkspace 查询
4. 一个或多个匹配：按 updatedAt 降序自动绑定并持久化唯一最新任务的 ID、名称和 workspace
