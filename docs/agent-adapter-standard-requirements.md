...
基础接入由 RabiRoute 统一管理绑定、解析、投递和回执，各端只适配宿主实际拥有的会话接口。DSH 使用其 owner API；不以额外的 Rabi 工具插件、插件固定版本或增强状态作为基础发现/投递前提。扫描不请求旧 `rabirouteAgent/status`，也不提示安装退役的插件集合。

宿主增强是独立可选能力。例如 DSH 任务绑定 Rabi 计划后在侧边栏展示计划，属于未来界面扩展，**目前未实现**，不要求 Codex 或新增 Agent 端复制同一界面。增强只通过受管接口访问 Manager 的权威计划，不另存业务真源。生命周期 Hook 也按实际需要单独安装、诊断和验收；增强可选不允许绕过既有审批、正式外发或权限合同。

迁移不自动卸载已安装扩展、不变更原任务绑定、不回滚 DSH profile；先使基础扫描不依赖扩展，再分别验证宿主会话发现、同任务续投和增强功能。源码测试通过不等于运行 Manager 已部署，真实投递仍须单独验收。

## 功能归属：Rabi 优先，宿主只补必要能力

本节是 Rabi 与 DSH、Codex 及其他 Agent 端的功能归属规范，不是已部署功能清单。能由 Rabi 统一实现的业务与策略，应在 Rabi 实现并通过公共合同提供给各端；宿主增强只补 Rabi 无法通过现有受支持接口直接完成、且必须进入宿主内部的能力。Rabi 暂时缺少接口不是把业务搬到 DSH 的理由，应先评估补齐 Rabi 接口。

### 职责划分

- **Rabi 统一管理业务与策略**：人格定义、上下文选取策略、记忆管理与整理调度、计划状态、秘书与业务任务绑定、消息路由、回复请求、渠道发送及回执。需要模型思考或工具执行时，Rabi 可委派给 DSH、Codex 等处理端；执行结果回到 Rabi，不把规则和业务真源迁到处理端。
- **Agent 负责真实执行**：模型调用、回答问题、文件和工具操作、宿主会话生命周期及本地权限执行仍归实际 owner。Rabi 的业务授权不扩大宿主权限，也不替代宿主的审批和沙箱。
- **适配层负责翻译和传递**：Rabi 侧 adapter 使用宿主会话 API；DSH 工具插件把模型请求转为 Manager 调用并返回结果；生命周期 Hook 上报宿主事件、应用 Manager 返回的上下文或决定。工具注册不等于自动调度，Hook 事件转发不等于在宿主实现人格或记忆策略。
- **宿主增强只处理宿主专属缺口**：例如 DSH 内部事件捕获、在正式扩展点注入上下文、执行工具拦截、展示绑定计划的本地界面。这些是归属示例，不表示全部已经实现。展示和暂存不得成为业务真源；增强不能复制 Rabi 计划引擎、记忆库、路由器或跨 Agent 调度器。

### 开发前判断

每项需求在设计或任务说明中写清以下内容，再进入实现：

1. **业务归属**：该能力是否也适用于 Codex 或其他 Agent？跨端通用的业务规则和状态由 Rabi 持有，不按 DSH 工具名称或界面结构定义公共业务模型。
2. **接口缺口**：列出可复用的 Rabi 接口；接口不足时说明应补充的公共能力。只有确实需要宿主内部事件、权限或 UI，才把最小实现留在宿主，并说明 Rabi 无法直接完成的具体原因。
3. **最小分工**：分别列出 Rabi 的决定与状态、adapter 的协议转换、宿主的动作与临时状态。不得用第二份计划、人格、记忆、绑定或回复队列换取接入方便；必要缓存必须声明真源、失效和恢复方式。
4. **跨端与降级**：明确 DSH、Codex 及其他端可共用的合同和各自能力差异。未安装可选增强的端仍按其基础接入合同工作；缺少宿主能力时如实报告，不伪造对等能力、不暗中换 Runtime 或降低安全要求。
5. **验收证据**：分别验证公共业务、宿主适配和实际目标会话。跨端公共行为应覆盖 DSH 与 Codex 的适用路径；无法实测的端标为未验证，不以另一端成功代替。工具已注册、Hook 已加载、Manager 健康、真实投递完成必须分开报告。

### 典型归属

- 自动整理记忆、推进计划、选择秘书、安排跨 Agent 回复：Rabi 负责规则、调度和状态；处理端按授权执行委派任务。
- 在 DSH 工具执行前取得事件并执行 Manager 的拒绝决定：DSH Hook 负责本地事件和执行，Rabi 负责业务策略，宿主保留自身权限限制。
- 在 DSH 侧边栏展示 Rabi 计划：DSH 可提供可选 UI，读取 Manager 真源；Codex 无须复制界面，也不应因此失去公共计划能力。此侧边栏示例目前未实现。
- “让模型能调用 Rabi API”：属于工具接入，不应宣传为 DSH 独立实现了计划、记忆或消息系统。
...
1. **存在会话时，直接投递到 Rabi 配置所绑定的会话。** 对 Codex 用内部完整任务 ID 读取 owner 记录；ID 存在且任务未归档就是有效绑定，随后按当前配置的 workspace 续投同一任务，不新建。任务保存的默认 cwd、owner 标题和 SQLite `title` 都不能参与身份判定。固定归属 workspace 的其它 Agent 端仍按各自合同校验。保存 ID 指向已归档任务时，不再查找或复用其它同名任务；真实投递或保存提交点幂等创建一个新任务，持久化新 ID 后再投递。Desktop 唤醒重试后精确 ID 已无法从本地状态读取时，也按旧 ID 隔离地创建替代任务，投递成功后返回警告。
2. **不存在会话时，先按 Manager 保存名称查找，只创建一次再投递。** 只有 ID 为空、非法或确实不存在时，才按“RabiPC Manager 保存名称 + 规范化 workspace”查找；存在一个或多个候选时按 `updatedAt` 降序绑定唯一最新任务，最新时间并列才让用户选择；零匹配才幂等创建，持久化新 ID 后再投递。用户显式输入新名称时 UI 必须先清空旧 ID。
...
5. **改名或默认 cwd 变化不能隐式改变目标。** Codex 完整 ID 仍指向原任务，当前 workspace 只决定本次执行目录，因此继续续投且不创建。用户在 Rabi 端明确输入新名称时，UI 清空旧 ID 后再按名称 + workspace 查找/创建并保存新 ID。
...
```mermaid
flowchart TD
    M["RabiRoute 准备投递真实消息"] --> I{"精确读取保存 Session ID"}
    I -->|"存在且未归档"| B["绑定原会话；按当前 workspace 投递"]
    I -->|"ID 已归档"| C2["按旧 ID 隔离的幂等键创建新会话"]
    I -->|"ID 为空、非法或确实不存在"| Q["按保存名称 + 规范化 workspace 查找未归档任务"]
    Q -->|"一个或多个同名"| N{"是否有唯一最新 updatedAt？"}
    Q -->|"零匹配且原 ID 不存在"| C["按幂等键创建一次会话"]
...
```text
1. 有有效 Session ID：精确读取 owner 记录
2. Codex ID 存在且未归档：直接绑定并使用本轮 workspace，不比较任务保存的默认 cwd 或可变标题
3. ID 已归档：真实投递或保存提交点直接幂等创建新任务，不复用其它同名任务；扫描、刷新和失焦仍只报告归档状态
4. ID 为空、非法或确实失效：按 savedName + normalizedWorkspace 查询
5. 一个或多个匹配：按 updatedAt 降序自动绑定并持久化唯一最新任务的 ID、名称和 workspace
6. 最大 updatedAt 并列或都无有效时间：返回 candidates，等待用户选择
7. 零匹配：空/非法/失效 ID 按名称决定是否创建；归档 ID 按旧 ID 隔离的幂等键创建新任务
```
...
- [ ] Codex 完整 ID 是任务身份真源；workspace 是本次执行目录，默认 cwd、名称和时间只作运行、显示或无 ID 查找。
- [ ] Agent 端改名或首投改写标题后仍续投同一 ID；Rabi 端输入新名称先清空旧 ID，再安全重绑或只创建一次。
