...
sequenceDiagram
    ...
    G->>P: 16 kHz mono PCM
    P->>R: audio-streams/rabilink start/chunk/stop
    R->>PC: 有序 PCM
    PC->>PC: RabiSpeech VAD/切句/ASR/声纹
    PC->>PC: 主机通用语音库只写一次
    PC->>A: 已启用 Route 按 hot/keyword 投递
    A->>R: Outbox 下行文本
    R-->>P: outbox_available 事件
    P->>R: 按 cursor 读取一次增量
    P->>R: 受鉴权的 TTS 请求
    R->>PC: RabiSpeech TTS
    PC-->>P: WAV
...
Relay 默认限制单个附件 64 MiB，可通过环境变量调整。手机现在会先把媒体二进制和元数据写入应用私有磁盘队列，再串行慢传；失败项保留，支持显式/事件重试，并按 7 天或 500 项裁剪。文本/control 同样使用 48 小时、2000 项私有磁盘队列。断点续传和最终 delivered/played 回执仍是上线前可靠性工作。

## 生命周期与安全

- 手机后端已经迁移到带常驻通知、可暂停的 `RabiConversationService` Foreground Service，并使用 `START_STICKY`、开机恢复和独立输入模式管理；Activity 只负责表现与用户操作。系统回收、重启和厂商后台限制仍需真机验收。
...
3. 一段录音只产生一条主机通用语音记录；已启用 Route 再按 `hot/keyword` 进入对应人格，双通道重复控制消息不会二次提交。
...
2. 真机验证现有 Foreground Service 的系统回收、重启、连接/暂停和输入模式切换。
3. 在已有文本/control/媒体磁盘队列和保留期基础上补齐弱网退避与投递/播放回执；明确短暂 PCM 在进程死亡时的产品取舍。
