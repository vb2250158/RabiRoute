...
当前 RabiRoute 已经支持通用 `webhook` 入站：外部系统可以 `POST /webhook` 把文本事件送入路由。完整的手机 App 双向接入还需要新增 mobile 专用会话、WebSocket 长连接和 per-device 消息状态队列。本文把“现在能用的最小链路”和“正式双向接入需要实现的接口”分开描述。
...
- App 通过 HTTPS 发一条文本，RabiRoute 记录为 mobile / webhook 入站事件。
