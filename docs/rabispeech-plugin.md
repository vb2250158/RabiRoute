...
- 开启“划词朗读”后，在 Windows 任意支持文本选区的软件中拖动划选文字会显示系统悬浮条：左侧“朗读”点击后进入主机播放队列，右侧“投递至当前人格”通过角色面板消息投递给托盘当前选中的 Route；划选本身不会自动执行动作，朗读默认使用当前可用的默认 TTS 模型，只有开启“高级选项”后才显示并使用固定模型选择；
