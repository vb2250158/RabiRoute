...
NapCat 图片在消息进入时立即下载到运行目录，并记录 `ready` 或 `unavailable`。`ready` 图片同时作为 Desktop `localImage` 输入投递，引用链中已保存的图片也会一并带入。每个新需求列出必须核对的原消息 ID、引用链 ID 和附件 ID；Agent 提交 `sourceEvidenceReview` 后才能回复或关闭。任何附件不可读时，Manager 拒绝猜测式回复，只允许重新取图或转交。
