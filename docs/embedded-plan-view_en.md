...
When reopened, the page immediately displays its last full snapshot from browser session storage and sends its view revision to Manager. An unchanged view returns `304` without transferring plan details or attachment metadata again. Plan, approval, or persona status configuration changes return a new snapshot. The archive holds at most eight plans and four million JSON characters; a browser storage quota failure skips archiving without blocking the current view. A failed validation remains visible as an error rather than treating old content as newly verified. Plan events refresh only the addressed plan.
