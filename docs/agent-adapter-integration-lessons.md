...
4. ID 不是合法任务 ID、已失效或名称与 ID 指向记录不一致时，清除旧绑定并按“保存名称 + 规范化 cwd”自动解析：唯一匹配重新绑定，零匹配创建，多个同名要求用户选择。
...
3. 名称与 ID 不一致时旧 ID 视为陈旧绑定，按保存名称 + cwd 查询；唯一匹配重绑、零匹配幂等创建、多匹配要求选择。
...
- 配置绑定统一走一个 resolver：读取有效 ID 并校验保存名称 → 名称 + ID 与 cwd 都匹配才精确绑定；无效/失效/名称不一致 → 保存名称 + 规范化 cwd；唯一匹配 → 自动绑定；零匹配 → 创建空任务；多个同名 → 返回候选。
