...
3. 投递前从 Desktop 状态按精确 ID 读取，并校验 `cwd` 与归档状态；不要用可变 `title` 否定有效 ID。若精确 ID 已归档，先查找同 workspace 的未归档同名任务；存在唯一最新候选时复用它，零候选时停止，绝不能因归档绑定创建替代任务。
...
| 保存 ID 指向已归档误副本 | 先复用同 workspace 唯一最新的未归档同名任务；没有候选才要求恢复/重选；不得创建替代任务 |
