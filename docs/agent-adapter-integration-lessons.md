...
这份文档总结 RabiRoute 接入 Codex、Copilot CLI、Marvis、AstrBot、DSH 和远程 Agent 时出现过的典型问题，并给出统一设计规则。重点不是某个端口或某段脚本，而是把 **Agent 所有权、会话身份、工具能力和 RabiRoute 路由职责** 分开。
...
3. 投递前从 Desktop 状态按精确 ID 读取，并校验 `cwd` 与归档状态；不要用可变 `title` 否定有效 ID。若精确 ID 已归档，真实投递直接幂等创建新任务并更新绑定，不查找或复用其它同名任务；扫描和状态刷新仍只报告归档。
4. ID 为空、不是合法任务 ID 或确实不存在时，按“保存名称 + 规范化 cwd”自动解析：一个或多个同名候选按 `updatedAt` 绑定唯一最新者，零匹配创建，最大时间并列才要求用户选择。
...
根因：resolver 把“保存名称必须等于 owner 当前 `title`”当成身份条件。该字段并不稳定；真实轮次后自动标题、首条 prompt 或索引延迟都可能改变它。更隐蔽的一种错误是直接用 SQLite `threads.title` 构建同名索引：Desktop 侧边栏仍显示人工名称，但数据库标题已经是首条 prompt，于是 lookup 返回零匹配并错误创建新任务。
...
症状：停留在设置页时 Manager 持续收到 `/api/scan/agents`，展开卡片、输入、失焦或保存也会重复读取全部项目和任务。

根因：把扫描同时当成页面初始化、状态刷新、字段校验和保存后的同步手段，扫描触发点分散在多个 watch/event 中。

正确处理：

1. 进入 Agent 设置页或打开快速配置时自动扫描一次，并缓存结果供下拉选择。
2. 后续只有用户点击“扫描/刷新”按钮才再次向 RabiPC Manager 请求项目和会话列表。
3. 展开/折叠、添加 Agent、输入、`blur`、保存、健康轮询和计时器都不能触发全量扫描；按名称 lookup 与全量 scan 分开。
4. 测试记录请求次数：页面进入 `+1`，每次显式点击 `+1`，其余交互和空闲等待保持不变。

### 3C. 测试和构建都绿，安装版仍运行旧逻辑

症状：源码单测已经证明只创建一次，实际 RabiPC Manager 仍创建两个任务；源目录与安装目录某些文件哈希相同，却仍观察到旧行为。

根因：TypeScript 输出根目录漂移产生 `dist/src/*`，旧 `dist/*.js` 没被清理；安装器采用覆盖复制，目标目录又保留旧文件，启动脚本继续运行旧根入口。

正确处理：固定 `rootDir`、构建前删除受控 `dist`、禁止 `dist/src` 阴影树；安装时对代码产物目录做安全的镜像替换而非叠加复制。验收必须检查实际 launcher 入口包含新合同、源/安装关键文件哈希一致且安装目录没有阴影运行树。
...
- 存在多个同名同 cwd 匹配时自动绑定 `updatedAt` 唯一最新者，不创建新任务；最大时间并列或都无有效时间时才让用户选择。
...
| 安装版启动入口 | 无 `dist/src` 阴影树；实际 launcher 文件包含当前 resolver；源/安装关键产物哈希一致 |
...
- [ ] 完成重名、失效 ID、两端改名、错误 cwd、扫描次数、工具缺失和认证过期测试。
- [ ] 验证构建与安装目录没有阴影运行树，实际启动入口就是本次构建产物。
