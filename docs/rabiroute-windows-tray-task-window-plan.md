...
计划展开区采用“行动摘要优先”的层级：顶部双栏展示 `currentStep` 与 `nextAction`，其后展示优先级、类型、项目、等待事项、时间、来源和文件等已有元数据。托盘读取层兼容可选 `steps` 数组；只有源文件真实提供步骤时，才根据步骤状态计算完成数并显示进度条、前 6 个步骤和“查看全部”入口。这个展示计算是只读派生，不回写计划，也不会在缺少步骤时虚构百分比。
