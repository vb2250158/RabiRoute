...
托盘窗口通过 Manager API 只读消费角色 DTO，不负责新建、修改、完成、归档计划，也不负责写入记忆。读写和文件事实源统一由 Manager 管理。
...
- `当前`：先展示 Manager API 返回的 `进行中` 计划，再展示近期记忆；文件事实源仍归 Manager 后端。
- `计划`：全部未归档计划的只读总览。
...
计划展开区采用“完整步骤优先”的层级：源计划提供 `steps` 时，顶部先显示完成数、进度条和 `currentStepId` 指向的“当前执行：第 N 步”，随后一次列出全部步骤，并高亮唯一的 `进行中` 步骤。完整步骤已经表达后续路径，因此不再重复展示 `nextAction`。只有 Manager 返回 `presentation.tone=blocked` 时，界面才把状态、当前位置和步骤行显示为“阻塞中 / 当前阻塞 / 已阻塞”；原始 `blockedBy` 文本不触发第二套判断。优先级、类型、项目、截止和更新时间压缩为摘要；来源、创建时间和文件收进可展开的计划资料。这个展示计算是只读派生，不回写计划。旧计划缺少 `steps` 时明确提示无法展示完整步骤和准确执行位置，并保留旧版当前/下一步兼容区。

只有 Manager 返回的结构化当前步骤是进行中的 `qa-* / verify-*` 时，卡片右上角才显示紫色“等待 QA 验收”。未来 QA 步骤或实施步骤正文中的“QA 门禁 / 尚未通知 QA”不触发；托盘不扫描自由文本关键词。实施/打包/QA 状态机只适用于代码、Prefab、资源、配置等会改变项目内容的计划；调查、设计评审、运营、资料收集、外部依赖与控制面维护必须保留真实流程。
