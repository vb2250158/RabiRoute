...
| Per-endpoint recent context | current | persona `recentMessageLimits` for 11 logical endpoints | `0–200`, default `12`; `0` disables auto-injection only. Inbound and outbound records share the budget within the current persona, logical endpoint, and conversation | persona sliders plus exact numeric inputs | shared config model and AgentPacket |
...
| Plan API | current | handler/Manager call | five top-level states; paused plans preserve the resume step, stay only under Plans, and sort last; steps may store approval execution contracts, while Manager lightly derives `none/incomplete/ready`, missing fields, shared colors, and ordering for WebGUI and Qt | `/api/roles/:roleId/plans` | `src/roleKnowledge.ts`, `src/roleKnowledgePresentation.ts`, role route parser/control plane |
| Plan approval feedback | current | `data/roles/<RoleId>/plans/feedback/*.jsonl` | Agent, WebGUI, and Qt tray | persists and returns `pending`, notifies the Agent in the background, then emits terminal state through Manager events; never updates the plan directly | `GET/POST /api/roles/:roleId/plans/:planId/feedback` | `src/planFeedback.ts`, Manager control plane |
