...
| Remote Agent | experimental | primary-Manager devices/tasks plus Host device/password/Agent profile | primary controller creates and owns tasks; lightweight Host reuses Agent UI/adapters and returns terminal events without route/persona/plan control surfaces | primary `/api/remote-agent/*`; loopback Host settings/Agent APIs | `src/messageEndpoints/remoteAgentManager.ts`, `src/remoteAgentHost.ts`, `src/remoteAgentHost/*` |
...
| Agent thread bridge | current | `/api/agent/threads` | list/read/resolve/create/rename/send formal Codex tasks through the exact Desktop owner; persistent plan-assistant slots reuse this contract | local Manager API | `src/agentThreads.ts`, `src/codexRuntime.ts`, `src/manager/controlPlaneRoutes.ts` |
| Plan API | current | handler/Manager call plus persona-private `plans/attachments/<planId>/` | five top-level states and managed attachments; Manager derives shared `Executing / Awaiting QA / Awaiting shared package / Awaiting environment, assets, information, or external response / Awaiting approval / Paused / Completed` presentation stages, `counts.stages`, palettes, and ordering from the structured current step, approval contract, and authoritative wait fields. WebGUI and Qt consume the DTO, use a neutral unknown state when `presentation` is absent, and never maintain a second classifier. WebGUI uses the constrained Manager endpoint for fixed-width 16:9 image, video, and short Markdown preview cards, complete in-page previews, and ordinary file cards instead of reading arbitrary local paths | `/api/roles/:roleId/plans`, `GET /api/roles/:roleId/plans/:planId/attachments/:attachmentId` | `src/roleKnowledge.ts`, `src/planAttachments.ts`, `src/roleKnowledgePresentation.ts`, `src/roleKnowledgePagination.ts`, `src/manager/planAttachmentRoutes.ts`, control plane |
| Plan approval feedback | current | `data/roles/<RoleId>/plans/feedback/*.jsonl`, private `attachments/<feedbackId>/`, and managed attachments from the current plan | bound business task, persona Agent, WebGUI, and Qt tray | validates and persists text/uploads; WebGUI can type `@` to select plan attachments, while Manager validates stable IDs and snapshots references; returns `pending`; a complete `taskBinding` sends approval directly to the original business task through Desktop IPC and notifies the persona that delivery already happened, while missing/failed bindings fall back to persona handling; no chat history or recent messages; never updates the plan directly | `GET/POST /api/roles/:roleId/plans/:planId/feedback` | `src/planFeedback.ts`, `src/manager/planApprovalFeedbackDelivery.ts`, Manager control plane, `src/routing/systemEventRules.ts` |
