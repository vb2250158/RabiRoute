...
| Plan-task completion reminder | experimental; code and mock path tested | plan `taskBinding`, Codex Stop session/turn/cwd/final message, and exact persona Route/gateway binding | Stop Hook → Manager → role-panel timeline → Forwarding / AgentPacket → main persona updates the plan and continues the original assistant task in the same turn; routing deduplicates by session plus turn and never mutates the plan itself | plan POST/PATCH `taskBinding.completionHook` plus Route `codexHooks.planTaskCompletionEnabled`; the reminder contract requires exact `taskBinding` continuation, slot release/reassignment on completion or pause, and zero actionable idle plans; real two-Desktop-task acceptance is still pending | `src/manager/codexHookContext.ts`, `src/manager/planTaskCompletionDelivery.ts`, `src/manager/controlPlaneRoutes.ts`, `plugins/rabi-codex-context/` |
...
| Plan API | current | handler/Manager call plus persona-private `plans/attachments/<planId>/` | five top-level states and managed attachments; paused plans preserve the resume step, stay only under Plans, and sort last; steps may store approval execution contracts, while Manager lightly derives `none/incomplete/ready`, missing fields, shared colors, and ordering for WebGUI and Qt. WebGUI uses the constrained Manager endpoint for 16:9 image/video thumbnails, in-page media preview, and ordinary file cards instead of reading arbitrary local paths | `/api/roles/:roleId/plans`, `GET /api/roles/:roleId/plans/:planId/attachments/:attachmentId` | `src/roleKnowledge.ts`, `src/planAttachments.ts`, `src/roleKnowledgePresentation.ts`, `src/manager/planAttachmentRoutes.ts`, control plane |
| Plan approval feedback | current | `data/roles/<RoleId>/plans/feedback/*.jsonl` plus private `attachments/<feedbackId>/` | Agent, WebGUI, and Qt tray | validates and persists text/attachments, returns `pending`, emits an independent `plan_feedback` event without chat history or recent messages, then publishes terminal state through Manager events; never updates the plan directly | `GET/POST /api/roles/:roleId/plans/:planId/feedback` | `src/planFeedback.ts`, Manager control plane, `src/routing/systemEventRules.ts` |
