...
| Plan-task completion reminder | experimental; code and mock path tested | plan `taskBinding`, Codex Stop session/turn/cwd/final message, and exact persona Route/gateway binding | Stop Hook → Manager → role-panel timeline → Forwarding / AgentPacket → persona Route handler session; deduplicated by session plus turn, ignored for paused plans, and never auto-advances the plan | plan POST/PATCH `taskBinding.completionHook` plus Route `codexHooks.planTaskCompletionEnabled`; omitted plan Hook defaults on; real two-Desktop-task acceptance is still pending | `src/manager/codexHookContext.ts`, `src/manager/planTaskCompletionDelivery.ts`, `src/manager/controlPlaneRoutes.ts`, `plugins/rabi-codex-context/` |
