...
| Plan approval feedback | current | `data/roles/<RoleId>/plans/feedback/*.jsonl`, private `attachments/<feedbackId>/`, and managed attachments from the current plan | Agent, WebGUI, and Qt tray | validates and persists text/uploads; WebGUI can type `@` to select plan attachments, while Manager validates their stable IDs and snapshots the references; returns `pending`, emits an independent `plan_feedback` event without chat history or recent messages, then publishes terminal state through Manager events; never updates the plan directly | `GET/POST /api/roles/:roleId/plans/:planId/feedback` | `src/planFeedback.ts`, Manager control plane, `src/routing/systemEventRules.ts` |
