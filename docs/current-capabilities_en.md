...
- Plans & Memory first reads Manager-sorted lightweight summaries containing title, type, status, and ordering metadata. The larger memory read starts only after that first summary response so synchronous disk I/O cannot compete with the first screen. Cursor summaries continue even when the user does not scroll. Body text, steps, approvals, and attachment metadata hydrate only for cards genuinely near the viewport; each observer turn promotes the nearest two cards with at most two concurrent requests. A full-content search requests the remaining details only when needed. Memory cards render in bounded batches, while image/video attachments use light loading/failure placeholders and low-priority lazy loading so large knowledge sets do not block the first screen.
