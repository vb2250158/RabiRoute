...
- Persistent plan assistants use two-level concurrency: the main task binds one unfinished plan to one assistant slot; that assistant may create temporary child agents, but must remain the long-lived owner, aggregate results, update the plan, and report back to the main task. After a stage result returns, the main persona must consume it and continue the exact plan `taskBinding` in the same turn. Completion or pause releases the slot for the next actionable plan, and independent plans should fill all available slots in parallel. Code, configuration, and contract tests cover the model, while real Desktop multi-task visibility and tool-owner acceptance remain pending, so the feature is still experimental.
...
- Experimental plan-task completion reminders are implemented. A plan binds an exact Codex execution session through `taskBinding`; omitting `completionHook` defaults it to enabled, while `completionHook.enabled=false` disables one plan. The `Stop` Hook sends the official final answer to Manager, which reminds the same persona's main task through role-panel, Forwarding, and AgentPacket. The routing layer deduplicates by session plus turn and never mutates the plan itself. The reminder contract requires the main persona, in the same turn, to read and update the plan and memory, continue the original `taskBinding.sessionId + workspace`, or release and reassign the slot after completion/pause; the turn ends only when no actionable plan is idle. Conflicts fail closed. Code, HTTP, plugin, and mock RolePanel-path tests exist, but two-real-Desktop-task acceptance is still pending.
