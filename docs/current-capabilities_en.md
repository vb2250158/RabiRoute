...
- `AgentPacket` includes the event, recent bidirectional messages for ordinary endpoints in the current persona/logical endpoint/conversation, role and relative paths, plan/memory/skill indexes, required-read items, log paths, reply API, and `replyContext`. For an enabled current Route, `replyContext` also carries a cross-persona capability bound to that Route and persona. When persistent plan-assistant tasks are configured, it also includes each slot's full ID, name, workspace, and assignment rules. Heartbeat and `plan_feedback` always omit the history section.
- Persistent `协助处理计划` tasks are plan-management secretaries, not business-execution owners. They maintain plans and memory, deduplicate and bind business tasks, inspect real status, consume results, and continue the bound task. A plan's `taskBinding` always identifies an independent business task. Neither a secretary nor its temporary control-plane child agents may modify business files; investigation, implementation, and validation stay in the business task. The main persona must act on stage results in the same turn, while multiple secretaries manage separate plan shards. Real Desktop multi-task acceptance remains pending, so the feature is still experimental.
...
- Message-group processing is experimental. With `messageProcessingAgents.codex.enabled`, chat messages automatically write raw and persona conversation records before settling into recoverable groups; there is no per-endpoint off switch. ASR/voice transcripts and structured events continue direct delivery. Dispatch prefers an explicit reply match, then endpoint, conversation, and speaker familiarity, and creates a Desktop task when no worker matches. Tasks use stable persona-prefixed `协助处理消息` titles, add one-based numbering when multiple workers exist, and migrate old titles by renaming the same task ID without changing its owner. Message Agent turns use `gpt-5.6-luna` / `medium`. Fragments arriving during delivery remain queued as same-group supplements instead of being removed with the preceding batch. Recovery validates persisted state again: derivable runtime fields receive safe defaults, while groups missing required message identity, conversation, or content fields do not enter scheduling. **Message Agent mode**, **Plan assistant tasks**, and **Hook management** are capability-gated handler extensions and are currently supported only by Codex; other Agent cards do not show them and non-Codex configuration does not retain them. Automated tests cover recovery, deduplication, concurrent supplements, and HTTP delivery; live group/DM and four-Agent acceptance remain pending, so this is not verified capability.
...
- Plans & Memory first reads eight Manager-sorted lightweight summaries containing title, type, status, and ordering metadata. The larger memory read starts only after that first summary response. Larger background cursor pages then complete the directory without requiring scroll, yielding one rendering frame between pages instead of waiting for body hydration. The body mounts a bounded forward window from the selected directory target, and later batches append below the current reading position rather than inserting older cards above it. Body text, steps, approvals, and attachment metadata hydrate only for cards genuinely near the viewport; each observer turn promotes the nearest two cards with at most two concurrent requests. A full-content search requests the remaining details only when needed. Memory cards render in bounded batches, while image/video attachments use light loading/failure placeholders and low-priority lazy loading so large knowledge sets do not block the first screen.
...
- Experimental plan-task completion reminders are implemented. A plan binds an exact independent business task through `taskBinding`; omitting `completionHook` defaults it to enabled, while `completionHook.enabled=false` disables one plan. The `Stop` Hook sends the official final answer to Manager, which reminds the same persona's main task. The main task assigns a plan secretary to update plan/memory and continue the original business `taskBinding`. Secretary rotation or plan pause never clears the business binding. The turn ends only when no actionable plan is unmanaged and no actionable business task is idle. Conflicts fail closed. Code, HTTP, plugin, and mock RolePanel-path tests exist, but real Desktop acceptance is still pending.
