...
需要 Node.js、Python 3.10+、PyInstaller 和 Inno Setup 6。脚本只从 Git 跟踪文件复制公开运行资源，不复制根目录 `data/`、日志、录音、转写、`.env` 或本机配置；然后嵌入固定版本的 Windows x64 Node.js，使用生产依赖启动临时 Manager 做 `/meta` 冒烟测试，最后生成安装器、便携 ZIP 和 SHA-256 清单。
