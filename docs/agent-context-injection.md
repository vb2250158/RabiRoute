...
命中召回发生在 Agent 投递前的热路径上，必须保持轻量。它不应该在每条消息到来时全量扫描、全量分词或读取大量文件。

计划和记忆的可检索关键词由 Agent 在写入或更新时主动提供。RabiRoute 只维护标题和 `keywords` 索引；消息到来时只对当前消息做简单匹配，然后查询已有索引。

新增或更新近期记忆时，`keywords` 必须存在且至少包含一个关键词。

命中召回只覆盖当前仍有操作价值的内容：

- 近期记忆中默认不直接显示的部分，默认是 `updatedAt` 超过 24 小时且尚未沉淀的近期记忆。
...
命中召回只注入索引，不注入全文。Agent 如果需要更多细节，应按 ID 查询对应计划或记忆。

MVP 使用标题 `includes` 和 Agent 写入的 `keywords` 做匹配。不在投递前做智能分词。后续如果需要更复杂的中文分词，也应作为写入/更新时的离线辅助，不进入消息投递热路径。
