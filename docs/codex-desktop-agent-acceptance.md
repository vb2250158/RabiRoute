...
1. 已绑定 ID 存在、workspace 一致且任务未归档时，直接投递到该任务，不创建新任务；Desktop 标题或 SQLite `title` 变化不影响绑定。保存 ID 指向已归档任务时，先在同 workspace 查找未归档的同名任务：存在唯一最新候选就复用，零候选才返回明确错误；任何情况下都不得因归档绑定创建替代任务。
2. 已绑定 ID 为空、非法或确实不存在时，按 `保存名称 + 规范化 workspace` 查找：一个或多个候选按 `updatedAt` 降序重绑唯一最新任务，零匹配才幂等创建一次；最大时间并列时让用户选择。
...
5. Desktop 改名或内部标题自动变成首条 prompt 时仍续投完整 ID；用户在 Rabi 明确输入新名称时，前端先清空旧 ID，再执行第 2 条完成目标切换。
...
flowchart TD
    ...
    V -->|"未归档"| B["复用现有绑定"]
    V -->|"已归档或不存在"| L["按名称 + workspace 查找未归档任务"]
    L -->|"一个或多个同名"| N{"是否有唯一最新 updatedAt？"}
    L -->|"零匹配且原 ID 不存在"| C["幂等创建一个空任务"]
    L -->|"零匹配且原 ID 已归档"| E["停止；要求恢复或重选"]
    N -->|"是"| P["持久化最新任务的新配对"]
...
| 保存 ID 指向已归档误副本，另有同名有效任务 | 重绑唯一最新有效任务；任务数不变 |
| 保存 ID 指向已归档任务且没有同名有效任务 | 阻止投递并要求恢复/重选；任务数不变 |
...
4. 用测试锁定 ID + workspace 稳定续投、标题改写、归档误副本恢复、归档零候选阻止、Rabi 显式改绑、single-flight、索引延迟、全量列表和扫描次数。
