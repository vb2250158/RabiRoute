...
1. 已绑定任务存在且未归档时，直接投递到 Rabi 保存的任务，不创建新任务；保存 ID 指向已归档任务时返回明确错误并要求恢复或重选，绝不能自动创建替代任务。
2. 已绑定任务不存在、ID 失效或名称与 ID 不再配对时，按 `保存名称 + 规范化 workspace` 查找：一个或多个候选按 `updatedAt` 降序重绑唯一最新任务，零匹配才幂等创建一次；只有最大时间并列或都无有效时间时才让用户选择。
