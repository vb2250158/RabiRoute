...
1. 已绑定 ID 存在、workspace 一致且任务未归档时，直接投递到该任务，不创建新任务；Desktop 标题或 SQLite `title` 变化不影响绑定。保存 ID 指向已归档任务时返回明确错误并要求恢复或重选。
2. 只有已绑定 ID 为空、非法或确实不存在时，才按 `保存名称 + 规范化 workspace` 查找：一个或多个候选按 `updatedAt` 降序重绑唯一最新任务，零匹配才幂等创建一次；最大时间并列时让用户选择。
...
| 保存 ID 指向已归档任务 | 阻止投递并要求恢复/重选；任务数不变 |
...
4. 用测试锁定 ID + workspace 稳定续投、标题改写、归档阻止、Rabi 显式改绑、single-flight、索引延迟、全量列表和扫描次数。
