...
| --- | --- |
| Authentication | Reuses `webguiLan.accessToken`. No pairing code, UDP discovery, device password, or second Token. |
...
The signing key lives in Manager private runtime data. Replacing or losing it changes the fingerprint, and existing nodes reject subsequent updates. A rotation must redistribute the new fingerprint through a trusted channel and update `releasePublicKeySha256` in each node private configuration.
