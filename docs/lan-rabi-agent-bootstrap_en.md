...
- Requests outside the catalog are no longer rejected by a remote-only business subset; raw-path, encoding and query-credential checks remain. Unlisted paths, methods or query parameters continue to the existing management-token check. Node credentials cannot substitute for that token, even over loopback. Catalog-external business requests may therefore still return `401 WEBGUI_TOKEN_REQUIRED`; this is not full local/remote API parity. Enabling an Agent does not open administrator settings, grant changes, arbitrary files or host control. Existing loopback-only handlers still reject remote calls; do not proxy through localhost to bypass them.
- JSON bodies use the local `readJsonBody` default and each endpoint's explicit limit, without the additional remote-only 1 MiB cap or any new shared cap. Source-identity validation, upload limits and existing business-handler checks remain unchanged.
