...
| 计划 | 已有 | `data/roles/<RoleId>/plans`、追加式 `plans/history/*.jsonl` 与私有 `attachments/<planId>/` | roleKnowledgeSnapshot、Agent 接口、WebGUI 与托盘只读页 | AgentPacket 构造或 API 调用时 | `plan.status` 只能写九个配置值：分析中、待审批、执行中、等待打包、等待 QA、待讨论、暂停、完成、关闭，并作为列表、详情和排序的唯一状态真源。`archiveStatus` 独立使用未归档/已归档；只有完成或关闭计划超时后可归档，且不改变 `status`。已归档计划不参加关键词召回，只能按明确 ID 或归档视图读取。Manager 返回与 `plan.status` 完全相同的 `presentation.status`，WebGUI 与 Qt 只消费其色板、等级和视图，不从步骤或正文建立第二套状态 | `/api/roles/:roleId/plans`、`GET /api/roles/:roleId/plans/:planId/history`、`GET /api/roles/:roleId/plans/:planId/attachments/:attachmentId` | `src/roleKnowledge.ts`、`src/planAttachments.ts`、`src/roleKnowledgePresentation.ts`、`src/roleKnowledgePagination.ts`、`src/manager/planAttachmentRoutes.ts`、`src/manager/controlPlaneRoutes.ts` | `docs/plan-and-memory-model.md` |
