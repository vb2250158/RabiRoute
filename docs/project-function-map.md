...
| AgentPacket | 已有 | RouteDecision、role paths、logs、role knowledge、Route 的计划管理秘书绑定 | Agent adapter | 命中规则后 | 会触发 roleKnowledgeSnapshot，可能刷新记忆 viewedAt；向主任务注入持久秘书槽的完整 ID、名称、workspace 和控制面边界，明确秘书不得替代计划业务 `taskBinding`；只有显式 memory-consolidation 触发才评估并创建待整理 run | 代码内部；preview 仍未实现 | `src/routing/agentPacket.ts`、`src/roleKnowledge.ts` | `docs/agent-context-injection.md` |
...
| 计划 | 已有 | `data/roles/<RoleId>/plans` 与私有 `attachments/<planId>/` | roleKnowledgeSnapshot、Agent 接口、WebGUI 与托盘只读页 | AgentPacket 构造或 API 调用时 | 可通过 API 创建 / 更新五种顶层状态与受管附件；暂停保留恢复步骤、只进计划分类且绝对末尾；步骤可保存审批执行合同，Manager API 轻量派生 `none/incomplete/ready`、缺失项、统一色板与排序。WebGUI 通过受控 API 显示固定宽度的 16:9 图片、视频和 Markdown 简短预览卡片、页内完整预览及普通文件卡片，不读取任意本机路径 | `/api/roles/:roleId/plans`、`GET /api/roles/:roleId/plans/:planId/attachments/:attachmentId` | `src/roleKnowledge.ts`、`src/planAttachments.ts`、`src/roleKnowledgePresentation.ts`、`src/manager/planAttachmentRoutes.ts`、`src/manager/controlPlaneRoutes.ts` | `docs/plan-and-memory-model.md` |
| 计划审批意见 | 已有 | `data/roles/<RoleId>/plans/feedback/*.jsonl`、私有 `attachments/<feedbackId>/` 与当前计划受管附件 | Agent、WebGUI、Qt 托盘 | 用户从计划页提交文字/附件、在输入框用 `@` 引用计划附件，或 Agent 记录 QQ/其它入口的审批时 | 校验并落盘后返回 `pending`；Manager 按稳定附件 ID 校验并快照被引用的计划附件，后台生成独立 `plan_feedback` 事件通知 Agent；不写聊天历史/最近消息，再通过 Manager 事件发布终态；不直接更新计划 | `GET/POST /api/roles/:roleId/plans/:planId/feedback` | `src/planFeedback.ts`、`src/manager/controlPlaneRoutes.ts`、`src/routing/systemEventRules.ts` | `docs/plan-and-memory-model.md`、`docs/rabi-agent-interfaces.md` |
