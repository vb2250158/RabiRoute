...
| Outbox / Reply | 已有 | Agent reply request、replyContext、pipeline、`proactive` | QQ / WeCom / RabiLink / role panel 等回传 | Agent、定时器或规划器调用 `/api/agent/replies` 时 | 可能写 draft、阻止、外发、写回复日志；NapCat 群聊按 `messageId + replyToSource` 自动绑定源消息且避免重复 reply 段；RabiLink 文本可无前置 task 主动入队 | `POST /api/agent/replies` | `src/outbox.ts` | `docs/rabi-agent-interfaces.md` |
