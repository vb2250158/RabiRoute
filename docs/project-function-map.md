...
| RabiSpeech TTS / ASR 服务 | 实验支持，同步直接 API + RabiPC 语音消息端 + 模型发现 + 性能报告 | 本机 `plugin-adapters/rabi-speech/config.json`、provider registry、`data/roles/<RoleId>/voice`、RabiSpeech 私有模型缓存和本地 worker | 任意 HTTP 客户端经 RabiLink Relay 或本机回环 API；WebGUI 查询模型/参数、录音转写、人格 TTS；只有显式提交 Route 才进入 Agent | 插件启动后即可探测；公网调用还要求全局 Relay 显式开启语音中转 | 本机生成音频或转写；全局 FIFO 播放；Relay 与普通 API 不进入 Agent、人格路由或消息日志 | `/v1/models`、`/v1/audio/speech`、`/v1/audio/transcriptions`、`/v1/personas`、`/#/speech` | `plugin-adapters/rabi-speech/`、`src/manager/speechServiceStatus.ts`、`ribiwebgui/src/pages/SpeechServicePage.vue` | `docs/rabispeech-plugin.md`、`docs/rabispeech-performance-report.md` |
...
| Rabi Codex Context 插件 | 首个可测试版本 | 真实 Codex session ID、显式 RoleId 绑定、Rabi 角色目录 | Codex `SessionStart` / `UserPromptSubmit` Hook | 插件已安装且当前 session 显式绑定时 | 注入受限人格工作集、计划/记忆索引和关键词命中正文；未绑定会话无输出，不复制角色知识 | `[rabi:use <RoleId>]` 等严格控制标记 | `plugins/rabi-codex-context/`、`.agents/plugins/marketplace.json` | `docs/rabi-codex-context-plugin.md` |
