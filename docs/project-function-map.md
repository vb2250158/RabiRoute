...
| Remote Agent | 实验支持 | 主 Manager 的 devices/tasks + 远端 Host 的设备/密码/Agent profile | 主 Manager 控制端、轻量 Agent 消息端 | 设备发现/连接、任务创建、Agent 回传时 | 主 Manager 创建任务并归属计划/人格；Host 复用 Agent 卡片与 adapter 投递并回传终态，不拥有航线/人格/计划控制面 | 主端 `/api/remote-agent/*`；Host 本机设置与 Agent API | `src/messageEndpoints/remoteAgentManager.ts`、`src/remoteAgentHost.ts`、`src/remoteAgentHost/*` | `docs/remote-agent-manager-design.md`、`docs/rabi-agent-interfaces.md` |
