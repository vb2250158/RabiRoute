...
| DSH adapter | 实验支持 | Route 的 `dshSessionId`、`dshSessionName`、`dshCwd`、`dshBaseUrl` 与 DSH WebGUI 中预先创建的会话 | DSH apiproxy `POST /api/session.prompt` | 每次已匹配且主控 Agent 为 DSH 的消息投递时 | 只投递到已绑定会话；RabiRoute 不创建、重命名或归档 DSH 会话，也不自动切换备用 Agent。消息处理 Agent、计划协助、记忆整理和 Hook 仍由 Codex 管理 | Route Agent 端；先在 DSH WebGUI 创建会话并复制完整 ID | `src/dshSessionBridge.ts`、`src/agentAdapters/agentAdapter.ts`、`ribiwebgui/src/pages/RouteConfigPage.vue` | `docs/configuration.md` |
...
| Agent Codex 任务桥与必回复请求 | 已有；五分钟提醒仍需本机 Hook 纵向验收 | Desktop 任务状态、已配置的 `codexCwd`、`data/.runtime/agent-requests.json` | 主人格、计划 Agent、计划秘书、消息处理 Agent、Codex Desktop | Agent 调用 `/api/agent/threads`、目标轮次触发 `Stop`，或绕过投递触发 `PreToolUse` 时 | 查询/读取/解析 Desktop 任务，受控创建或改名空任务，并把实际消息交给精确 Desktop owner；Agent 互投必须声明投递源和是否要求回复，可正式回传结果并在未回复轮次结束五分钟后提醒；Route 开关可拒绝绕过 Rabi 的持久任务工具 | `POST /api/agent/threads`、`GET /api/agent/requests` | `src/agentThreads.ts`、`src/agentRequests/`、`src/manager/agentCommunicationHookPolicy.ts`、`src/manager/controlPlaneRoutes.ts` | `docs/rabi-agent-interfaces.md` |
...
| Windows 托盘 | 已有 | 与 RibiWebGUI 同源的 Manager HTTP API、打包资源 | Windows 桌面入口 | 用户启动托盘时 | 启动 / 退出 manager，异步显示 Manager DTO；不直接读取 `data/`。WebGUI“设置”页开启滑词菜单后，托盘用 UI Automation 读取任意支持文本选区的软件；划选后显示悬浮按钮，点击才执行。“滑词朗读”是子功能，关闭后只保留“投递至” | `/gateways?summary=1`、`/api/roles/:roleId/*`、托盘 exe | `desktop/tray-task-window/`、`scripts/build-tray-exe.ps1` | `docs/windows-launcher-and-packaging.md` |
