...
| RabiSpeech TTS / ASR 插件 | 实验支持，同步直接 API + 当前电脑状态页 + 可重复模型基准 | 本机 `plugin-adapters/rabi-speech/config.json`、provider registry、FenneNote 模型缓存、OumuQ worker、公开测试语料和基准 JSON | 任意 HTTP 客户端经 RabiLink Relay 或本机回环 API；WebGUI 实时页读取规范化状态，文档读取目标测试机公开 HTML | 插件启动后实时页即可探测；公网调用还要求全局 Relay 显式开启语音中转；模型报告只在显式运行 benchmark skill / script 后更新 | 本机生成音频或转写；基准生成 WAV、JSON、CSV、HTML；Relay 不进入 Agent、人格、路由或消息日志 | `/v1/audio/speech`、`/v1/audio/transcriptions`、DashScope 风格别名、`/#/speech`、`/#/docs` 的语音栏目 | `plugin-adapters/rabi-speech/`、`src/manager/speechServiceStatus.ts`、`ribiwebgui/src/pages/SpeechServicePage.vue`、`skills/benchmark-rabispeech-models/` | `docs/rabispeech-plugin.md` |
