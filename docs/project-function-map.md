...
| Outbox / Send | 接收明确发送请求，校验 Route、渠道、目标参数、内容和策略后发送 | 让处理端绕过 RabiRoute 写平台，或从来源上下文猜测目标 | `src/agentSend.ts`、`src/outbox.ts` |
...
| 人格声纹关系 | 已有，含 WebGUI 人工归类 | 人格 `voice/voice-identities.jsonl` 追加事件；身份键为 `sourceHostId + voiceprintId` | 当前接收语音的绑定人格 | 人格根据自身会话、记忆和关系确认或修正时；用户明确询问全天/区间说话人归类时，AgentPacket 只为本次任务注入查询和修正合同；人格页进入/切换/人工操作或收到语音、关系、同步事件时查询一次 | 保存人格自己的称呼、关系、可选 `isUser`、别名和说明；相同更新幂等，修正/删除追加事件或 tombstone；主机、RabiSpeech、Route 不写人格结论；只读视图按时间和 `user/other/unknown/conflict` 联结会话证据，并从完整筛选结果派生分类时长、覆盖率和未解决声纹汇总；`includeDetails=false` 不向 WebGUI 返回转写正文；“标记下一段”只用下一次语音事件和本次开始时间高亮候选，不自动认人；多 PC 冲突显式显示并可再次确认收敛；普通消息不注入提示，不轮询覆盖率 | WebGUI 人格页、`GET/PUT /api/roles/:roleId/voice-identities`、`GET /api/roles/:roleId/voice-transcripts`、AgentPacket `voiceIdentitiesPath` | `src/personaVoiceIdentities.ts`、`src/personaVoiceTranscriptView.ts`、`src/manager/personaVoiceTranscriptRoutes.ts`、`ribiwebgui/src/persona/personaVoiceIdentityClient.ts`、`ribiwebgui/src/persona/personaVoiceConfirmation.ts`、`ribiwebgui/src/pages/PersonaTemplatePage.vue`、`src/routing/agentCapabilityHints.ts` | `docs/rabispeech-plugin.md`、`docs/user-guide/personas-and-rules.md` |
| 人格身份关系 | 已有，追加事件、同步冲突保护与可审阅的对话情境影子记录 | 人格 `identity-relations/events.jsonl` 和 `conversation/situations/*.json`；账号键为 `platform + endpointIdentityNamespace + senderStableId`，并关联参与者和关系卡 | 当前投递消息绑定的人格 | 消息端提供稳定账号命名空间时精确查询；人格页可查看、新建和修正参与者、账号映射与关系卡，并回看最近不含聊天正文的情境判断；人格根据可核对证据显式确认、纠正或收敛同步分支 | 昵称、群权限、Route、关键词和一次发言都不能自动认人；候选和冲突记录不能支持称呼、授权或项目归属；适用于当前会话的项目关系只进入 AgentPacket 和影子记录，允许参与讨论但不能据此管理项目计划、任务或长期记忆；多 PC JSONL 并集合并保留并发头，冲突显示候选记录并停止自动确认，完整修正替代所有当前头 | 人格页、`GET/PUT /api/roles/:roleId/identity-relations`、`GET /api/roles/:roleId/conversation-situations`、AgentPacket `[身份关系]` 与 `[对话情境（影子判断）]`、`semanticConflicts` | `src/identityRelations.ts`、`src/routing/identityContext.ts`、`src/routing/conversationSituation.ts`、`src/conversationSituationStore.ts`、`src/manager/controlPlaneRoutes.ts`、`ribiwebgui/src/components/PersonaIdentityRelationsCard.vue` | `docs/agent-context-injection.md`、`docs/plan-and-memory-model.md`、`docs/persona-data-sync.md` |
...
| Heartbeat | 已验证 | `notificationRules[].schedules`、`heartbeatSkipWhenAgentBusy`、`messageProcessingAgents.codex`、heartbeat config | forwarding、AgentPacket、Codex active 状态、消息处理 Agent 池 | 定时器触发时 | 写 heartbeat 日志；消息处理 Agent 模式开启时立即投给独立消息处理任务且不注入聊天历史；未开启时才可在主人格会话工作中记为 `skipped/agent_busy` | route 启用 heartbeat；消息处理 Agent 模式关闭时可勾选忙碌跳过 | `src/adapters/heartbeatAdapter.ts`、`src/scheduling/heartbeatSchedules.ts`、`src/forwarding.ts`、`src/messageAgentPool.ts`、`src/codexRuntime.ts` | `docs/configuration.md` |
...
| AgentPacket | 已有 | RouteDecision、role paths、logs、role knowledge、Route 的计划管理秘书绑定 | Agent adapter | 命中规则后 | 会触发 roleKnowledgeSnapshot，可能刷新记忆 viewedAt；向主任务注入持久秘书槽的完整 ID、名称、workspace 和控制面边界，明确秘书不得替代计划业务 `taskBinding`，并声明同计划单 writer、不同计划并行、active cycle 不得形成全局屏障；只有显式 memory-consolidation 触发才评估并创建待整理 run | 代码内部；preview 仍未实现 | `src/routing/agentPacket.ts`、`src/roleKnowledge.ts` | `docs/agent-context-injection.md` |
...
| Agent Codex 任务桥与必回复请求 | 已有；五分钟提醒仍需本机 Hook 纵向验收 | Desktop 任务状态、已配置的 `codexCwd`、`data/.runtime/agent-requests.json` | 主人格、计划 Agent、计划秘书、消息处理 Agent、Codex Desktop | Agent 调用 `/api/agent/threads`、目标轮次触发 `Stop`，或绕过投递触发 `PreToolUse` 时 | 查询/读取/解析 Desktop 任务，受控创建或改名空任务，并把实际消息交给精确 Desktop owner；Agent 互投必须声明是否要求回复，可正式回传结果并在未回复轮次结束五分钟后提醒；Route 开关可拒绝绕过 Rabi 的持久任务工具 | `POST /api/agent/threads`、`GET /api/agent/requests` | `src/agentThreads.ts`、`src/agentRequests/`、`src/manager/agentCommunicationHookPolicy.ts`、`src/manager/controlPlaneRoutes.ts` | `docs/rabi-agent-interfaces.md` |
| 消息处理看板与发送需求 | 实验支持，状态机、接口和 WebGUI 已构建测试 | `data/.runtime/message-processing-board.json`、消息组来源、结构化转交、计划变动和 Outbox 回执 | 消息处理 Agent、heartbeat、WebGUI | 消息组进入处理、Agent 转交/提交判断、计划统一写入、Outbox 返回结果时 | 直接 @、直接回复、私聊和计划进展生成必须处理项；普通群讨论保留 Agent 判断；显示处理任务、转交、不回复原因、发送失败、超时和空闲无结果；计划通知通过写入事件触发，不扫描目录 | `GET /api/message-processing/board`、`POST /api/message-processing/requirements/:id/outcome`、`POST /api/agent/threads`、`POST /api/agent/send` | `src/messageProcessing/board.ts`、`src/messageProcessing/persistence.ts`、`src/messageProcessing/managerClient.ts`、`src/messageAgentPool.ts`、`src/manager/controlPlaneRoutes.ts`、`ribiwebgui/src/components/MessageProcessingBoard.vue` | `docs/group-message-batching-and-triage-plan.md`、`docs/rabi-agent-interfaces.md` |
...
| Outbox / Send | 已有 | 必填稳定 `deliveryId`、精确 `routeId`、`channel`、渠道专用 `params` 和 `payload` 的明确发送请求 | QQ / WeCom / RabiLink / speech / role panel 等发送出口 | Agent、定时器或规划器调用 `/api/agent/send` 时 | 可能写 draft、阻止、外发、写发送日志；发送前持久化 reservation，同 ID 同 payload 只执行一次，超时/重启先查询 `/api/agent/send/receipts/:deliveryId`，uncertain 不自动重发；来源上下文不参与目标推断；NapCat 只有显式 `replyToMessageId` 才引用消息，本地群文件校验 `allowedFileRoots` 后走 `upload_group_file`；RabiLink 必须明确目标设备 | `POST /api/agent/send`、`GET /api/agent/send/receipts/:deliveryId` | `src/agentSend.ts`、`src/manager/agentSendIdempotency.ts`、`src/outbox.ts`、`src/napcat.ts` | `docs/rabi-agent-interfaces.md` |
...
| 计划 | 已有 | `data/roles/<RoleId>/plans` 与私有 `attachments/<planId>/` | roleKnowledgeSnapshot、Agent 接口、WebGUI 与托盘只读页 | AgentPacket 构造或 API 调用时 | 可通过 API 创建 / 更新五种顶层生命周期状态与受管附件；Manager 根据结构化当前步骤、审批合同和权威等待字段统一派生“正在执行 / 等待 QA 验收 / 待统一打包 / 待环境、素材、资料或外部回执 / 待审批 / 暂停 / 完成”等展示阶段、`counts.stages` 汇总、色板与顺序。实施/打包/QA 四步只适用于代码、Prefab、资源、配置等会产生项目内容变动的计划；调查、设计评审、运营、资料收集、外部依赖与控制面维护保持真实流程。WebGUI 与 Qt 只透传 DTO，缺失 `presentation` 时显示中性未知状态，不建立第二套分类器 | `/api/roles/:roleId/plans`、`GET /api/roles/:roleId/plans/:planId/attachments/:attachmentId` | `src/roleKnowledge.ts`、`src/planAttachments.ts`、`src/roleKnowledgePresentation.ts`、`src/roleKnowledgePagination.ts`、`src/manager/planAttachmentRoutes.ts`、`src/manager/controlPlaneRoutes.ts` | `docs/plan-and-memory-model.md` |
| 计划审批意见 | 已有 | `data/roles/<RoleId>/plans/feedback/*.jsonl`、私有 `attachments/<feedbackId>/` 与当前计划受管附件 | 绑定业务任务、人格 Agent、WebGUI、Qt 托盘 | 用户从计划页提交文字/附件、在输入框用 `@` 引用计划附件，或 Agent 记录 QQ/其它入口的审批时 | 校验并落盘后返回 `pending`；有完整 `taskBinding` 时经 Desktop IPC 直投原业务任务并通知人格“已自动投递”，绑定缺失或直投失败才由人格处理；不写聊天历史/最近消息，再通过 Manager 事件发布终态；不直接更新计划 | `GET/POST /api/roles/:roleId/plans/:planId/feedback` | `src/planFeedback.ts`、`src/manager/planApprovalFeedbackDelivery.ts`、`src/manager/controlPlaneRoutes.ts`、`src/routing/systemEventRules.ts` | `docs/plan-and-memory-model.md`、`docs/rabi-agent-interfaces.md` |
...
| Windows 托盘 | 已有 | 与 RibiWebGUI 同源的 Manager HTTP API、打包资源 | Windows 桌面入口 | 用户启动托盘时 | 启动 / 退出 manager，异步显示 Manager DTO；不直接读取 `data/` | `/gateways?summary=1`、`/api/roles/:roleId/*`、托盘 exe | `desktop/tray-task-window/`、`scripts/build-tray-exe.ps1` | `docs/windows-launcher-and-packaging.md` |
...
| 人格声纹关系 | `data/roles/<RoleId>/voice/voice-identities.jsonl` | 当前人格 | 按处理主机 + 声纹 ID 保存人格自己的称呼、关系和可选用户判断；追加事件可参与多电脑 JSONL 合并 |
