...
| AgentPacket | 生成处理端最终收到的消息、replyContext、上下文和接口说明 | 决定 route 是否命中、外发消息 | `src/routing/agentPacket.ts` |
...
| RabiLink Relay worker | 已有 | Relay URL / token / device id、relay tasks | RabiLink PC worker、WebGUI 远程代理 | gateway 启动且 relay 启用后 | 轮询云端、转发本地 WebGUI 请求、回传回复 | route 配置、relay scripts | `src/adapters/rabilinkRelayWorker.ts`、`scripts/rabilink-relay-server.mjs` | `docs/rabilink-relay-server.md` |
...
| Outbox / Reply | 已有 | Agent reply request、replyContext、pipeline | QQ / WeCom / RabiLink / role panel 等回传 | Agent 调用 `/api/agent/replies` 时 | 可能写 draft、阻止、外发、写回复日志 | `POST /api/agent/replies` | `src/outbox.ts` | `docs/rabi-agent-interfaces.md` |
...
| 改 Agent 收到的消息 | `src/routing/agentPacket.ts`、`docs/agent-context-injection.md` | 不要在消息端拼 prompt |
