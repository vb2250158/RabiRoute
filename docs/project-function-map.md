...
| 计划会话任务完成提醒 | 实验支持，代码与 mock 链路已测试 | 计划 `taskBinding`、Codex `Stop` 的 `session_id/turn_id/cwd/last_assistant_message`、人格 Route/gateway 精确绑定 | `Stop` Hook → Manager → 角色面板 timeline → Forwarding / AgentPacket → 人格 Route 处理会话 | 绑定执行会话完成一轮，且计划与目标 Route 均未关闭完成通知时；计划省略 `completionHook` 默认开启 | 按 `sessionId + turnId` 去重并投递最终回答提醒；不自动推进计划。workspace、人格、gateway、多计划绑定及源目标同会话冲突失败关闭；尚未完成双真实 Desktop 任务验收 | 计划 POST/PATCH 的 `taskBinding.completionHook` 与 Route `codexHooks.planTaskCompletionEnabled` | `src/manager/codexHookContext.ts`、`src/manager/planTaskCompletionDelivery.ts`、`src/manager/controlPlaneRoutes.ts`、`plugins/rabi-codex-context/` | `docs/plan-and-memory-model.md`、`docs/rabi-codex-context-plugin.md` |
...
| 计划 | 已有 | `data/roles/<RoleId>/plans` | roleKnowledgeSnapshot、Agent 接口、WebGUI 与托盘只读页 | AgentPacket 构造或 API 调用时 | 可通过 API 创建 / 更新计划；Manager API 附加显示状态并按状态、更新时间排序 | `/api/roles/:roleId/plans` | `src/roleKnowledge.ts`、`src/roleKnowledgePresentation.ts`、`src/manager/roleKnowledgeRoute.ts`、`src/manager/controlPlaneRoutes.ts` | `docs/plan-and-memory-model.md` |
| 计划审批意见 | 已有 | `data/roles/<RoleId>/plans/feedback/*.jsonl` | Agent、WebGUI、Qt 托盘 | 用户从计划页提交，或 Agent 记录 QQ/其它入口的审批时 | 追加关联 `planId/stepId` 的审计记录；可选复用角色面板链通知 Agent；不直接更新计划 | `GET/POST /api/roles/:roleId/plans/:planId/feedback` | `src/planFeedback.ts`、`src/manager/controlPlaneRoutes.ts` | `docs/plan-and-memory-model.md`、`docs/rabi-agent-interfaces.md` |
