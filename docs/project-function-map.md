...
| Outbox / Reply | 已有 | Agent reply request、replyContext、pipeline、`proactive`；可选稳定 `deliveryId` 的持久 reservation/回执 | QQ / WeCom / RabiLink / role panel 等回传 | Agent、定时器或规划器调用 `/api/agent/replies` 时 | 可能写 draft、阻止、外发、写回复日志；带 `deliveryId` 时在发送前持久化 reservation，同 ID 同 payload 只执行一次，超时/重启先查询 `/api/agent/replies/receipts/:deliveryId`，uncertain 不自动重发；NapCat 群聊按 `messageId + replyToSource` 自动绑定源消息，本地群文件校验 `allowedFileRoots` 后走 `upload_group_file`；RabiLink 文本可无前置 task 主动入队 | `POST /api/agent/replies`、`GET /api/agent/replies/receipts/:deliveryId` | `src/manager/agentReplyIdempotency.ts`、`src/outbox.ts`、`src/napcat.ts` | `docs/rabi-agent-interfaces.md` |
