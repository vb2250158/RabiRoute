...
Normal mode remains the default: approve, provide information and submit directly inside plan cards. Use the **Focus** switch to the left of the search field to change layouts. Turn it on to enter Focus view, and off—or select **Close focus view**—to return to the original page. Both modes share the same draft and submission logic; switching never creates or submits another response.

Focus view brings together the current persona's pending approvals and information requests without making you search a long list for an input field. Its queue is loaded independently of the normal page's search and filters. Searching within Focus view filters pending items without changing the current answer or draft. Loading remains visible until the query completes; the displayed count does not represent every persona or a complete queue while loading. The refresh icon on the right of the search field reloads data instead of taking up a separate button. Refreshing or completing an item never automatically changes the selection.

On desktop, select an item on the left, read its question, proposal and necessary context above, and answer in the fixed bottom area. Choices, additional text, attachments and submission actions share that area; expand it for longer forms. Narrow screens prioritize the current item while retaining queue and exit controls. Incomplete approval materials still explain what is missing and prevent approval; Focus mode does not relax eligibility.

Switching items or closing and reopening Focus mode within the same persona's page preserves unsubmitted drafts. Drafts are not saved feedback; handle unfinished input before reloading the browser or switching personas. Changed questions or proposals require renewed confirmation rather than reusing old choices. After submission, the current item stays open with its save, delivery or failure result. It never automatically advances to the next item; choose another item after reviewing the result.
