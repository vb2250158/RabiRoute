...
The **Plans & Memory** page never reads `data/` directly or reinterprets categories, ordering, status colors, or contract completeness in the browser. Manager supplies shared `Current / Plans / Recent Memory / Archived` membership plus `status / tone / sortBucket / palette` and approval details used by both RibiWebGUI and the Qt tray. Except for paused plans, approval-ready cards sort first, incomplete contracts next, followed by `Blocked → Awaiting QA → In progress → Awaiting package → Not started → Completed → Archived` and update time. Blue `Awaiting package` remains current work but sorts below all ordinary executable running plans. Paused plans appear only under Plans with the shared slate palette and always sort last. RibiWebGUI shows a plan's non-duplicate `focus`, and search matches step details, current actions, waiting/blocking notes, approval contracts, attachment metadata, memory bodies, and source summaries. Multiple plans remain independently framed, with a sticky external directory that follows the current tab and search result. Running plans outside approval expose whole-plan guidance at the top of expanded details; approval plans instead keep the approval contract inside the Manager-identified step card. User feedback, Agent replies, and system records remain ordered from oldest to newest. Incomplete details are labeled `Approval information incomplete / approval disabled`; approval input, attachments, and submission stay disabled until `ready/enabled=true`.

A step waiting for approval, plan confirmation, or authorization carries a complete `approvalRequest`. Only a complete actionable contract with `responseStatus=pending` makes Manager show `Blocked` and enable the approval entry, so every blocked item is approvable. An incomplete contract disables formal approval but leaves the plan `In progress` for Agent investigation and repair. QA waits, missing inputs, execution failures, and external artifacts are not blocked. `Awaiting QA` still requires the current structured `qa-* / verify-*` step, while a qualifying package/build step becomes blue `Awaiting package`.
