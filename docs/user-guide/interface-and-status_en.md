...
The **Plans & Memory** page never reads `data/` directly or derives a second status. `plan.status` is the only status source and accepts exactly Analyzing, Awaiting approval, Executing, Awaiting package, Awaiting QA, Awaiting discussion, Paused, Completed, or Closed. Manager returns that same value with its palette, integer sort level, and counts; WebGUI and the Qt tray display it directly. Steps, wait reasons, and approval metadata never replace it.
...
A complete actionable `approvalRequest` with `responseStatus=pending` requires the Agent to store Awaiting approval; an incomplete contract remains Analyzing. Approval or explicit direct authorization moves the stored state to Executing. Discussion, pause, package, and QA use their exact stored states. Normal completion uses Completed; cancellation, invalidation, or replacement uses Closed. Archival is separate, preserves the plan status, and removes the plan from keyword recall.

Before approval, investigation, evidence gathering, solution work, available CLI checks, retries, sending, and coordination show cyan `Analyzing`. After approval or explicit direct user authorization, implementation and development validation show green `Executing`. A completed delivery gate that only lacks package identity or inclusion proof is blue `Awaiting package`; proven inclusion is purple `Awaiting QA`. Test infrastructure, assets, documents, owner replies, renewed authorization, or external receipts produce gray `Paused` only when no safe action remains, and their exact reason stays outside the status badge.
