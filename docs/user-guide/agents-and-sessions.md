...
一条 Route 可以保留多个不同的 Agent 配置，但命中规则的消息只交给一个主控 Agent。在“Agent 端”列表上方选择主控；其他 Agent 的项目、任务和服务参数会保留，但不会收到默认投递。

旧 Route 没有保存主控时，系统使用 Agent 列表中的第一项。删除当前主控后，界面会自动选择仍存在的第一项；如果没有 Agent，消息只保留匹配和消息包记录，不会投递。

主控投递失败时会记录失败，不会自动改投其他 Agent。这样可以避免两个处理端同时回复或重复执行外部动作。
...
如果固定目标任务已归档，RabiRoute 会阻止投递和自动新建；请先在 Desktop 恢复它，或从下拉重新选择。任务已删除、换账号或移动项目时，重新选择并保存。
