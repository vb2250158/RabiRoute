...
一条 Route 可以同时保留本机 Codex、DSH 和多个远端 Codex 等配置，但命中规则的消息只交给一个主控 Agent。在“Agent 端”列表上方选择具体的本机或远端 Agent；其他 Agent 的项目、任务和服务参数会保留，但不会收到默认投递。添加远端不会替换同类型的本机卡片，也不会自动改变主控。

旧 Route 没有保存主控时，系统沿用原来的第一项；旧版已绑定远端的主控仍保持原目标。删除当前主控后，需要明确选择新的主控，不会自动切换到本机或其他 Agent。没有主控时不执行 Agent 投递。

主控投递失败时会记录失败，不会自动改投其他 Agent。这样可以避免两个处理端同时回复或重复执行外部动作。

主控按消息路线分别设置，不是整个应用共用一个。排查消息为何进入某个 Agent 时，先核对消息里的“消息路线 ID”，再查看同一路线的主控。启用独立消息处理 Agent 后，消息可能先交给该路线主控所属处理端的消息处理会话，不代表切换了主控或可以改投另一个处理端。
...
如果固定目标任务已归档，RabiRoute 会在下一次真实投递时创建新任务并自动更新绑定，不会把消息送进归档记录，也不会复用其它同名任务。任务已删除、换账号或移动项目时，重新选择并保存。
