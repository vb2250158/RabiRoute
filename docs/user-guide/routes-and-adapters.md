...
| 米家 / Xiaomi Home | 实验 | Home Assistant 设备状态、有人移动事件和摄像头事件录像 | Home Assistant、环境变量中的访问 token、真实设备验收 |
...
米家的 Home Assistant 地址、事件监听、设备控制和录像参数都在“米家 / Xiaomi Home”消息端内配置，不在本机全局“设置”页重复出现。已授权的局域网 WebGUI 可以维护这些配置；设备目录、控制动作和录像内容接口仍只允许 Manager 本机访问。
