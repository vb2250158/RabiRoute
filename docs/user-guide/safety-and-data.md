...
当前没有通用、持久化、可在 WebGUI 中逐条审批外部动作的 Action Queue。计划页保存审批意见后显示“已审批”，仅确认投递后转为“分析中”；这只记录意见提交与投递进展，不代表所有题目或选项获批，不批准 Outbox 外发，也不自动实施。计划引导不改变状态。`draft` 是结果和审计状态，不是一个等待处理的完整审批中心。
