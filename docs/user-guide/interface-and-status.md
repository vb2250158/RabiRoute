...
Agent 按当前人格 `planWorkflow.roles` 选择状态 key。默认模板中，完整 `approvalRequest` 等待回执显示“待审批”，合同缺项显示“分析中”，批准或用户明确直接授权后的实施与开发验证显示“执行中”；讨论、暂停、包体、QA、正常结束和取消分别显示“待讨论”“暂停”“等待打包”“等待 QA”“完成”和“关闭”。人格可以调整 key 与显示名称，界面始终显示配置 label。归档是独立变量，不会把终态改成另一个状态。
