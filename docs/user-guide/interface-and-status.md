...
进入“计划与记忆”后，页面先请求首批 8 条轻量计划摘要，立即显示标题、类型、状态和排序；即使不滚动，也会使用更大的后台分页持续补齐后续摘要。左侧目录保留全部已返回标题，右侧正文只先挂载 8 张卡片并随滚动分批追加；点击尚未挂载的目录项时会从目标计划开始创建一个有界的向后窗口，不会一次创建前面的几十张正文卡，也不会在后续加载时把旧卡片插回当前阅读位置上方。正文、步骤、审批与附件元数据只在卡片真正接近视口时按需加载，每轮只提升最近的 2 张且最多 2 个并发请求；下一批摘要只让出一个渲染帧，不再等待正文详情加载完成，因此目录补齐不会被重卡片或附件拖慢。只有已经发出详情请求的卡片显示加载动画，尚未进入视口的卡片使用紧凑提示，不再为全部计划同时创建大型骨架；离屏计划卡由浏览器跳过布局和绘制。较大的记忆列表在首批计划返回后再加载，图片和视频在媒体就绪前显示浅色“附件加载中”占位，不会以纯黑块占住页面。
...
“计划与记忆”页不直接读取 `data/`，也不在浏览器里重新解释分类、排序、状态颜色或合同完整性。Manager 返回计划的 `status / tone / sortBucket / palette`、`counts.stages` 和审批状态。没有进入审批步骤的进行中计划在展开详情后提供“计划引导”：输入针对整个计划，不绑定某一步；Agent 会根据引导继续推进，并在需要时调整尚未开始的步骤。进入审批状态后只显示对应步骤里的审批入口，不同时开放计划引导。除暂停外，完整待审批合同优先、待补合同随后，再按“待审批 → 等待 QA 验收 → 正在执行 → 待环境/待素材/待资料/待外部回执 → 待统一打包 → 未开始 → 已完成 → 已归档”和 `updatedAt` 排序。外部等待使用琥珀色，蓝色“待统一打包”仍属于当前进行中计划，但排在普通执行与其它外部等待之后。暂停计划只显示在“计划”，使用灰蓝色状态且绝对排在最后。
