...
默认使用普通模式，仍可在计划卡片中直接审批、补充信息和提交。搜索框左侧的“专注”滑动开关用于切换布局；开启进入专注视图，关闭或点击“关闭专注视图”返回原页面。两种模式共用同一份草稿和提交逻辑，切换不会另建或重复提交意见。

专注视图集中处理当前人格的待审批和待补充信息事项，不用在长列表中寻找输入框。专注列表独立读取待处理计划，不受原页面搜索和筛选影响；专注视图中的搜索框只筛选待处理事项，不改变正在回答的事项或草稿。读取尚未完成时显示加载状态，已显示的数量不代表全部人格或全部待办。搜索框右侧的刷新图标用于重新读取数据，不再单独占用一个刷新按钮；刷新或完成事项都不会自动切换当前选择。

桌面端左侧选择事项，右侧上方阅读问题、方案和必要背景，底部固定回答区。选项、补充文字、附件和提交按钮都在同一区域；问题较多时可展开回答区。窄屏优先显示当前事项，并保留切换列表和退出入口。审批资料缺失时仍会显示原因并禁止审批，不因进入专注模式而放宽条件。

在同一人格的页面中切换事项或关闭后重新打开专注模式，会保留未提交草稿。草稿不是已保存的意见；刷新浏览器或切换人格前请先处理未提交内容。问题或方案发生变化时，需要重新确认，不能直接沿用旧选择。提交后停留在当前事项并显示保存、投递或失败结果，不会自动切到下一项；确认结果后再选择下一项。
