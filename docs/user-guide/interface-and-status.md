...
| 计划与记忆 | 查看当前人格的计划、近期记忆、沉淀记忆和审批记录 | 搜索、展开步骤、为需审批步骤提交建议、刷新 Manager 数据 |

计划卡片的状态、排序和“是否可填写审批建议”都由 Manager 返回。提交审批建议只会追加与当前计划/步骤关联的记录并通知 Agent，不会直接把计划标为通过、完成或进入下一步。记录成功但 Agent 通知失败时，页面会保留草稿供重试。
...
“计划与记忆”页不直接读取 `data/`，也不在浏览器里重新解释排序。Manager 返回计划的显示状态，并按“阻塞中 → 待QA测试 → 进行中 → 未开始 → 已完成 → 已归档”排序；同一状态内按 `updatedAt` 从新到旧。近期记忆和沉淀记忆同样由 Manager 按更新时间返回。
