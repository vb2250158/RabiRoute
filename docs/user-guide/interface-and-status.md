...
计划卡片的分类、状态、状态色板、排序和审批说明都由 Manager 返回；RibiWebGUI 与 Qt 托盘因此使用相同的“当前 / 计划 / 近期记忆 / 已归档”分类、卡片强调色和状态徽标颜色。“当前”同时展示进行中计划与近期记忆，“已归档”同时展示归档计划与沉淀记忆。审批卡片会列出已有的批准事项、原因、文件、完整命令、配置/数据/外部变更、验证、回退和明确排除范围。信息不完整时页面列出建议 Agent 补充的项目，但审批输入仍然可用。点击提交后，意见落盘即结束按钮加载，Agent 通知在后台继续；终态只刷新当前卡片，失败时恢复原意见供重试，不再整页重载计划与记忆。
...
长计划列表会延迟绘制屏幕外卡片；展开详情不执行高度动画，审批输入也不自动反复增高。因此滚动、展开和输入不会因为其它卡片的复杂内容持续触发整页布局。
