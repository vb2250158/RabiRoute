...
等待审批、方案确认或授权时，当前步骤必须显式写入 `isBlocked=true`，并在 `blockedBy` 写清谁要批准什么；计划卡显示“阻塞中”，秘书继续追问，但业务任务不反复续投实施。待 QA、资料或外部产物不自动变成阻塞。“待QA测试”只认当前结构化 `qa-* / verify-*` 步骤；实施步骤正文即使提到 QA 门禁或尚未通知 QA，也保持“进行中”。审批资料缺少必要栏目时，卡片直接显示“审批资料不完整/禁止审批”。
...
审批意见落盘后，Agent 通知会在后台继续。通知期间输入框、粘贴图片和新附件仍可编辑，只暂时禁止再次提交，并在输入区附近明确说明原因。当前步骤没有有效审批入口、正在保存或没有可投递 Route 时，同一状态条也会说明具体原因和恢复方式，不再只显示一组置灰控件。
