...
“计划与记忆”页不直接读取 `data/`，也不在浏览器里重新解释分类、排序、状态颜色或合同完整性。Manager 返回计划的 `status / tone / sortBucket / palette` 和审批状态。没有进入审批步骤的进行中计划在展开详情后提供“计划引导”：输入针对整个计划，不绑定某一步；Agent 会根据引导继续推进，并在需要时调整尚未开始的步骤。进入审批状态后只显示对应步骤里的审批入口，不同时开放计划引导。除暂停外，可直接审批的计划排在最前，需要 Agent 补合同的计划紧随其后，再按“阻塞中 → 待QA测试 → 进行中 → 等待打包 → 未开始 → 已完成 → 已归档”和 `updatedAt` 排序。蓝色“等待打包”仍属于当前进行中计划，但排在所有普通可执行进行中计划之后。暂停计划只显示在“计划”，使用灰蓝色状态且绝对排在最后。

等待审批、方案确认或授权时，当前步骤写入完整的 `approvalRequest`。只有合同完整、可提交且 `responseStatus=pending` 时，Manager 才显示“阻塞中”，并同时开放审批入口；因此界面里的每个阻塞项都可以审批。审批资料缺项时卡片显示“审批资料不完整/禁止审批”，但计划保持“进行中”，由 Agent 继续调查和补齐。待 QA、缺资料、执行失败或外部产物都不显示阻塞。“待QA测试”只认当前结构化 `qa-* / verify-*` 步骤；符合条件的 package/build 步骤显示蓝色“等待打包”。
