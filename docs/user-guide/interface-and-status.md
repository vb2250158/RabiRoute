...
“计划与记忆”页不直接读取 `data/`，也不在浏览器里重新解释分类、排序、状态颜色或合同完整性。Manager 返回计划的 `status / tone / sortBucket / palette`、`counts.stages` 和审批状态。没有进入审批步骤的进行中计划在展开详情后提供“计划引导”：输入针对整个计划，不绑定某一步；Agent 会根据引导继续推进，并在需要时调整尚未开始的步骤。进入审批状态后只显示对应步骤里的审批入口，不同时开放计划引导。除暂停外，完整待审批合同优先、待补合同随后，再按“待审批 → 待 QA → 正在执行 → 待环境/待素材/待资料/待外部回执 → 待统一打包 → 未开始 → 已完成 → 已归档”和 `updatedAt` 排序。外部等待使用琥珀色，蓝色“待统一打包”仍属于当前进行中计划，但排在普通执行与其它外部等待之后。暂停计划只显示在“计划”，使用灰蓝色状态且绝对排在最后。

等待审批、方案确认或授权时，当前步骤写入完整的 `approvalRequest`。只有合同完整、可提交且 `responseStatus=pending` 时，Manager 才显示“待审批”，并同时开放审批入口。审批资料缺项时卡片显示“审批资料不完整/禁止审批”，计划仍显示“正在执行”，由 Agent 继续调查和补齐。“待 QA”只认当前结构化 `qa-* / verify-*` 步骤；权威 `waitingFor` 可派生“待环境 / 待素材 / 待资料 / 待外部回执”；前序工作完成且符合条件的 package/build 当前步骤显示蓝色“待统一打包”。这些都是展示阶段，不改变计划的五种顶层生命周期状态。
