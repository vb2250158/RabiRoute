...
计划头部显示更新时间和标记状态；当前步骤标签与标题同行。步骤列表默认折叠正文，点击标题行或用键盘展开；当前步骤摘要保持可见，审批操作独立于正文折叠。两处正文左右各缩进 10px。文件变动独立折叠，按记录时间分组，分别显示变动类型、可换行路径与说明，使用与正文一致的 11px 字号；SHA-256 默认折叠，不再用错误色突出普通记录。

## 操作记录的来源身份

计划版本记录显示“操作者（渠道）— 操作 — 日期”，仍可展开查看状态与审批详情。新记录在同一存储事务中保存 `actor`（类别、Agent 宿主类型、完整会话 ID、名称快照及渠道）；计划当前绑定的任务不是操作者来源。旧记录缺少身份时显示“来源未知”，不回填猜测。

点击 Agent 名称时，Manager 按历史记录中保存的宿主和完整 ID 核对原会话再打开；不使用当前 taskBinding，也不创建或替换会话。不存在、归档、宿主不可用或不支持打开时显示错误。历史名称保留写入时快照，改名不改写历史。

Agent 的显式 `messageSource` 经 owner 查询核对仅用于显示归因，不是不可伪造的认证身份，不能授权或替代审批。未提供来源的调用保持未知；浏览器用户入口与内部系统操作应分别记录用户和系统类别。`actor.agentType` 表示 `dsh`、`codex` 等宿主，不是 Agent 职责标签。
...
`taskBinding` 是可选的“计划 ↔ 执行会话”精确绑定。`agentType` 决定会话所在的处理端：`codex` 使用 Codex Desktop 任务，`dsh` 使用 DSH 会话。`sessionId` 是必填的完整 ID；`sessionTitle` 只用于展示；`workspace` 决定该计划每次投递的执行目录，不要求等于 Codex 任务保存的默认 cwd。DSH 绑定仍按其 workspace 所有权校验，并可保存 `baseUrl` 指向实际 apiproxy。`completionHook.enabled=true` 仅适用于 Codex 完成回传：Manager 在该任务完成一轮后处理官方最终回答；启用并绑定计划秘书时直接投给 `secretaryBinding`，不写主人格角色面板；没有可用秘书时才回退到同人格 Route。`gatewayId` 用于多 Route 消歧。该提醒按 `sessionId + turnId` 去重，只记录阶段完成事实，不自动推进步骤、修改计划状态或写入记忆；计划状态为 `planWorkflow.roles.paused` 指向的 key 时不投递完成提醒，避免暂停期间重新驱动绑定任务。完成回传会遍历全部角色查找绑定计划，尚未写过计划、因此还没有已发布计划目录的角色属于合法冷态：跳过该角色并在首次读取时自动创建空的 `plans/active`、`plans/archive`，不视为失败，也不影响其它角色的正常投递。
...
当前步骤直接显示计划的真实状态。待审批、待补充信息和分析中的步骤不显示为“正在执行”，审批等待也不显示为技术故障式的“已阻塞”。已完成的步骤显示“已完成”。
