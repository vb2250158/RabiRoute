...
`src/agentSend.ts` validates the explicit send contract at the Manager HTTP boundary: stable `deliveryId`, exact `routeId`, `channel`, channel-specific `params`, and `payload` are required, and source `replyContext` never selects the destination. `src/manager/agentSendIdempotency.ts` then persists a reservation under runtime `data/agent-send-idempotency/` before allowing the single winning request into Outbox. Concurrent calls and restart recovery for the same ID/payload only read the original result; changed payloads conflict, while `reserved/sending/uncertain` states fail closed. `GET /api/agent/send/receipts/:deliveryId` exposes only that durable receipt. It neither guesses external-platform success nor auto-replays; QQ and similar channels still require real platform readback by `sentMessageId`.
