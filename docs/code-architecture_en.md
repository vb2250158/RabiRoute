...
`src/agentSend.ts` validates the explicit send contract at the Manager HTTP boundary: stable `deliveryId`, caller-declared `sender.agentType + sender.sessionId`, exact `routeId`, `channel`, channel-specific `params`, and `payload` are required, and source `replyContext` never selects the destination. Message-processing replies also require `tracking.requirementId + tracking.sendContextReviewToken` bound to a current context review. `src/manager/agentSendIdempotency.ts` then persists a reservation under runtime `data/agent-send-idempotency/` before allowing the single winning request into Outbox. Concurrent calls and restart recovery for the same ID/request only read the original result; a changed sender or any other changed field conflicts, while `reserved/sending/uncertain` states fail closed. `GET /api/agent/send/receipts/:deliveryId` exposes the durable receipt, while `GET /api/agent/send/traces?channel=...&sentMessageId=...` can trace a platform receipt back to the calling Agent session. Neither endpoint guesses external-platform success or auto-replays; QQ and similar channels still require real platform readback by `sentMessageId`.
