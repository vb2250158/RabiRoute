...
`src/planFeedback.ts` owns approval-feedback JSONL associated with `planId/stepId`, delivery-state collapsing for one `feedbackId`, and read summaries. Manager's `/api/roles/:roleId/plans/:planId/feedback` is the only write boundary: UI submissions persist and return `pending` immediately, role-panel notification continues in the background, and terminal state is emitted as `plan_feedback_changed`. Background work is deduplicated by feedback ID and bounded by a 45-second delivery deadline. Agents record QQ approvals or handling results as `record_only`. The module never modifies plan JSON; only a later explicit plan PATCH can advance steps or status.
...
`ribiwebgui/` is the Vue/Vuetify control surface for configuration, scans, status, logs, documentation, and Plans & Memory. `ribiwebgui/src/pages/RoleKnowledgePage.vue` reads `/api/roles/:roleId/plans` and `/memory`; plan content stays read-only, approval cards render Manager-provided details and missing-field guidance, and both `incomplete` and `ready` can append user feedback. After persistence, the page updates only the local card and listens for `plan_feedback_changed` to read one plan summary instead of reloading all knowledge. Offscreen cards use deferred rendering and details expand without height animation to avoid repeated long-list layout. The page is a client of Manager APIs, not the configuration source of truth.
