...
When a Route configures persistent plan-assistant tasks, AgentPacket adds each slot's full task ID, visible name, workspace, and one-unfinished-plan assignment rule. An assistant may create temporary child agents, but long-lived ownership, aggregation, plan updates, and reporting back to the main task do not transfer.
...
`codexPlanAssistantSessions` stores 1–8 persistent Desktop-task slots per Route. The main task binds one unfinished plan to one slot. That assistant may create temporary child agents for parallel work, but remains responsible for aggregation, plan updates, and reporting back. This layer remains experimental until real Desktop multi-task visibility and tool-owner acceptance are complete.
...
`src/roleKnowledge.ts` defines five top-level plan states, the current step's explicit `isBlocked` fact, and the step-level `approvalRequest` execution contract. `waitingFor` names an inquiry target and does not automatically mean blocked; approval capability is also independent from blocked state. A paused plan may preserve its one in-progress step and `currentStepId` as the resume position. Contracts remain compatible with plan reads and writes; missing sections do not reject the whole plan update. `src/roleKnowledgePresentation.ts` produces only the read-only Manager presentation DTO. It derives `Blocked` only from `isBlocked=true` plus `blockedBy`, separately derives `Awaiting QA`, owns `current / plans / archived` membership and the shared status palette, lightly classifies the current human gate as `none / incomplete / ready`, returns missing fields plus the normalized contract, and orders plans by ready approval, incomplete contract, status, and update time. Paused plans derive no approval state, belong only to `plans`, and are forced to the end of the final order. It never rewrites plan files and does not participate in RouteDecision or Agent-context selection. WebGUI and Qt must consume Manager's presentation, categories, palette, contract, and list order instead of reinterpreting raw `blockedBy` text.

`src/planAttachments.ts` owns plan-level attachment count/size limits, local-path or Base64 ingestion, image/video signature checks, hashing, and materialization into the persona-private managed directory. `src/manager/planAttachmentRoutes.ts` serves only `roleId + planId + attachmentId` reads and verifies both lexical containment and realpath containment before responding; images and videos are inline, videos support one byte range, and the public plan DTO omits the local `path`. WebGUI consumes only this HTTP boundary for 16:9 image/video thumbnails, ordinary file cards, and in-page media preview, and consistently passes LAN resources through `managerResourceUrl` to apply the current-session credential. It owns neither a plan editor nor arbitrary local-path access.

`src/planFeedback.ts` owns approval-feedback JSONL associated with `planId/stepId`, delivery-state collapsing for one `feedbackId`, and read summaries. Manager's `/api/roles/:roleId/plans/:planId/feedback` is the user/client write boundary: UI submissions persist and return `pending` immediately, an independent `plan_feedback` system event continues in the background, and terminal state is emitted as `plan_feedback_changed`. `routing/systemEventRules.ts` supplies the fixed delivery rule without depending on editable persona message rules. Forwarding does not write this event to the role-panel timeline, compatibility message history, or unified conversation ledger, and AgentPacket fixes its recent-message budget at `0`. Delivery still carries a dedicated reply context; `src/outbox.ts` validates the Route persona, plan, and step before storing the Agent's normal reply as a record-only `approval_response` and emitting the same event. This path never modifies plan JSON automatically; the Agent must explicitly PATCH the plan or step first.
...
`ribiwebgui/` is the Vue/Vuetify control surface for configuration, scans, status, logs, documentation, and Plans & Memory. `ribiwebgui/src/pages/RoleKnowledgePage.vue` reads `/api/roles/:roleId/plans` and `/memory`; plan content stays read-only, a non-duplicate `focus` is shown below the title, adjacent problems use independently framed work items with filtered-result ordinals, and each card separates overview, current execution, and full steps. Each approval contract is nested in the step card identified by Manager's `presentation.approval.stepId`. Both `incomplete` and `ready` can append user feedback. After persistence, the page updates only the local card and listens for `plan_feedback_changed` to read one plan summary instead of reloading all knowledge. The directory outside the plan panel is sticky and scrolls within the viewport; browser visibility observation synchronizes the card being read into the directory highlight, smooth directory jumps lock the selected highlight until scrolling settles, the directory scrolls itself only when that item leaves its viewport, plan cards remain in normal page flow, and details expand without height animation. The page is a client of Manager APIs, not the configuration source of truth.
