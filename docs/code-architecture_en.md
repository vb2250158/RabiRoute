...
`src/roleKnowledge.ts` defines the five top-level plan states and the step-level `approvalRequest` execution contract, while `planApprovalGate()` is the single interpreter for approval preparation and pending decisions. `planIsBlocked()` is true only when the current contract is complete, actionable, and has `responseStatus=pending`. `isBlocked` is generated during read/write normalization as a legacy-client compatibility projection, and `blockedBy` is explanatory text only. Legacy non-approval blockers are downgraded to running at read time and cleaned on the next canonical write. Missing contract fields produce `preparing/incomplete` without rejecting the plan write, so the Agent continues investigation and repair. `src/planPackageWaiting.ts` still derives package waiting only from a structured package/build step, completed prior work, and an explicit package dependency. `src/roleKnowledgePresentation.ts` produces the read-only Manager DTO: the same approval gate controls both `approval.ready/enabled` and `Blocked`, guaranteeing that every blocked plan is approvable. QA, missing materials, execution failures, and external artifacts remain actionable. WebGUI and Qt consume Manager's presentation, categories, palette, contract, and list order instead of reinterpreting raw text.
...
`src/planFeedback.ts` owns plan-feedback JSONL, delivery-state collapsing for one `feedbackId`, and read summaries. `guidance` is associated only with `planId` for running plans outside approval, while `approval_suggestion` is associated with an approval step. Manager persists either type at `/api/roles/:roleId/plans/:planId/feedback`, then `src/manager/planApprovalFeedbackDelivery.ts` reuses the exact business `taskBinding`: a complete binding goes through `/api/agent/threads` and Desktop IPC to the original task, while only an incomplete binding sends full feedback to the persona Agent. Terminal state is emitted as `plan_feedback_changed`, and events do not enter the role-panel timeline or conversation ledger. The bound task handles guidance by PATCHing the whole plan and any affected not-started steps before writing a step-less `guidance_response`; approval still writes `approval_response`. Feedback never modifies plan JSON automatically.
