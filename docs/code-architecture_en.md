...
`codexPlanAssistantSessions` stores 1–8 persistent Desktop plan-management secretary slots per Route. These slots are control-plane sessions and never replace the independent business task stored in plan `taskBinding`. Secretaries maintain plans and memory, resolve and deduplicate business tasks, inspect status, consume results, and continue the bound task. They and their temporary control-plane children do not modify business files. This layer remains experimental until real Desktop multi-task acceptance is complete.
...
`src/roleKnowledge.ts` defines five top-level plan states, the current step's explicit `isBlocked` fact, and the step-level `approvalRequest` execution contract. Ordinary `waitingFor` targets do not automatically mean blocked, but a current approval, plan-confirmation, or authorization step must also set `isBlocked=true` with a concrete `blockedBy`; Manager rejects the write otherwise. A paused plan may preserve its one in-progress step and `currentStepId` as the resume position. Contracts remain compatible with plan reads and writes; missing approval sections do not reject the plan write, but they disable approval. `src/roleKnowledgePresentation.ts` produces only the read-only Manager presentation DTO. It derives `Blocked` only from `isBlocked=true` plus `blockedBy`. `Awaiting QA` is derived only when `steps/currentStepId` identifies an in-progress structured `qa-* / verify-*` step; free text in `currentStep`, `nextAction`, titles, details, or `waitingFor` is never scanned for phase keywords. The module owns `current / plans / archived` membership and the shared status palette, classifies the current human gate as `none / incomplete / ready`, returns missing fields plus the normalized contract, and orders plans by ready approval, incomplete contract, status, and update time. Paused plans derive no approval state, belong only to `plans`, and are forced to the end of the final order. It never rewrites plan files and does not participate in RouteDecision or Agent-context selection. WebGUI and Qt must consume Manager's presentation, categories, palette, contract, and list order instead of reinterpreting raw blocker or QA wording.
