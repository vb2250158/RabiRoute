...
普通 Agent 回复中，QQ / 企业微信这类聊天路由默认把 RabiRoute 当作 outbox。Agent 应把回复文本 POST 回：
...
自动发送还会经过对应消息端的 `messageAdapterPolicies`。NapCat 使用 `messageAdapterPolicies.napcat`，企业微信使用 `messageAdapterPolicies.wecom`。默认策略建议是 `inputEnabled=true`、`outputEnabled=true`，允许 Agent 通过 RabiRoute 主动发送到明确的群聊或私聊目标。
