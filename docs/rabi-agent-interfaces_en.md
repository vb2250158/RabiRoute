...
1. Discover the complete current Manager URL from Host status (installed), structured READY output (source), or explicit test injection. Bound every request to at most 12 seconds. Before writing, GET `/meta` and verify nonempty `applicationGenerationId` and `managerInstanceId` against the current Host/READY identity. Ordinary business requests require `health.live=true`, `health.requiredReady=true`, and `health.state` of either `healthy` or `degraded`. Neither `businessReady=false` nor an unrelated degraded Route blocks all requests; the corresponding Manager endpoint checks dependencies such as plan recovery and the target Route.
...
5. After the response, GET `/meta` again with a bounded timeout and check only that both identities match the pre-write values; unrelated health changes do not constitute a generation change. Missing identity fields, failed reads, or changed identities make the result uncertain: rediscover the endpoint and preserve the original request/key for authoritative readback, without automatic replay. Matching identities do not remove uncertainty already caused by a write timeout, 5xx response, or similar failure. Read back the resource to verify the actual result.

Exact `GET /meta` is a diagnostic entry point: after obtaining a valid response and verifying the current identity, return the health details without requiring `live`, `requiredReady`, or business readiness. Origin restrictions, authentication, timeouts, redirect rejection, and identity checks still apply. Do not extend this exception to other GET requests; some reads update `viewedAt`. This degraded-access and post-write identity-separation contract requires an updated client. Older plugins that still demand aggregate `healthy` status must be upgraded; this documentation update does not establish that a running plugin has been installed or verified.
...
Use `GET /api/agent/help` to discover the current Agent API operations and query one by `operationId`, method, or path. Each entry includes `contractLevel`: `baseline` means exact schema verification is incomplete, not that the API is unusable; consult the authoritative detailed contract to resolve `coverage.missing` before calling. `auditLevel: implementation-summary` indicates an implementation audit summary, not `verified`. Coverage increments `verifiedCount` only when the verified label, both exact-schema flags and an empty missing list agree. Unknown levels or inconsistent declarations enter `unverifiedCount`. baselineCount + verifiedCount + unverifiedCount equals operationCount; unknown entries never imply verification.
...
Help accepts `operationId`, `path`, and `method`, each nonempty and at most once; filters intersect. Use the exact catalog path template for `path`; `method` is case-insensitive. The handler returns 400 `AGENT_HELP_INVALID_QUERY` with repair guidance for unknown, repeated, or empty filters; LAN authorization may reject invalid queries earlier. An unmatched `operationId` returns 404 `AGENT_HELP_NOT_FOUND`; unmatched path/method searches retain 200 with an empty list for compatibility. Errors do not echo query values. Never place credentials in query parameters.
 ...
 The plugin layer also provides `node scripts/agent-api-route-inventory.mjs`, which separates exact method/path entries, `dispatchScopes` (prefixes), and `unresolved` wildcard or unexpanded entries. It uses the TypeScript AST without executing plugins, handles reordered/quoted properties, ignores commented declarations, and reports dynamic paths/methods or spread declarations as unresolved. Only objects with an explicit routeId property are recognized; fully imported or factory-generated declarations are outside its coverage. Exact counts distinguish file and routeId, not globally unique URLs. Do not treat prefixes or the single Agent catalog as all Rabi APIs. Use `POST /api/agent/send` for group/private messages and files. Before sending, call `GET /api/agent/send/capabilities` to obtain current channels, parameter guidance, and examples. The built-in catalog's `params` keys also drive the send field allowlist; actual validators still enforce types, conditional requirements, and authority. Descriptive text is not a complete JSON Schema. QQ/NapCat uses `channel: "napcat"`; `channel: "qq"` is not supported. Unknown channels return structured `errorCode`, `help`, `repair`, and supported-channel guidance. Help coverage for other parameter errors depends on the specific handler. Use `POST /api/personas/{targetPersonaId}/messages` for cross-persona delivery, and the Rabi thread bridge `POST /api/agent/threads` for persistent task prompts and follow-ups. Submit formal callbacks according to the source requirement contract. Preserve taskBinding, source identity, requestId, tracking, reply references, and stable idempotency fields. Rabi adapters calling NapCat, Desktop IPC, or another platform are part of the managed path; callers skipping Rabi constitute a bypass.
...
Common parameters include `query` (multiple terms separated by whitespace, ASCII comma, Chinese comma, or enumeration comma), `match=any|all` (default `any`), `adapter`, `kind=group|private`, `sender`, `target`, `conversationKey`, `from`, `to`, `includeArchives=1`, and `limit`. When the user says “the last hour” or another relative time range, convert it to Unix-second `from` (and `to` when needed); when the user asks for “all messages”, omit `query` rather than inventing keywords. `limit` is only a response cap, not proof of complete coverage; an archive query without a time range scans the full archive and should be avoided for large histories. The response contains `entries`, `count`, and `coverage`; records retain endpoint, group/private conversation key, sender, target, message ID, reply ID, and attachment summaries. `kind=group` limits results to inbound group messages and `kind=private` to inbound private messages; do not treat `reply_sent` outbound replies as original user feedback.
...
#### Partial machine-readable upload Help contract

`GET /api/agent/help?path=%2Fapi%2Fagent%2Fuploads%2F%3AuploadId` exposes optional `help.machineReadable` for PUT/GET (`version:1`, `kind:partial-upload-contract`), defined in `src/manager/agentUploadContract.ts`. It is neither an all-API OpenAPI document nor an execution validator or authorization rule.

- `request.path.uploadId.schema` describes lowercase UUIDs (versions 1–8, RFC variant); `request.query.allowed=[]` means the Agent catalog permits no query parameters.
- `request.headers` lists upload business headers, not a complete authentication-header contract. Required PUT headers must be single-valued; `idempotency-key.equalsPathParameter=uploadId` expresses cross-field equality. Content-Type values are case-insensitive but accept no parameters; `content-encoding` is forbidden. The request SHA-256 accepts either case and the receipt uses lowercase. File names are URI-decoded and then checked by both route and store for basename, safe-name and UTF-8 byte constraints; these semantics are not fully modeled as schemas. Optional Content-Length is converted with Number and checked by the store as a nonnegative safe integer matching the received byte count.
- PUT `request.body` is `{kind:binary,mediaType:application/octet-stream,encoding:raw,emptyAllowed:true}`, with **no JSON Schema**; it is not a JSON string or Base64. GET uses `kind:none`: the handler does not read the body, without claiming to actively reject every request carrying one.
- `responses["200"].schema` uses JSON Schema 2020-12 and describes only the successful JSON body shape `{code:0,data:{id,fileName,size,sha256,expiresAt}}`, with all fields required and no additional fields. PUT/GET share this shape. GET does not return upload progress, a full state machine, file bytes, owner or local path. `scope:success-body-shape` does not claim coverage of all value semantics or HTTP boundaries.

Both operations remain `contractLevel:baseline`, `coverage.exactRequestSchema=false`, and `coverage.exactResponseSchema=false`. `coverage.missing` equals the machine contract's `missing`: authentication/authorization boundaries, error-response schemas, response headers, storage DTO value semantics, plus PUT filename/stream-integrity/runtime-limit checks or GET ownership/expiry/integrity checks remain explicitly incomplete. Consumers must not promote an operation to verified, bypass ownership checks or replay uploads merely because a 200 schema exists.

Upload errors preserve the existing string `code` and add the matching `errorCode`, `help { method, path }`, `repair`, and `retryable:false`. This forbids automatic replay, not a later explicitly corrected request. For 415, correct the binary format; for 403, check the original identity and enabled state; for 409 or uncertain results, GET the original ID before proceeding, without changing IDs to bypass the conflict. Help links grant no additional authority. HTTP 408 `upload_timeout` and disconnected-stream `upload_aborted` have specific repair guidance, retain `retryable:false`, and require GET on the original ID before proceeding, never automatic re-upload. A disconnected client is not guaranteed to receive error JSON.
