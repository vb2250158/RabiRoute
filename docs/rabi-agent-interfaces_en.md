...
New plans must provide an ordered `steps` array. An in-progress plan must also provide `currentStepId`, pointing to the only step whose status is `进行中`; clients use this to list every step and mark the current execution point. Manager maintains step `startedAt/completedAt` during status writes: in-progress records the start time, completed records the completion time while preserving the start, reopening clears the stale completion time, and resetting to not-started clears both. RibiWebGUI shows only the start time for in-progress steps, only the completion time for completed steps, and no time for not-started steps. When a user explicitly pauses the plan, PATCH the top-level `status` to `暂停`, preserve that step and `currentStepId` as the resume position, and stop driving the bound task; resume by changing only the top-level status back to `进行中`. Waiting for an owner, approval, review, QA, or an external result uses `waitingFor`; every inspection asks or follows up until it gets a clear result, and the step remains In progress. Only when inquiry, escalation, retry, and alternative actions are all unavailable should the current step set both `isBlocked=true` and `blockedBy`. Clients render only the Manager-derived blocked state instead of interpreting raw wording. Legacy plans remain readable, but should gain structured steps on their next update; missing timestamps are backfilled with that plan-write time and do not reconstruct older history. Every step must be `已完成` before the plan can become completed or archived.
...
The reminder body requires the main persona to consume the result and continue scheduling in the same turn. If the plan remains actionable, it uses the `/api/agent/threads` `send` action for the plan's own exact `taskBinding.sessionId + workspace`. Completion or pause PATCHes `taskBinding=null` to release the slot and immediately binds the next actionable plan. The main persona inspects every non-terminal plan and assistant slot and ends only when `actionable but idle plan count = 0`. These decisions and writes belong to the Agent; neither the Stop Hook nor Manager performs them automatically.
