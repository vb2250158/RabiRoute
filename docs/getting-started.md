...
首次启动时，如果没有 `data/gateways.json`，manager 会优先复制整包 `examples/data`，这样默认 QQ gateway 和 Rabi 示例人格会一起落地。即使发布包里没有 examples，manager 也能自己建立最小 QQ / NapCat 到 Codex Desktop 配置。
...
如果修改了 `data/gateways.json` 后 reload 失败，也检查文件末尾是否混入了字面量 `\n`：
...
- `src/manager.ts`：读取 `data/gateways.json`、启动/停止 gateway、提供 RibiWebGUI API。
