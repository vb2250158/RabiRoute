...
Manager 每小时检查过期文件和空间上限。达到空间上限时，先删除最旧的性能文件。
...
- 性能汇总、原始 JSONL 解析和响应 JSON 序列化在专用低优先级子进程中执行。该池同时执行 1 项、等待 1 项、超时 60 秒；相同查询共用任务，队列满时返回 503。
- 只读子进程在请求之间保持常驻并复用模块缓存，避免每次记忆读取或性能查询都重新启动进程和加载模块。所有池合计最多执行 2 项低优先级重任务，避免一次 Agent 扫描让记忆和性能查询排队一分钟；各状态中的 `executionMode`、`workerPids`、`globalActive` 和 `globalMaxConcurrency` 显示隔离方式与总预算，`workers` 与 `spawnedWorkers` 可发现异常重启。
- Manager 启动时按流读取已有性能 JSONL，每次只解析一条记录，不整文件读取和拆分。
