...
当前主线不再让 AIUI 或眼镜直接请求 Relay。眼镜通过 Classic Bluetooth/P2P 把 PCM、控制消息和设备媒体交给手机；手机调用 Relay，Rabi PC 的眼镜消息端完成 ASR/TTS。历史 AIUI 网络代理行为仅保留为兼容证据。

## 目标架构

```mermaid
flowchart LR
    Glass["Rokid 眼镜<br/>HUD / PCM / 设备媒体"]
    Phone["RabiLink 手机后端<br/>网络 / 凭证 / cursor / 队列"]
    Watch["手表 / 耳机 / 其他便携端"]
    ...
    Glass <-->|"Classic BT / P2P 音频与媒体"| Phone
    Phone <-->|"HTTPS"| Relay
