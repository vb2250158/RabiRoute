...
> 状态：历史任务书，不再作为当前执行授权。人格数据同步部分已经退役，不得据本文重新实现、开启或验收同步；当前跨电脑访问使用 RabiLink，见 [跨电脑接口调用](rabilink-peer-rpc.md)。以下保留当时的设计与验收背景，其他领域也须以各自当前合同为准。
>
> 文档类型：Codex 目标模式执行任务书（历史）。
>
> 日期：2026-07-24。
>
