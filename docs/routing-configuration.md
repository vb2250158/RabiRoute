...
## pipeline preset / channel preset

`routeConfig.json` 可以额外写 `pipelinePreset` 和 `pipeline`，把默认输入端、输出端、提示词输出模式和 TTS 配置意图打成一组。这个配置不会强行改变旧路由行为；不写时仍按原来的 `messageAdapters`、规则和模板投递。
