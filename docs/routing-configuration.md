...
<div align="center">
<a href="./routing-configuration_en.md">English</a> | 简体中文
</div>
<!-- /docs-language-switch -->
...
路由入口参数来自 `data/route/<配置名>/adapterConfig.json`。manager 会用其中的 `agentRoleId` 找到对应角色，再从该角色的 `personaConfig.json` 中读取相同 `configName` 的规则。绑定人格但没有匹配外部消息规则时，外部消息只记录不投递；内置 `role_panel_message` 规则仍会保留。显式无人格 route 会按已启用消息入口生成默认规则。
...
`adapterConfig.json` 可以额外写 `pipelinePreset` 和 `pipeline`，把默认输入端、输出端、提示词输出模式和 TTS 配置意图打成一组。它不改变消息是否命中规则；未配置时使用 legacy fallback：`outputAdapter=agent`，回复默认保留在 Agent 会话，除非带有明确来源或外部目标。
