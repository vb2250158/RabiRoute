...
Check that the ID still exists, normalize moved/case/symlink-changed paths, and confirm whether the task is archived, deleted, or owned by another account/`CODEX_HOME`. An archived fixed binding is replaced on the next real delivery; deleted, cross-account, or workspace-mismatched targets still require reselection.
