const MESSAGE_ADAPTER_CONTROL_CONTRIBUTIONS = [
  {
    kind: "page",
    surface: "web.pages",
...
const MANAGER_PLUGIN_DEPENDENCIES = {
...
const DIAGNOSTICS_CONTRIBUTIONS = [
  {
    kind: "page",
    surface: "web.pages",
    id: "runtime-page",
...
function plugin(context, id, name, contributions) {
...
function basePluginDefinitions(context) {
  return [
    plugin(context, "core", "RabiRoute Manager Core", [
      {
        kind: "page",
        surface: "web.pages",
        id: "overview-page",
    ...
    plugin(context, "persona", "Rabi Persona Management", [
      {
        kind: "page",
        surface: "web.pages",
        id: "persona-page",
    ...
    plugin(context, "speech", "RabiSpeech Manager", [
      {
        kind: "page",
        surface: "web.pages",
        id: "speech-page",
    ...
    plugin(context, "performance", "Performance Monitoring", [
      {
        kind: "page",
        surface: "web.pages",
        id: "performance-page",
    ...
    plugin(context, "desktop", "RabiRoute Desktop", [
      {
    ...
    plugin(context, "gateway-runtime", "RabiRoute Gateway Runtime", [
      {
        kind: "command",
        surface: "desktop.panel",
    ...
    plugin(context, "bilibili-history", "Bilibili History", []),
    plugin(context, "route-control", "Route Control", [
      {
        kind: "command",
        surface: "web.commands",
        id: "quick-setup",
    ...
    plugin(context, "message-adapter-control", "Message Adapter Control", MESSAGE_ADAPTER_CONTROL_CONTRIBUTIONS),
    plugin(context, "agent-adapter-catalog", "Agent Adapter Catalog", []),
    plugin(context, "agent-state-control", "Agent State Control", []),
    plugin(context, "agent-thread-control", "Agent Thread Control", []),
    plugin(context, "agent-communication", "Agent Communication", []),
    plugin(context, "copilot-control", "GitHub Copilot Control", []),
    plugin(context, "astrbot-control", "AstrBot Control", []),
    plugin(context, "marvis-control", "Marvis Control", []),
    plugin(context, "remote-agent", "Remote Agent", []),
    plugin(context, "diagnostics", "Manager Diagnostics", DIAGNOSTICS_CONTRIBUTIONS),
    plugin(context, "rabilink-relay", "RabiLink Relay", []),
    plugin(context, "memory-consolidation", "Memory Consolidation", []),
    plugin(context, "fennenote-output", "FenneNote Output", []),
    plugin(context, "message-processing-control", "Message Processing Control", []),
    plugin(context, "message-processing-automation", "Message Processing Automation", []),
    plugin(context, "plan-feedback-delivery", "Plan Feedback Delivery", []),
    plugin(context, "napcat-control", "NapCat Control", []),
    plugin(context, "napcat-supervisor", "NapCat Supervisor", [])
  ];
}

export const managerPluginInstanceIds = Object.freeze(
  basePluginDefinitions({ bundle: { version: "0" } }).map(definition => definition.instanceId)
);

export function createPlugin(context) {
  const definition = basePluginDefinitions(context)
    .find(item => item.instanceId === context.instanceId);
  if (!definition) {
    throw new Error(`Unknown rabi.manager.base instance: ${context.instanceId}.`);
  }
  if (typeof context.services.activate !== "function") {
    throw new Error("The RabiRoute base bundle requires its scoped Manager activation capability.");
  }
  return {
    ...definition,
    async apply(ctx) {
      await context.services.activate(ctx);
    }
  };
}
