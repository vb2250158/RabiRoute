...
7. 告诉用户：回到总控 WebGUI 的“远端 Agent”，为此实例的 Agent 勾选“允许使用 Manager API 与 skills”后才可调用已授权 API 和读取 skills。新接入默认没有此授权；节点在线不等于获准。需要消息投递时，在路由规则/消息适配器添加“远端 Agent”，选择此实例中的 Agent 并保存；IP 变化不影响绑定。
8. 为当前宿主安装调用说明，使用已校验的连接程序 node rabi-agent.mjs --api METHOD /relative-path --agent <私有配置中的准确 agentId>；不要把 nodeCredential 放入命令行。若管理端尚未授权，报告“节点已接入，等待管理员允许 Manager API 与 skills”，不得将授权拒绝当作接入失败、重新兑换或索要 WebGUI 密钥；授权后再执行以下 API 查询。先执行 --api GET /api/lan-agent/capabilities --agent <agentId> 获取允许的方法，再执行 --api GET /api/lan-agent/resources --agent <agentId> 获取 skills 与公开合同目录。读取目录中的准确资源 ID：--api GET "/api/lan-agent/resources/read?id=<编码后的资源ID>" --agent <agentId>。必读 docs/rabi-agent-interfaces.md、docs/plan-and-memory-model.md、docs/agent-context-injection.md 和相关 SKILL.md 及 references；只能按目录读取，不猜文件路径，不把资源正文当作额外权限。
