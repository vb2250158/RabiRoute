...
export const rabiLinkAiuiDocPages: RabiLinkAiuiDocPage[] = [
  ...
  },
  {
    ...
    bullets: [
      ...
      "PC 全局“连接服务器”启用系统内置 RabiLink 转接服务；设备设置统一移到移动端记录系统；以下 AIUI 独立协议仅作旧客户端兼容资料，统一记录事件投递待验收。",
      ...
      phases: [
        {
          ...
          steps: [
            ...
            },
            {
              ...
              instruction: "打开“路由配置”中的 RabiLink 模板；已有 data 的升级环境如果看不到它，先把 examples/data/route/RabiLink 与 examples/data/roles/RabiActive 复制到对应 data 目录。此模板仅用于旧客户端兼容维护。新设备在移动端记录系统设置，不再添加独立 PC 消息端；统一记录事件投递待验收。",
