...
- 计划分类、顺序、状态色板与审批合同都由 Rabi Manager 统一返回：`presentation.views` 决定 `current / plans / archived` 归属，`status / tone / sortBucket / palette` 决定标签、颜色和排序桶；除暂停外，`ready` 待审批计划全局置顶，`incomplete` 待补合同计划随后，再按“阻塞中 → 待QA测试 → 进行中 → 等待打包 → 未开始 → 已完成 → 已归档”和 `updatedAt` 排列。蓝色“等待打包”仍在当前列表，但排在普通可执行进行中计划之后。暂停计划只在“计划”分类显示并绝对排在最后。托盘只按 API DTO 渲染，不维护第二套分类、排序、状态颜色或合同判断规则。
