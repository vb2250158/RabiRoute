...
- 计划分类、顺序、状态色板与审批合同都由 Rabi Manager 统一返回：`presentation.views` 决定 `current / plans / archived` 归属；除暂停外，`ready` 待审批计划全局置顶，`incomplete` 待补合同计划随后，再按“阻塞中 → 待QA测试 → 进行中 → 未开始 → 已完成 → 已归档”和 `updatedAt` 排列；暂停计划只在“计划”分类显示，使用灰蓝色状态，不派生阻塞或审批，并绝对排在最后。`presentation.palette` 同时提供卡片强调色与状态徽标前景/背景色。托盘只按 API DTO 渲染，不维护第二套分类、排序、状态颜色或合同判断规则。
- Manager 判定当前计划/步骤需要审批时，展开卡片会与 WebGUI 一致地展示已有的批准事项、原因、文件、完整命令、配置/数据/外部变更、验证、回退和排除范围。说明不完整时列出建议 Agent 补充的项目，但仍显示审批输入。提交只等待 Manager 落盘，默认 5 秒请求边界；返回 `pending` 后立即结束 loading，Agent 通知在后台继续。意见关联 `planId` 与 `stepId`，记录失败时可用同一 `feedbackId` 重试。该入口不直接推进步骤或改变计划状态。
