...
- YeYu 桌宠从 Manager 读取受限动作包和人格绑定，支持 GIF / PNG 序列、拖动位置、缩放、置顶、锁定、点击穿透、气泡与全屏自动隐藏；实际拖动期间播放 `drag`，松手后恢复拖动前状态，双击打开 YeYu 人格面板。只有带 `personaId=YeYu` 的 `work_ended` 事件会触发成功或担心动作，普通任务状态和人格文件不会由桌宠直接改写。资源包导入与设置均在 WebGUI 经 Manager 完成，Qt 不直接扫描或修改 `data/roles/YeYu/desktop-pet/`。
