...
- `plan.status` 是托盘唯一显示状态，只能是“分析中、待审批、执行中、等待打包、等待 QA、待讨论、暂停、完成、关闭”。Rabi Manager 返回与其完全相同的 `presentation.status` 以及 tone、色板、排序等级和视图；托盘不从步骤、等待原因或审批合同派生第二个状态。`archiveStatus` 独立决定归档视图，归档不改变 `plan.status`；已归档计划不参加关键词召回，只能按明确计划 ID 或归档视图读取。
