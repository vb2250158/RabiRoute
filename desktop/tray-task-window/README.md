...
- Manager / Gateway 状态快照通过 `/gateways?summary=1` 读取显式白名单的轻量摘要，不包含日志、诊断文件和平台凭据；同一后台刷新任务会同时完成 Manager HTTP、计划/记忆文件和当前可见聊天记录读取，Qt 主线程只应用完成结果。隐藏面板不读取聊天或重建 QWidget；托盘菜单显示期间会延迟应用刷新结果；无变化时不会重建人格菜单或重复渲染面板，避免 10 秒自动刷新与托盘右键竞争。同一时间只保留一个刷新任务，但不会丢失用户明确点击的手动刷新。短暂超时时保留上一次成功的人格列表并标记为旧快照，真正的 Manager 离线仍会清空运行状态。
