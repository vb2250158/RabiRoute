...
- 计划分类、阶段汇总、顺序、状态色板与审批合同都由 Rabi Manager 统一返回：`presentation.views` 决定 `current / plans / archived` 归属，`status / tone / sortBucket / palette` 决定标签、颜色和排序桶，`counts.stages` 提供各展示阶段计数；除暂停外，完整待审批合同全局置顶，待补合同随后，再按“待审批 → 等待 QA 验收 → 正在执行 → 待环境/待素材/待资料/待外部回执 → 待统一打包 → 未开始 → 已完成 → 已归档”和 `updatedAt` 排列。琥珀色外部等待与蓝色“待统一打包”仍在当前列表；暂停只在“计划”显示并绝对排在最后。托盘只按 API DTO 渲染，不维护第二套阶段识别、分类、排序、状态颜色或合同判断规则。
