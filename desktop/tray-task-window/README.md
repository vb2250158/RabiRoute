...
- Desktop 会从 Manager 插件目录启用系统截图、剪贴板贴图快捷键和 `system` / `light` / `dark` 主题。目录只接受内置 `commandId`、默认快捷键和 `desktopResourceId` 组合；实际快捷键仍读取“设置”中保存的值。目录缺失或声明不受支持时，对应插件快捷键不会注册，主题回退为系统主题。
