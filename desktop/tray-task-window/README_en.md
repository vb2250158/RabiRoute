...
- Plan categories, order, status colors, and approval contracts come from Rabi Manager. `presentation.views` supplies `current / plans / archived` membership. Except for paused plans, `ready` approvals sort first, `incomplete` contracts next, followed by `Blocked → Awaiting QA → In progress → Not started → Completed → Archived` and newest `updatedAt`. Paused plans appear only under Plans, use the shared slate palette, derive neither blocked nor approval presentation, and always sort last. `presentation.palette` supplies the card accent plus status-badge colors. The tray renders the API DTO without maintaining a second category, sorting, status-color, or contract-completeness rule.
