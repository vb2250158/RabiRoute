...
- When **Enable selected-text menu** is turned on in WebGUI **Settings**, any Windows app that exposes a UI Automation text selection can use the system floating bar. Hovering over **Send to** opens the currently enabled and running persona list; clicking one item reuses role-panel delivery for that Route. Selection alone performs no action. Password controls and unreadable selections are ignored; the tray never simulates `Ctrl+C` or changes the clipboard. **Selected-text reading** is a sub-feature: when it is on, the left button is **Read aloud** and a click enters the RabiSpeech host FIFO; when it is off, the bar keeps only **Send to**. The TTS model selector appears only when both **Selected-text reading** and **Advanced options** are on.
- Under **Settings**, enabling system screenshots and configuring a shortcut makes the screenshot action available from any Windows application. The shortcut opens a preview, text input, and active-persona selector; **Send** delivers the image and text through the same role-panel entry, and Codex/DSH receive the image as real image input. Screenshots are kept in the private project `.rabiroute-message-images/` directory and are not part of public examples.
- **Settings → Windows login startup** synchronizes a per-user Startup shortcut. Turning it off removes that shortcut. The tray watches the settings file, so changing the screenshot toggle, shortcut, or login-startup option does not require restarting the tray.
...
- `system_selection`: Windows drag-selection detection, UI Automation text extraction, the no-activate floating bar, the active-persona hover menu, and Manager-only speech/role-panel actions. The Read aloud button is hidden when `readAloudEnabled` is false.
- `system_screenshot`: Windows global screenshot hotkey, multi-monitor capture composition, preview/text composer, and role-panel image-attachment delivery; persistent settings are owned by Manager at `/api/desktop/settings`.
