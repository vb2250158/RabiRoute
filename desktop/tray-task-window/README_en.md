...
- The card shows a purple `Awaiting QA` badge only when Manager identifies the structured current step as an in-progress `qa-* / verify-*` step. A future QA step, or prose such as “QA gate” and “QA not notified” inside an implementation step, does not change the badge; the tray never scans free-text phase keywords.
- Plan categories, order, status colors, and approval contracts come from Rabi Manager. `presentation.views` supplies `current / plans / archived` membership, while `status / tone / sortBucket / palette` supplies the label, colors, and ordering bucket. Except for paused plans, `ready` approvals sort first, `incomplete` contracts next, followed by `Blocked → Awaiting QA → In progress → Awaiting package → Not started → Completed → Archived` and newest `updatedAt`. Blue `Awaiting package` remains in Current but sorts below ordinary executable running plans. Paused plans appear only under Plans and always sort last. The tray renders the API DTO without maintaining a second category, sorting, status-color, or contract-completeness rule.
