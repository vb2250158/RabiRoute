...
- Desktop enables the system-capture hotkey, clipboard-image pin hotkey, and `system` / `light` / `dark` themes from the Manager plugin catalog. The catalog accepts only built-in `commandId`, default-binding, and `desktopResourceId` combinations; active shortcuts still use the values saved in **Settings**. Missing or unsupported declarations leave the corresponding plugin hotkey unregistered and fall back to the system theme.
