...
- Reads an explicitly allowlisted Manager/Gateway summary from `/gateways?summary=1`, excluding logs, diagnostic files, and platform credentials. One background refresh performs the Manager HTTP request, plan/memory file reads, and the currently visible panel's conversation read; the Qt thread only applies the completed result. A hidden panel neither reads conversation data nor rebuilds widgets, completed refreshes wait while the tray menu is visible, and unchanged snapshots neither rebuild the persona menu nor rerender the panel. This prevents the ten-second refresh from competing with a tray right-click. Only one refresh runs at a time, but an explicit manual refresh is queued instead of discarded. A transient timeout retains and labels the last successful persona list, while a real Manager disconnect still clears live runtime state.
