...
- `plan.status` is the tray's only displayed status and accepts exactly Analyzing, Awaiting approval, Executing, Awaiting package, Awaiting QA, Awaiting discussion, Paused, Completed, or Closed. Manager returns the same `presentation.status` plus tone, palette, sort level, and views. The tray never derives another state from step IDs, prose, waiting reasons, or approval metadata.
- `archiveStatus` independently controls Archived view membership. Only Completed or Closed plans may archive after the delay, and archival preserves `plan.status`. Archived plans do not participate in keyword recall and are available only by explicit plan ID or Archived view.
