...
- When **Read selected text aloud** is enabled on the Speech Service TTS page, any Windows app that exposes a UI Automation text selection can use the system floating bar. Drag-selecting text shows Read aloud on the left, which enters the RabiSpeech host FIFO, and Send to current persona on the right, which reuses role-panel delivery for the Route currently selected in the tray. Selection alone performs no action. Password controls and unreadable selections are ignored; the tray never simulates `Ctrl+C` or changes the clipboard.
...
- `system_selection`: Windows drag-selection detection, UI Automation text extraction, the no-activate two-button bar, and Manager-only speech/role-panel actions.
