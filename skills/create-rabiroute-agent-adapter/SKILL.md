...
1. 已保存的会话名称与完整线程 ID 同时匹配 owner 记录时，直接投递到该会话，不创建新会话。
2. 目标不存在时，按明确名称幂等创建一个会话，再把真实消息投给新会话；并发、重试和索引延迟期间仍只能创建一次。
...
5. 会话身份是“可见名称 + 完整线程 ID”配对，不是只认 ID。Agent 端改名或 Rabi 端修改名称导致配对不一致时，旧 ID 视为陈旧绑定；按保存的新名称重新查找，唯一匹配就重绑，零匹配就幂等创建，多匹配要求用户选择。
...
- 统一 resolver 必须先读取有效 ID，但只有 owner 返回的名称也与保存名称完全一致时才算精确绑定；ID 非法、失效或名称不一致时，再按保存名称 + 规范化 cwd 查找。唯一匹配自动重绑，零匹配按用户输入的新名称创建，多个同名要求选择。
