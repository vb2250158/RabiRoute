...
2. 目标不存在时，先按 RabiPC Manager 保存的明确名称 + workspace 查找；唯一匹配直接重绑，零匹配才幂等创建一个会话，再把真实消息投给新会话，并发、重试和索引延迟期间仍只能创建一次。允许提供用户显式点击的“自动初始化会话”：它必须先完成同一保存/解析事务，再通过唯一 owner 投递一条携带角色文件、记忆、计划和必读项的人格初始化消息。
