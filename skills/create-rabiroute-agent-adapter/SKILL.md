...
Codex Desktop / Codex App 目前是已跑通的基线：能扫会话、能扫工作目录、能显示绑定状态和错误。其他 Agent 在没有完成本 skill 的验证清单前，都按“实验/未验证”处理，不能在 UI、README 或示例里暗示已经稳定可用。
...
| Agent | 默认等级 | 说明 |
| --- | --- | --- |
| Codex Desktop | `verified` | 已跑通；作为会话发现、项目目录、运行状态诊断的 UI 参照。 |
| Codex App | `verified` 或随当前实现标记 | 复用 Codex 会话/目录模型；如果本机没有验证，降为 `experimental`。 |
...
- **桌面会话型**：有本地 app、会话列表、可能有项目目录，例如 Codex Desktop、Codex App。
...
3. **列出会话线程**：优先找官方 API、CLI 命令、本地索引文件、插件 API，拿到 thread/session 列表。
