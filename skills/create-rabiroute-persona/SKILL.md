...
`growth.md` 可记录这个角色如何复盘、学习和更新自己；`skills.md` 可记录这个角色常用的 skill、资料源、判断方法或工作技巧；`prompts/` 可放更细分的场景提示词。
...
- 成长方式：它闲下来时如何复盘、学习、找资料、更新自己的提示词或补充专用 skill。
...
建议为成长型人格创建一条 `heartbeat` 路由。心跳不是“必须找事做”，而是给角色一个低频自检机会：先看最近是否有未处理消息；如果没有即时任务，就思考自己如何更好地扮演当前角色，并生成改进建议或可审阅的更新草稿。
...
- 空闲或心跳触发时，先检查最近消息和待处理事项。
- 如果没有即时任务，复盘自己是否更好地扮演了当前角色。
- 可以查找适合本角色的技巧、资料或 skill，例如 PM 学 PM 方法，老师学讲解方法，客服学安抚和澄清方法。
- 可以提出对 `persona.md`、`growth.md`、`skills.md` 或 `prompts/` 的更新建议。
- 只有在用户或项目规则明确允许时，才实际修改自己的人格文件夹。
...
```json
{
  ...
  "targetGroupId": "",
  "regex": "",
  "template": "定时触发提醒：到了 {routeProfileName} 的成长心跳时间。\n事件时间：{time}\n当前时间：{currentTime}\n星期：{currentWeekday}\n间隔：{heartbeatIntervalSeconds} 秒\n消息：{message}\n\n请先读取 {heartbeatLogPath} 和必要的消息日志，判断是否有未处理事项。若没有即时任务，请按 persona.md 的成长机制复盘自己如何更好地扮演当前角色，并提出可审阅的改进建议；只有在获得授权时，才修改 {agentRoleDir} 下的人格文件。",
...
- 如果写了成长机制，确认心跳路由只生成复盘和更新建议，实际自我修改需要授权。
