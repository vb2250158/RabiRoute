...
拉取前比较本地改动与目标远端提交的文件交集，并逐项检查语义和未跟踪文件。没有交集时保留原地改动；有交集时按用户授权保存相关补丁，不因工作区不干净整体回退或暂存。字段与目录改名后检查本地脚本及忽略规则，保留原有签名、凭据与配置。

共享盘不支持依赖链接时，先按项目构建要求使用本机磁盘。确需在共享盘安装且包管理器支持时，可评估 `--install-links`；它会改变依赖与锁文件语义，先保存锁文件并核对差异，只恢复本次安装造成的部分，不能覆盖用户已有改动。

源码提交、构建产物与已安装版本分别核验。通过当前 Host 获取运行身份和安装信息，不根据源码版本断言运行实例已经升级。
