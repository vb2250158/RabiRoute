...
如果你想“把示例拖出来就能跑”，优先复制 `examples/data` 到仓库根目录的 `data`。这会得到默认 QQ gateway、Rabi 人格和对应路由规则。

## 示例 data 包

- `data/gateways.json`：默认 QQ / NapCat 到 Codex Desktop 的示例配置。
- `data/default-main/roles/Rabi/`：默认 gateway 配套的 Rabi 示例人格。
...
复制后可以直接在 WebUI 里看到默认 QQ gateway 和 Rabi 人格。
...
- `roles/Rabi/`：轻量公开人格示例，包含 `persona.md` 和 `routes.json`。
