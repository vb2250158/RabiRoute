...
> 状态：实验集成。该模板与 RabiLink AIUI 前台连续转录链路配套，不代表锁屏或后台常驻录音能力。
...
模板使用 L0 静默观察到 L5 紧急介入的等级，并综合用户收益、意图置信度、时效、打扰成本和行动风险。Codex/Rabi 的主动下行与眼镜上行记录相互独立；成功投递仍写回同一会话账本。
...
- `rabilink` 消息端。
- `codex` Agent adapter，并绑定一个由 Codex/ChatGPT Desktop 加载的目标任务。
- RabiLink Relay 全局连接。
...
AIUI 可以在页面前台持续启动单轮 ASR、自动续接并将文本同步到 PC 账本。它不是 Android 前台服务：退出页面、锁屏或系统回收 AIUI 后，不能承诺仍在录音。

离线时页面最多保留最近 48 小时、2000 段待同步文本。真实消息最终由 Desktop IPC 投给已加载任务；Desktop 不可用或任务未加载时失败关闭，不启动备用 Runtime。
