...
$checks = [ordered]@{
    ...
    queryRequested = $joined -match "appIsInstalled target=com\.rabi\.link\.glass|已请求查询眼镜端 ASR APK 安装状态"
    installRequested = $joined -match "appUploadAndInstall|已请求安装内置眼镜 APK"
    startRequested = $joined -match "appStart entry=com\.rabi\.link\.glass\.GlassAsrProbeActivity|已请求启动眼镜端 ASR 应用"
