...
这条证据把 RokidAiSdk 的 ABI 判断拆开了：当前手机 `4b64e6d6` 只有 `arm64-v8a`，所以官方 1.4.3 `armeabi-v7a` 语音 SDK 不适合放在手机 APK 内验证；但眼镜端同时支持 `armeabi-v7a` 和 `arm64-v8a`，因此“把官方 RokidAiSdk 作为眼镜端独立语音探针”在 ABI 层面存在继续实验的可能。它仍不等于已闭环，因为还需要官方 SDK 资产、开放平台凭证和眼镜端启动服务权限；同时当前眼镜端 Android 系统 `SpeechRecognizer` / `TextToSpeech` 仍然不可用。

随后把眼镜端测试 APK 强制为 32 位进程：`glass-asr` 增加 `ndk { abiFilters "armeabi-v7a" }`，版本升到 `versionCode=6` / `0.1.5`。重新构建、安装、启动后，CXR CustomCmd 仍能收到 `RABI_STATUS`，并且 native 目录已经切到 32 位：
...
| APK 安装 | 已安装到 `4b64e6d6`。 | `adb install -r app-debug.apk` 成功。 |
...
.\scripts\Set-RokidAiSdkConfig.ps1 -Serial 4b64e6d6 -Clear
```

也可以从本地下载的官方 demo 源码中导入测试配置。脚本不会在 stdout 输出原值：

```powershell
.\scripts\Set-RokidAiSdkDemoConfig.ps1 -Serial 4b64e6d6
```
...
& $adb -s 4b64e6d6 shell pm grant com.xiaomi.mibrain.speech android.permission.RECORD_AUDIO
& $adb -s 4b64e6d6 shell appops set com.xiaomi.mibrain.speech RECORD_AUDIO allow
```
...
```powershell
.\scripts\Set-RokidNativeVoiceAuth.ps1 -Serial 4b64e6d6 -Clear
```
...
.\scripts\Test-RokidNativeVoiceBridge.ps1 -Serial 4b64e6d6 -Install
```
...
```powershell
cd <repo>\examples\android-rabi-link-probe
.\scripts\Install-RokidGlassAsrWithRetry.ps1 -Serial 4b64e6d6 -MaxAttempts 4
.\scripts\Test-RokidNativeVoiceRealDevice.ps1 -Serial 4b64e6d6 -Commands ping,tts,asr_start,asr_stop
```
...
```powershell
cd <repo>\examples\android-rabi-link-probe
.\scripts\Send-RokidNativeVoiceCommand.ps1 -Serial 4b64e6d6 -Command tts -Text "这是一条外部命令 TTS" -WaitSeconds 3
```
...
```powershell
cd <repo>\examples\android-rabi-link-probe
.\scripts\Watch-RokidNativeVoiceEvents.ps1 -Serial 4b64e6d6 -Dedupe
```
...
```powershell
.\scripts\Watch-RokidNativeVoiceEvents.ps1 -Serial 4b64e6d6 -ClearBeforeWatch -Dedupe
```

2. 外部命令：

```powershell
.\scripts\Send-RokidNativeVoiceCommand.ps1 -Serial 4b64e6d6 -Command asr_start -WaitSeconds 1
.\scripts\Send-RokidNativeVoiceCommand.ps1 -Serial 4b64e6d6 -Command tts -Text "收到你的消息" -WaitSeconds 1
```
...
```powershell
.\scripts\Start-RokidNativeVoiceWebhookBridge.ps1 -Serial 4b64e6d6 -Dump -Dedupe -DryRun -MaxEvents 1
```
...
```powershell
cd <repo>\examples\android-rabi-link-probe
.\scripts\Start-RokidNativeTtsPlaybackServer.ps1 -Serial 4b64e6d6 -Port 8794
```
...
```powershell
.\scripts\Start-RokidNativeVoiceStack.ps1 -Serial 4b64e6d6 -TtsCommand rokid_ai_tts
```
...
```powershell
cd <repo>\examples\android-rabi-link-probe
.\scripts\Run-RokidNativeVoiceTrial.ps1 -Serial 4b64e6d6 -SkipRealDevice
```
...
```text
examples/android-rabi-link-probe/out/rokid-native-voice/rokid-stack-*/rokid-native-voice-stack.json
examples/android-rabi-link-probe/out/rokid-native-voice/rokid-stack-*/*.log
```
...
```powershell
.\scripts\Test-RokidNativeVoiceReadiness.ps1 -Serial 4b64e6d6
```
...
| 项 | 结果 |
| --- | --- |
| 手机 APK 安装 | `app-debug.apk` 已安装到 `4b64e6d6` |
...
```powershell
$adb = ".\out\tools\android-sdk\platform-tools\adb.exe"
& $adb -s 4b64e6d6 shell pm grant com.rabi.link android.permission.RECORD_AUDIO
& $adb -s 4b64e6d6 shell pm grant com.rabi.link android.permission.CAMERA
& $adb -s 4b64e6d6 shell pm grant com.rabi.link android.permission.BLUETOOTH_CONNECT
& $adb -s 4b64e6d6 shell pm grant com.rabi.link android.permission.NEARBY_WIFI_DEVICES
& $adb -s 4b64e6d6 shell pm grant com.rabi.link android.permission.POST_NOTIFICATIONS
```
