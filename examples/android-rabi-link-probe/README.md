...
Rokid 链路当前目标也不是直接接入 RabiRoute/Codex/MCP，而是在同一个 `com.rabi.link` 手机 APK 内验证 Rokid 手机侧 SDK 到底开放了哪些能力，再决定后续是否作为消息端桥接。
...
- `modules/rokid/`：Rokid 眼镜手机侧探针，已接入 CXR-L `client-l:1.0.4`，提供授权、连接、CustomView、音频流、拍照、设备信息和亮度/音量测试入口。
...
```text
examples\android-rabi-link-probe\out\mi-health-cloud\
```
