...
final class RokidProbeUi {
    ...
    static Views install(Activity activity, Actions actions) {
        ...
        addCapabilityBlockWithExtra(
                ...
                "07 眼镜端 ASR/TTS / CustomApp",
                "安装并启动一个内置的最小眼镜 APK，直接调用眼镜端 Glass SDK 的 ASR 和 TTS。",
                "用途：验证是否能拿到“用户说了什么”的文本结果，而不是只拿到 PCM/WAV 录音；也验证眼镜端 TTS 是否能自然播报。",
                "前置：已有 token；需要切换到 CustomApp 会话；眼镜允许安装和启动测试 APK。",
                "证据：CustomApp 会话状态、安装/查询/启动回调、眼镜端 ASR 日志和 RABI_ASR 文本消息。",
                nativeVoiceExtra,
                ...
                new ActionButton(ACTION_SAVE_NATIVE_VOICE_AUTH, "保存语音授权", v -> actions.saveNativeVoiceAuthorization()),
                ...
                new ActionButton(ACTION_START_NATIVE_ASR, "远程开始 ASR", v -> actions.startNativeAsrRemote()),
                new ActionButton(ACTION_STOP_NATIVE_ASR, "远程停止 ASR", v -> actions.stopNativeAsrRemote()),
                new ActionButton(ACTION_SEND_NATIVE_TTS, "发送 TTS 测试", v -> actions.sendNativeTtsTest()),
                new ActionButton(ACTION_START_NATIVE_ECHO, "ASR 回声测试", v -> actions.startNativeEchoTest()),
                ...
                new ActionButton(ACTION_PROBE_GLASS_ANDROID_VOICE, "眼镜系统语音", v -> actions.probeGlassAndroidVoice()),
                new ActionButton(ACTION_START_GLASS_ANDROID_ASR, "眼镜系统 ASR", v -> actions.startGlassAndroidAsr()),
                new ActionButton(ACTION_STOP_GLASS_ANDROID_ASR, "停眼镜系统 ASR", v -> actions.stopGlassAndroidAsr()),
                new ActionButton(ACTION_SEND_GLASS_ANDROID_TTS, "眼镜系统 TTS", v -> actions.sendGlassAndroidTtsTest()),
                new ActionButton(ACTION_PROBE_GLASS_ROKID_AI_SDK, "眼镜 AI SDK 状态", v -> actions.probeGlassRokidAiSdk()),
                new ActionButton(ACTION_SAVE_GLASS_ROKID_AI_SDK_CONFIG, "发送眼镜 AI 配置", v -> actions.saveAndSendGlassRokidAiSdkConfig()),
                new ActionButton(ACTION_START_GLASS_ROKID_AI_SDK, "启动眼镜 AI SDK", v -> actions.startGlassRokidAiSdk()),
                new ActionButton(ACTION_SEND_GLASS_ROKID_AI_SDK_TTS, "眼镜 AI TTS", v -> actions.sendGlassRokidAiSdkTtsTest()),
                new ActionButton(ACTION_STOP_GLASS_ROKID_AI_SDK, "停止眼镜 AI SDK", v -> actions.stopGlassRokidAiSdk()),
                ...
                new ActionButton(ACTION_PROBE_PHONE_VOICE_AUTH, "检查手机语音授权", v -> actions.probePhoneVoiceAuthorization()),
                new ActionButton(ACTION_APPLY_PHONE_VOICE_AUTH, "激活手机语音授权", v -> actions.applyPhoneVoiceAuthorization()),
                new ActionButton(ACTION_INIT_PHONE_VOICE, "初始化手机语音", v -> actions.initPhoneVoiceProbe()),
                new ActionButton(ACTION_START_PHONE_ASR_FEED, "手机 ASR 喂音频", v -> actions.startPhoneAsrFeed()),
                new ActionButton(ACTION_STOP_PHONE_ASR_FEED, "停止手机 ASR", v -> actions.stopPhoneAsrFeed()),
                new ActionButton(ACTION_SEND_PHONE_TTS, "手机 TTS 测试", v -> actions.sendPhoneTtsTest()),
        ...
        addCapabilityBlock(
                ...
                "08 Android 系统 ASR/TTS",
                "调用 Android 原生 SpeechRecognizer 和 TextToSpeech，验证系统音频路由是否能复用已连接眼镜。",
                "用途：在 Rokid Glass SDK / Phone SDK 都未 ready 时，确认系统级语音入口是否能作为可用文本和播报路径。",
                "前置：麦克风权限已允许；眼镜若作为蓝牙音频设备连接，应在音频设备列表里出现。",
                "证据：SpeechRecognizer 可用性、输入/输出音频设备、ASR partial/final 文本、TTS onDone 回调。",
                new ActionButton(ACTION_PROBE_ANDROID_SYSTEM_VOICE, "检查系统语音", v -> actions.probeAndroidSystemVoice()),
                new ActionButton(ACTION_ROUTE_ANDROID_SYSTEM_BLUETOOTH, "路由到蓝牙", v -> actions.routeAndroidSystemBluetooth()),
                new ActionButton(ACTION_CLEAR_ANDROID_SYSTEM_BLUETOOTH, "清除蓝牙路由", v -> actions.clearAndroidSystemBluetooth()),
                new ActionButton(ACTION_START_ANDROID_HEADSET_VOICE, "耳机语音通道", v -> actions.startAndroidHeadsetVoiceRecognition()),
                new ActionButton(ACTION_STOP_ANDROID_HEADSET_VOICE, "停耳机语音", v -> actions.stopAndroidHeadsetVoiceRecognition()),
                new ActionButton(ACTION_START_ANDROID_SYSTEM_ASR, "开始系统 ASR", v -> actions.startAndroidSystemAsr()),
                new ActionButton(ACTION_START_ANDROID_SYSTEM_ASR_INTENT, "前台系统 ASR", v -> actions.startAndroidSystemAsrIntent()),
                new ActionButton(ACTION_STOP_ANDROID_SYSTEM_ASR, "停止系统 ASR", v -> actions.stopAndroidSystemAsr()),
                new ActionButton(ACTION_SEND_ANDROID_SYSTEM_TTS, "系统 TTS 测试", v -> actions.sendAndroidSystemTtsTest()),
                new ActionButton(ACTION_START_ANDROID_SYSTEM_LOOPBACK, "系统语音回环", v -> actions.startAndroidSystemLoopback())
        );
        addCapabilityBlockWithExtra(
                activity,
                content,
                ...
                "09 RokidAiSdk ASR/TTS",
                "调用 RokidAiSdk 官方 AudioAi service，启动手机侧麦克风 ASR，并测试 SDK TTS。",
                "用途：判断 Rokid 开放平台语音 SDK 是否能直接给出文本，而不是只得到眼镜音频流；也验证 TTS 能否由 SDK 播放。",
                "前置：APK 内置 demo 的 AAR 与 workdir_asr_cn；需要 Rokid 开放平台 key/secret/deviceTypeId/deviceId/seed；麦克风权限已允许。",
                "证据：资源和凭证 readiness、service bind、record socket、ASR final 文本、TTS 请求结果、verifyFailed 错误。",
                rokidAiSdkExtra,
                new ActionButton(ACTION_PROBE_ROKID_AI_SDK, "检查 AI SDK", v -> actions.probeRokidAiSdkVoice()),
                new ActionButton(ACTION_SAVE_ROKID_AI_SDK_CONFIG, "保存 AI 配置", v -> actions.saveRokidAiSdkConfig()),
                new ActionButton(ACTION_START_ROKID_AI_SDK_ASR, "启动 AI ASR", v -> actions.startRokidAiSdkAsr()),
                new ActionButton(ACTION_ROKID_AI_SDK_PICKUP_ON, "AI 拾音开", v -> actions.enableRokidAiSdkPickup()),
                new ActionButton(ACTION_ROKID_AI_SDK_PICKUP_OFF, "AI 拾音关", v -> actions.disableRokidAiSdkPickup()),
                new ActionButton(ACTION_SEND_ROKID_AI_SDK_TTS, "AI TTS 测试", v -> actions.sendRokidAiSdkTtsTest()),
                new ActionButton(ACTION_STOP_ROKID_AI_SDK, "停止 AI SDK", v -> actions.stopRokidAiSdkVoice())
        );
