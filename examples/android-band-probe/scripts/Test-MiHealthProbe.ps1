...
function Invoke-GradleBuild {
    ...
    $gradlePath = Join-Path $env:USERPROFILE ".gradle\wrapper\dists\gradle-7.5.1-bin\7jzzequgds1hbszbhq3npc5ng\gradle-7.5.1\bin\gradle.bat"
    if (-not (Test-Path -LiteralPath $gradlePath)) {
        throw "找不到 Gradle: $gradlePath"
    }
