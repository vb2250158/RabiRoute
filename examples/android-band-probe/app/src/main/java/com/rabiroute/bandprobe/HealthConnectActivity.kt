...
class HealthConnectActivity : ComponentActivity() {
    ...
    private val permissionLauncher = registerForActivityResult(
        ...
        if (granted.containsAll(requiredPermissions)) {
            ...
            append("没有获得心率读取权限，无法读取 Health Connect 心率历史。")
        }
    }
    ...
    private fun buildUi() {
        ...
        hint.text = "用于测试小米运动健康是否把手环心率同步到了 Android Health Connect。"
        hint.textSize = 13f
    ...
    private fun checkAvailability() {
        ...
        when (status) {
            HealthConnectClient.SDK_AVAILABLE -> {
                ...
                append("Health Connect 可用。请点击“读取最近24小时”。")
            }
    ...
    private fun requestPermissionThenRead() {
        ...
        lifecycleScope.launch {
            ...
            if (granted.containsAll(requiredPermissions)) {
                ...
                append("正在请求心率读取权限...")
                append("如果系统打开权限页，请允许“心率”读取权限后返回本应用。")
    ...
    private fun readHeartRate() {
        ...
        lifecycleScope.launch {
            try {
                ...
                append("读取时间范围：${format(start)} -> ${format(end)}")
                ...
                append("心率记录条数：${response.records.size}")
                append("心率样本数量：${samples.size}")

                if (samples.isEmpty()) {
                    append("没有读到心率样本。可能原因：")
                    append("1. 小米运动健康没有写入 Health Connect。")
                    append("2. Health Connect 里没有给小米运动健康写入权限。")
                    append("3. Health Connect 里没有给本测试 App 读取权限。")
                    append("4. 最近24小时手机未同步手环数据。")
                    return@launch
                }
                ...
                append("最低心率：$min bpm")
                append("最高心率：$max bpm")
                append(String.format(Locale.US, "平均心率：%.1f bpm", avg))
                append("最近一条：${format(samples.last().time)} -> ${samples.last().bpm} bpm")
                append("前10个样本：")
                samples.take(10).forEach {
                    append("${format(it.time)} -> ${it.bpm} bpm")
                }
            } catch (error: Throwable) {
                append("读取失败：${error.javaClass.simpleName}: ${error.message}")
            }
        }
    }
    ...
    private fun copyReport() {
        ...
        clipboard.setPrimaryClip(ClipData.newPlainText("手机健康心率探针结果", report.toString()))
