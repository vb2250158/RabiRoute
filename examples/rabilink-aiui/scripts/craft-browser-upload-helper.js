(function rabiLinkAiuiCraftBrowserUploadHelper() {
  "use strict";
  ...
  const PANEL_ID = "rabilink-aiui-craft-upload-helper";
  ...
  function parseQuery() {
    ...
    return {
      agentId: params.get("defaultAgentId") || params.get("agentId") || params.get("botId") || params.get("id") || "",
  ...
  h.textContent = "RabiLink AIUI Craft Upload";
  ...
  summary.textContent = `origin=${location.origin}; session=${session.token ? "present via " + session.source : "missing"}; accountId=${tokenAccountId ? "parsed" : "not parsed"}`;

  const fileInput = document.createElement("input");
  fileInput.type = "file";
  fileInput.accept = ".aix,application/octet-stream";
  fileInput.style.cssText = "width:100%;margin:6px 0 10px;color:#eaffea";
  ...
  output.textContent = "Select dist/rabilink-aiui.aix, then upload. This helper never prints the account token.";
  ...
  appendButton(buttons, "Upload selected AIX", async () => {
    try {
      const file = fileInput.files && fileInput.files[0];
      if (!file) {
        report.error = "Select rabilink-aiui.aix first.";
      ...
      writeOutput(`Uploading ${file.name} (${file.size} bytes) to ${agentId}...`);
  ...
  panel.append(title, summary, fileInput, grid, buttons, output);
