...
- 每完成一个功能修改，先运行与改动匹配的测试，再执行 `npm run build`；该命令会编译 Manager 并重新生成、同步 `rabi.manager.base` 的 Web Bundle。
- 修改 Manager、WebGUI、插件 Bundle、静态资源或启动链后，必须重启实际 Manager，并从已重建的运行地址回读健康状态和目标页面。不能拿旧 Manager 进程、旧 `plugins/packages/rabi.manager.base/*/web/` 文件、旧浏览器标签页、Vite 开发服务或之前的构建日志当作当前部署证据。
