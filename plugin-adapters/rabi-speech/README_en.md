...
`/v1/microphone/*` is the host device control plane. It is available only to loopback RabiPC/Manager and is not included in the public RabiLink speech allowlist. After startup, the RabiSpeech process owns the capture stream; closing the browser does not stop it. `microphone.json` persists the host device, thresholds, ASR model, ASR streaming switch, and internal session; a legacy `route_id` migrates to `null`. **Enable ASR streaming** on the Speech Service ASR page is the host-wide master switch. Turning it on continuously records and transcribes; turning it off stops continuous capture immediately, while manual uploaded-audio transcription remains available. Route subscriptions do not control recording and only select which Routes receive completed transcripts. With no subscriptions, RabiSpeech still transcribes and stores host records. Runtime settings can be updated through `PUT /v1/microphone/settings`.

`barge_in_mode` defaults to `off`. The default continues suppressing VAD during host playback so the persona's own TTS cannot loop back. Set it explicitly to `echo_protected` only after the real input path has passed device-level AEC validation or has reliable physical separation from playback output. In that mode, input must remain above the VAD threshold for `barge_in_confirm_ms` (default `200 ms`) before the host FIFO synchronously runs `stop(clear_pending=true)`: current audio is silenced, every stale queued job is cancelled, and the triggering PCM is retained for normal end-of-utterance ASR, voiceprint analysis, and Route delivery. A stop failure emits `barge_in_failed`, discards the untrusted candidate, and restores playback suppression.

RabiSpeech does not currently own an acoustic echo reference or AEC. The Windows remote voice client also pauses microphone upload while it is playing, so it cannot provide natural barge-in yet. Do not merely disable `suppress_during_playback`; keep `barge_in_mode=off` whenever echo-protection evidence is absent. WebGUI requires an explicit confirmation before selecting `echo_protected`, but that confirmation is only an operator assertion and does not replace real-device acceptance.

LAN remote audio is a separate network sound-card channel. When `remote_audio` is enabled, the lightweight Windows client continuously uploads 16 kHz mono PCM over TCP `8782` and receives WAV files from the host FIFO; UDP `8783` is discovery-only. The client never performs VAD, segmentation, ASR, TTS, or Route delivery. Loopback endpoints `GET /v1/audio-streams` and `PUT /v1/audio-streams/selection` own the selection, which defaults to local and never silently falls back after a remote disconnect. Each RabiLink phone registers automatically with a stable per-install device id and stable stream id. The first phone may become the zero-configuration default; later phones join the list without stealing the persisted choice. Several phones may remain online, while only the selected phone feeds VAD/ASR, and reconnecting the selected phone resumes that choice. Each client snapshot also exposes `received_bytes` and `accepted_chunks`, counting only new PCM accepted into the host feed; duplicate chunks do not increase either counter. ADB is only an installation/debug harness and is not a runtime dependency for discovery, connection, selection, streaming, or recovery. See [`../../desktop/rabi-voice-client/README_en.md`](../../desktop/rabi-voice-client/README_en.md). RabiLink is not a configuration dependency for this LAN path.
...
Startup first performs real embedding inference against the official 3D-Speaker model in an isolated child process using ONNX Runtime plus kaldi-native-fbank. An incompatible model format or runtime may terminate that probe, but the RabiSpeech service must remain online and expose the reason through capabilities; an experimental voiceprint model must not take down TTS, ASR, microphone capture, or manual bindings. `max_samples_per_profile` bounds manually confirmed prototypes per person, while `max_unconfirmed_samples` bounds all samples that have not been manually confirmed. Frames below the configured RMS floor and segments with material cross-speaker overlap are not admitted as voiceprint samples. Unknown clusters use conservative complete-link admission: a new embedding must meet `cluster_threshold` against every stored sample in the cluster, preventing an intermediate or mixed sample from bridging different voices through centroid similarity. A temporary extra unknown cluster is safer than a false same-person merge.

Voiceprint matching is not speaker diarization. ASR must at least return reliable `start/end` boundaries before RabiSpeech can extract separate multi-speaker embeddings. Provider `speaker` labels may still be wrong: when one label appears across multiple disjoint turns, the voiceprint layer preserves the raw `speaker`, assigns a per-turn `speaker_label`, and clusters each turn independently so voiceprints can correct a Provider merge. For local ASR such as faster-whisper that returns timestamps but no speaker labels, RabiSpeech groups consecutive segments separated by `unlabeled_turn_gap_seconds` (default `0.5` seconds) into temporary `voice#turn-N` turns, extracts each turn independently, and lets voiceprint clustering merge turns from the same voice. Audio without timestamps, without enough inter-turn silence, or with overlapping speakers still cannot be separated reliably.
