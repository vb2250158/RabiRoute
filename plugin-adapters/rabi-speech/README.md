...
每个成品文件按自己的 mtime 默认保留 `1440` 分钟（24 小时），不按自然日整批删除。服务启动、写入新 TTS 成品以及后台每 60 秒的周期检查都会清理已超过窗口的人格与 fallback 缓存；实际删除最多受一个检查周期影响，因此记录中的预计过期时间不是精确到秒的删除承诺。
...
启动时会先在独立子进程中探测模型与当前 `sherpa-onnx` 的兼容性。模型格式或 native runtime 不兼容时，探测进程可以失败，但 RabiSpeech 主服务必须继续在线并通过 capability 返回具体原因；不得让实验性声纹模型拖垮 TTS、ASR、麦克风或人工绑定。`max_samples_per_profile` 限制每个人工确认人物保留的原型数，`max_unconfirmed_samples` 限制尚未人工确认的聚类/匹配样本总量；过低 RMS 的帧和明显跨说话人重叠的片段不会进入声纹样本。

声纹匹配不等于说话人分离。只有 ASR 返回了可靠的 `speaker + start/end` 分段时，RabiSpeech 才能分别提取多人 embedding；普通 ASR 没有分段标签时，一次 VAD 切片仍按单个临时 `voice` 处理，不能识别同一切片中的轮流说话或重叠说话。

`scripts/benchmark_speaker_models.py` 使用同一批私有 WAV 同时评估芬妮 68 维频谱基线、ERes2NetV2 和 CAM++，输出 EER、FAR、FRR、已知人物识别率、未知保留率与 p50/p95 延迟。语料清单格式见 `benchmarks/speaker-cases.example.json`；真实录音必须留在 Git 忽略目录，不进入公开报告。
