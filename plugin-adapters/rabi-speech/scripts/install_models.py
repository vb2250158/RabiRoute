...
def main() -> int:
    ...
    aliases = list(MODEL_REPOSITORIES) if "all" in selected else list(dict.fromkeys(selected))
    ...
    try:
        ...
        raise SystemExit("huggingface_hub is required; install it in the downloader environment.") from exc
...
def write_manifest(root: Path, results: list[InstallResult]) -> None:
    ...
    manifest = {
        ...
        "models": [merged[key] for key in MODEL_REPOSITORIES if key in merged],
    }
