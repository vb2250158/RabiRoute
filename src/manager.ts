...
type GatewayDefinition = {
  ...
  botNickname?: string;
  qclawGatewayUrl?: string;
  qclawGatewayToken?: string;
  qclawModel?: string;
  dataDir?: string;
...
function envFor(definition: GatewayDefinition): NodeJS.ProcessEnv {
  return {
    ...
    QCLAW_GATEWAY_URL: definition.qclawGatewayUrl ?? process.env.QCLAW_GATEWAY_URL ?? "http://127.0.0.1:10978",
    QCLAW_GATEWAY_TOKEN: definition.qclawGatewayToken ?? process.env.QCLAW_GATEWAY_TOKEN ?? "",
    QCLAW_MODEL: definition.qclawModel ?? process.env.QCLAW_MODEL ?? "qclaw/pool-deepseek-v4-pro",
    TARGET_GROUP_ID: definition.targetGroupId ?? "",
    BOT_NICKNAME: definition.botNickname ?? process.env.BOT_NICKNAME ?? "胖虎助手",
...
function runtimeStatus(runtime: GatewayRuntime): Record<string, unknown> {
  return {
    ...
    qclawGatewayUrl: runtime.definition.qclawGatewayUrl ?? "http://127.0.0.1:10978",
    qclawModel: runtime.definition.qclawModel ?? "qclaw/pool-deepseek-v4-pro",
    botNickname: runtime.definition.botNickname ?? "胖虎助手",
  ...
  const cards = gateways.map((gateway) => {
    ...
    return `
      <section class="card">
        ...
        <div class="meta">QClaw=${escapeHtml(String(gateway.qclawGatewayUrl))}</div>
        <div class="meta">Model=${escapeHtml(String(gateway.qclawModel))}</div>
        <div class="actions">
...
<body>
  <header><h1>QClaw Gateway Manager</h1></header>
  <main>${cards}</main>
</body>
</html>`);
}
