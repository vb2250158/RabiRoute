...
export type SpeechTranscriptWord = {
  id?: number;
  word: string;
  start?: number;
  end?: number;
...
export type SpeechTranscriptSegment = {
  ...
  /** Stable opaque host-local evidence used by personas; never a host profile or suggestion id. */
  voiceprintId?: string;
  speakerScore?: number;
...
export type SpeechMessageCommand = {
  ...
  /** Stable ASR recording identity generated by the speech host. */
  ...
  /** Logical Route message endpoint selected after ASR. */
  messageAdapterType?: "speech" | "rabilink" | null;
