...
export function proactiveCommunicationPolicyLines(mode: AgentCommunicationMode): string[] {
  if (mode === "explicit") {
    return [
      ...
      "只有纯结束语或表情、完全重复的消息、机器人自己发出的消息，或者已有成员已经完整回答且本角色没有任何新增价值时，才允许不回复；例如对方问“我这边改 ok?”且负责人已经回复“ok”，本角色应保持安静。内部结论必须写明具体命中了哪一种情况。"
    ];
  }
  if (mode === "ambient") {
    return [
      ...
      "其他成员已经完整回答且本角色没有新增价值时保持安静；例如对方问“我这边改 ok?”且负责人已经回复“ok”，不要再补总结、建议或验收要求，也不要为了显得积极而制造回复链。"
    ];
  }
  if (mode === "heartbeat") {
    return [
