...
export function resolveCanonicalPlanStorageLocation(
  ...
  for (const bucket of ["active", "archive"] as const) {
    ...
    for (const entry of entries) {
      ...
      const plan = JSON.parse(fs.readFileSync(planFile, "utf8")) as { id?: unknown; status?: unknown; archiveStatus?: unknown; activationStatus?: unknown };
      ...
      const isArchived = plan.activationStatus !== undefined ? plan.activationStatus === "已归档" : plan.archiveStatus === "已归档" || plan.status === "已归档";
      const expectedBucket: PlanStorageBucket = isArchived ? "archive" : "active";
