...
export function agentCommunicationToolDenial(
  ...
  return {
    ...
    reason: [
      ...
      "Agent 间投递必须填写 messageSource.type、messageSource.agentAdapter、messageSource.sessionName、messageSource.sessionId、sourceThreadId、sourceAgentType 和 responsePolicy；messageSource.sessionId 必须等于 sourceThreadId，responsePolicy 只允许 required 或 none。",
      "responsePolicy=required 时还必须填写 responseInstruction。",
      "回复已有请求时必须填写 inReplyToRequestId、result、nextAction，并再次明确 responsePolicy。",
      "示例：{\"action\":\"send\",\"threadId\":\"目标任务ID\",\"cwd\":\"目标工作目录\",\"messageSource\":{\"type\":\"agent\",\"agentAdapter\":\"codex\",\"sessionId\":\"当前任务ID\",\"sessionName\":\"当前任务名称\"},\"sourceThreadId\":\"当前任务ID\",\"sourceAgentType\":\"plan_agent\",\"responsePolicy\":\"required\",\"responseInstruction\":\"请完成后回复结果和下一步\",\"prompt\":\"重新编写的投递内容\"}"
    ].join("\n")
  };
}
