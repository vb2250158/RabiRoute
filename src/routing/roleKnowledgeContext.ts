...
export function planMemoryApiHint(roleId: unknown): string[] {
  ...
  return [
    ...
    "- 当前步骤等待审批、方案确认或授权时必须 isBlocked=true，blockedBy 具体写明谁要批准什么；秘书继续追问，但不得续投业务实施，审批通过、否决或取消后同轮清除或更新阻塞",
    "- 请求审批前必须补齐 approvalRequest：approver、request、recommendation、alternatives、reason、files/commands/changes、validation、rollback、outOfScope、requestedAt、sourceMessageId 或 feedbackId、responseStatus；信息不完整时保持阻塞并禁止审批",
