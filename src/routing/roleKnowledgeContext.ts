...
export function planMemoryApiHint(roleId: unknown): string[] {
  ...
  return [
    ...
    "- 调查、审批、执行、打包、QA、讨论、暂停、完成和关闭分别使用 planWorkflow.roles 中对应角色所指的状态 key；写计划前先读取状态目录，不猜 key",
    ...
    "- 请求审批前必须补齐 approvalRequest：approver、request、recommendation、alternatives、reason、files/commands/changes、validation、rollback、outOfScope、requestedAt、sourceMessageId 或 feedbackId、responseStatus；信息不完整时使用 roles.analysis，完整等待回执时才使用 roles.approval",
...
function focusedApiHint(roleId: unknown): string[] {
  ...
  return [
    ...
    `先 GET ${base}/plan-statuses。plan.status 只保存当前人格启用状态的 key；展示和行为来自 planWorkflow 配置。审批前、待审批和获批执行分别使用 roles.analysis、roles.approval、roles.execution 所指的 key。`,
