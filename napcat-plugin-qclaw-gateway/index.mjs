...
let logger = null;
let currentConfig = {
  gatewayRoot: "C:\\Users\\GAN\\.qclaw\\workspace\\NapCatQClawGateway",
...
function startManagerProcess() {
  ...
  logger?.info(`QClaw 网关管理器已启动，pid=${managerProcess.pid ?? "未知"}`);
}
...
const plugin_init = async (ctx) => {
  ...
  try {
    ...
    logger?.warn("读取 QClaw Gateway 插件配置失败", error);
  }
  ...
  ctx.router.page({
    ...
    description: "管理多个 QQ 到 QClaw (OpenClaw) 的网关"
  });

  logger?.info("QClaw Gateway 插件已初始化");
};
...
const plugin_cleanup = async () => {
  logger?.info("QClaw Gateway 插件已清理");
};
